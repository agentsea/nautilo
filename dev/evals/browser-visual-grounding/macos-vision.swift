...
while argumentIndex < arguments.count {
    ...
    if argument == "--recognition" {
        ...
        guard argumentIndex < arguments.count,
              ["fast", "accurate", "hybrid"].contains(arguments[argumentIndex]) else {
            fputs("--recognition requires fast, accurate, or hybrid\n", stderr)
            exit(1)
        }
...
for imagePath in imagePaths {
    ...
    if recognitionMode == "hybrid" {
        let candidates = rectangles.filter { rectangle in
        ...
        for candidate in candidates {
            ...
            do {
                try VNImageRequestHandler(cgImage: crop).perform([cropRequest])
            } catch {
                continue
            }
