...
interface SharedAgentLiveShadowPreparedAttemptBase {
...
export type SharedAgentLiveShadowPreparedAttempt = SharedAgentLiveShadowPreparedAttemptBase & (
...
export type SharedAgentLiveShadowAdmissionResult =
  | Readonly<{
...
export interface SharedAgentLiveShadowAdmissionDependencies {
  ...
  readonly resolveCurrentHumanAuthority: Parameters<
    typeof admitSharedAgentLiveShadowMessage
...
function fallback(
  operationId: string,
  reason: Extract<SharedAgentLiveShadowAdmissionResult, {
    status: "ordinary_fallback";
  }>["reason"],
...
function destroyPlan(plan: Awaited<
  ReturnType<typeof admitSharedAgentLiveShadowMessage>
>["plan"]): void {
...
export async function admitAndPersistSharedAgentLiveShadowMessage(
  deps: SharedAgentLiveShadowAdmissionDependencies,
  input: SharedAgentLiveShadowPreparedAttempt,
): Promise<SharedAgentLiveShadowAdmissionResult> {
  const full = input.representationMode === "full_encryption";
  let admitted: Awaited<ReturnType<typeof admitSharedAgentLiveShadowMessage>>;
  try {
    admitted = await admitSharedAgentLiveShadowMessage({
      crypto: deps.crypto,
