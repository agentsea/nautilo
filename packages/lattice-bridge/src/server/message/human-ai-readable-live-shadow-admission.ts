...
interface HumanAiReadableLiveShadowPreparedAttemptBase {
...
export type HumanAiReadableLiveShadowPreparedAttempt = HumanAiReadableLiveShadowPreparedAttemptBase & (
...
export type HumanAiReadableLiveShadowAdmissionResult =
  | Readonly<{
...
export interface HumanAiReadableLiveShadowAdmissionDependencies {
  ...
  readonly resolveCurrentHumanAuthority: Parameters<
    typeof admitHumanAiReadableLiveShadowMessage
...
function fallback(
  operationId: string,
  reason: Extract<HumanAiReadableLiveShadowAdmissionResult, {
    status: "ordinary_fallback";
  }>["reason"],
...
function destroyPlan(plan: Awaited<
  ReturnType<typeof admitHumanAiReadableLiveShadowMessage>
>["plan"]): void {
...
/** Admit, append, and complete one topology-neutral Human AI-readable sibling. */
export async function admitAndPersistHumanAiReadableLiveShadowMessage(
  deps: HumanAiReadableLiveShadowAdmissionDependencies,
  input: HumanAiReadableLiveShadowPreparedAttempt,
): Promise<HumanAiReadableLiveShadowAdmissionResult> {
  const full = input.representationMode === "full_encryption";
  let admitted: Awaited<ReturnType<typeof admitHumanAiReadableLiveShadowMessage>>;
  try {
    admitted = await admitHumanAiReadableLiveShadowMessage({
      crypto: deps.crypto,
  ...
  try {
    ...
    try {
      allocated = await deps.product.appendAllocated({
        ...
        // M298 checkpoint 1 reuses the additive M296 Human-operation ledger;
        // only its immutable authenticated plan/request family changes here.
        sharedAgentLiveShadow: {
