...
export interface PreparedSharedAgentLiveShadowMessage {
...
export type PrepareVaultSharedAgentLiveShadowMessageResult =
  | Readonly<{
      status: "prepared";
      value: PreparedSharedAgentLiveShadowMessage;
    }>
  | Readonly<{
      status: "unavailable";
      reason:
...
export interface PrepareVaultSharedAgentLiveShadowMessageInput {
...
function unavailable(
  reason: Extract<PrepareVaultSharedAgentLiveShadowMessageResult, {
    status: "unavailable";
  }>["reason"],
): PrepareVaultSharedAgentLiveShadowMessageResult {
...
export async function prepareVaultSharedAgentLiveShadowMessage(
  input: PrepareVaultSharedAgentLiveShadowMessageInput,
): Promise<PrepareVaultSharedAgentLiveShadowMessageResult> {
  let plan;
  try {
    plan = decodeSharedAgentLiveShadowMessagePlanV1(input.planBytes);
  } catch {
        ...
        async (profileBytes) => {
          ...
          try {
            ...
            if (snapshot.kind !== "human-v2") {
              throw new TypeError("Shared-Agent preparation kind disagrees");
            }
            ...
            if (namespaceEnvelopeBytes === undefined) {
              throw new TypeError("Shared-Agent Namespace envelope is absent");
            }
            canonicalPlanBytes =
              encodeSharedAgentLiveShadowMessagePlanV1(plan);
            const request = prepareSharedAgentLiveShadowMessageRequest(
              input.crypto,
              {
                subjectHumanId: plan.subjectHumanId,
                operationId: plan.operationId,
