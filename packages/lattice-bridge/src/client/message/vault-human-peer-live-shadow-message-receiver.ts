...
type HumanPeerEvent = Extract<LiveShadowMessageRealtimeEventV1 | FullEncryptionMessageRealtimeContentEventV2, {
  type: "message.human_peer_shadow";
}>;

type HumanPeerLiveShadowReceiveBase = Readonly<{
  operationId: string;
  messageId: string;
...
export type HumanPeerLiveShadowReceiveResult =
  | HumanPeerLiveShadowReceiveBase & Readonly<{
      status: "verified"; payload: MessagePayloadV2;
    }>
  | HumanPeerLiveShadowReceiveBase & Readonly<{
      status: "fallback"; payload?: MessagePayloadV2;
    }>;

export interface VaultHumanPeerLiveShadowMessageReceiver {
  receive(
    event: unknown,
    ordinarySibling?: MessagePayloadV2,
  ): Promise<HumanPeerLiveShadowReceiveResult | null>;
}

export interface HumanPeerLiveShadowReceiverApiPort {
  planHumanPeerLiveShadowAcknowledgement(input: Readonly<{
    roomId: string;
    operationId: string;
  ...
  >;
  acknowledgeHumanPeerLiveShadowMessage(input: Readonly<{
    roomId: string;
    operationId: string;
...
function destroyPlan(plan: ReturnType<
  typeof decodeHumanPeerLiveShadowMessagePlanV1
>): void {
...
export function createVaultHumanPeerLiveShadowMessageReceiver(input: Readonly<{
  crypto: LatticeCrypto;
  vault: ClientProfileVault;
  coordinates: ClientProfileCoordinates;
  namespaceAuthority: NamespaceAuthorityClient;
  api: HumanPeerLiveShadowReceiverApiPort;
  now?: () => number;
  ...
  return Object.freeze({
    async receive(candidate: unknown, ordinarySibling?: MessagePayloadV2) {
      ...
      try {
        ...
        if (parsed.type !== "message.human_peer_shadow") return null;
        event = parsed;
      } catch {
        return null;
      }
      ...
      let plan: ReturnType<typeof decodeHumanPeerLiveShadowMessagePlanV1>
        | undefined;
      let fallbackPayload = ordinarySibling;
      let status: "verified" | "fallback" = "fallback";
      let reason: HumanPeerLiveShadowReceiveResult["reason"] =
        "transport_unavailable";
      try {
        ...
        ordinaryBytes = event.wireVersion === 1
          ? fromBase64url(event.ordinaryPayloadBytesBase64url)
          : undefined;
        ...
        const ordinaryPayload = ordinaryBytes === undefined
          ? undefined : decodeMessagePayloadV2(ordinaryBytes);
        fallbackPayload = ordinaryPayload ?? ordinarySibling;
        const visibleSiblingBytes = ordinarySibling === undefined
          ? undefined : encodeMessagePayloadV2(ordinarySibling);
        try {
          if (
            ...
            || (event.wireVersion === 1 && (
              ordinaryBytes === undefined || visibleSiblingBytes === undefined
              ...
              if (protectedPayload.status !== "encrypted") {
                ...
                try {
                  ...
                  if (opened.status !== "opened" || opened.value === null) {
                    ...
                    if (
                      (ordinaryBytes !== undefined && !equal(openedPayload, ordinaryBytes))
                      || (visibleSiblingBytes !== undefined && !equal(openedPayload, visibleSiblingBytes))
                    ) {
                      reason = "parity_mismatch";
                    } else {
                  ...
                  async (profileBytes) => {
                    ...
                    try {
                      ...
                      let acknowledgement: ReturnType<
                        typeof prepareHumanPeerLiveShadowAcknowledgement
                      > | undefined;
                      try {
                        acknowledgement =
                          prepareHumanPeerLiveShadowAcknowledgement(
                        ...
                        await input.api.acknowledgeHumanPeerLiveShadowMessage({
                          roomId: currentPlan.roomId,
                          operationId: currentPlan.operationId,
