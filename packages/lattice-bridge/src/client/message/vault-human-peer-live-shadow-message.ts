...
export interface PreparedHumanPeerLiveShadowMessage {
...
export type PrepareVaultHumanPeerLiveShadowMessageResult =
  | Readonly<{
      status: "prepared";
      value: PreparedHumanPeerLiveShadowMessage;
    }>
  | Readonly<{
      status: "unavailable";
      reason:
...
export interface PrepareVaultHumanPeerLiveShadowMessageInput {
...
function unavailable(
  reason: Extract<PrepareVaultHumanPeerLiveShadowMessageResult, {
    status: "unavailable";
  }>["reason"],
): PrepareVaultHumanPeerLiveShadowMessageResult {
...
export async function prepareVaultHumanPeerLiveShadowMessage(
  input: PrepareVaultHumanPeerLiveShadowMessageInput,
): Promise<PrepareVaultHumanPeerLiveShadowMessageResult> {
  let plan;
  try {
    plan = decodeHumanPeerLiveShadowMessagePlanV1(input.planBytes);
  } catch {
        ...
        async (profileBytes) => {
          ...
          try {
            ...
            if (snapshot.kind !== "human-v2") {
              throw new TypeError("Human-peer preparation kind disagrees");
            }
            ...
            if (namespaceEnvelopeBytes === undefined) {
              throw new TypeError("Human-peer Namespace envelope is absent");
            }
            canonicalPlanBytes =
              encodeHumanPeerLiveShadowMessagePlanV1(plan);
            const request = prepareHumanPeerLiveShadowMessageRequest(
              input.crypto,
              {
                subjectHumanId: plan.subjectHumanId,
                operationId: plan.operationId,
