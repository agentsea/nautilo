...
```typescript
import { initialize, MemDocStore } from '@nautilo/office-docs';
...
pnpm --filter @nautilo/office-docs dev          # Start Vite dev server with demo
pnpm --filter @nautilo/office-docs test         # Run unit tests (Vitest)
pnpm --filter @nautilo/office-docs build        # Library build (ESM + CJS)
pnpm --filter @nautilo/office-docs typecheck    # TypeScript check
```
