...
import { completeNestedWork, type NestedWork } from './nested-work.js';
...
function* layoutCellBlocks(
...
): Generator<NestedWork<LayoutTable>, { lines: LayoutLine[]; blockBoundaries: number[] }, LayoutTable> {
  if (blocks.length === 0) {
  ...
  for (const block of blocks) {
    ...
    if (block.type === 'table' && block.tableData) {
      const nestedLayout = yield computeTableLayoutWork(
        block.tableData,
        block.id,
        measurer,
        maxWidth,
        composingContext,
        docStyles,
        // Nested work carries the surface too: without it a heading nested two
...
export function computeTableLayout(
  ...args: Parameters<typeof computeTableLayoutWork>
): LayoutTable {
  return completeNestedWork(computeTableLayoutWork(...args));
}

function* computeTableLayoutWork(
  tableData: TableData,
  tableBlockId: string,
  measurer: TextMeasurer,
  contentWidth: number,
  ...
  for (let r = 0; r < numRows; r++) {
    ...
    for (let c = 0; c < numCols; c++) {
      ...
      const { lines, blockBoundaries } = yield* layoutCellBlocks(
  ...
  const rowHeights = Array.from<number>({ length: numRows }).fill(0);
...
export function findRowSplitHeight(
  ...args: Parameters<typeof findRowSplitHeightWork>
): number {
  return completeNestedWork(findRowSplitHeightWork(...args));
}

function* findRowSplitHeightWork(
  layout: LayoutTable,
  rowIndex: number,
      ...
      if (line.nestedTable) {
        ...
        for (let nr = 0; nr < nt.rowHeights.length; nr++) {
          ...
          if (rowEnd <= availableHeight) {
            ...
            if (nestedAvail > 0) {
              const innerSplit = yield findRowSplitHeightWork(nt, nr, nestedAvail);
              if (innerSplit > 0) {
