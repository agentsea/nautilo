...
import { validateRichPaste, type RichPasteContent } from './paste-validation.js';
...
export class TextEditor {
  ...
  /**
   * Optional host admission for structurally valid rich paste. It runs before
   * snapshots, selection deletion, or any document write. Full-document Docs
   * leaves this unset; constrained hosts such as Slides text boxes can reject
   * block kinds they cannot lay out or paint.
   */
  richPasteAdmission: ((content: RichPasteContent) => boolean | void) | null = null;
  ...
  private setCanvasCursor(cursor: string): void {
    const target = this.cursorTarget
      ?? (this.container.querySelector('canvas[data-role="doc-canvas"]'));
  ...
  private applyPastePlan(plan: PastePlan): void {
    if (plan.kind !== 'text') {
      if (plan.kind === 'blocks') {
        validateRichPaste({ blocks: plan.blocks });
        if (this.richPasteAdmission?.({ blocks: plan.blocks }) === false) return;
      } else {
        validateRichPaste({ tableCells: plan.cells });
        if (this.richPasteAdmission?.({ tableCells: plan.cells }) === false) return;
      }
    }
  ...
  private handleTab(shift: boolean): void {
    ...
    const changes: Array<{ block: Block; level: number }> = [];
    this.forEachBlockInSelection((b) => {
      if (b.type !== 'list-item') return;
      const currentLevel = b.listLevel ?? 0;
      const newLevel = shift
        ? Math.max(0, currentLevel - 1)
        : indentedListLevel(currentLevel);
      if (newLevel === currentLevel) return;
      changes.push({ block: b, level: newLevel });
    });
    if (changes.length === 0) return;

    this.saveSnapshot();
    for (const { block, level } of changes) {
  ...
  private handleIndent(): void {
    ...
    const changes: Array<
      ...
      | { block: Block; marginLeft: number }
    > = [];
    this.forEachBlockInSelection((block) => {
      if (block.type === 'list-item') {
        changes.push({ block, level: indentedListLevel(block.listLevel ?? 0) });
      } else {
        const marginLeft = (block.style.marginLeft ?? 0) + INDENT_STEP;
        if (!Number.isFinite(marginLeft)) {
          throw new RangeError(
            'Cannot indent this block further: its margin cannot be represented safely.',
          );
        }
        changes.push({ block, marginLeft });
      }
    });
    if (changes.length === 0) return;
    ...
    for (const change of changes) {
      if ('level' in change) {
        this.doc.setBlockType(change.block.id, 'list-item', {
          ...
          listLevel: change.level,
        });
      } else {
        this.doc.applyBlockStyle(change.block.id, { marginLeft: change.marginLeft });
      }
    }
    this.invalidateLayout();
    this.requestRender();
   ...
   * level: descending into a cell, the next iteration uses `cellLocalX`
   ...
   *
   * Iterates via `line.nestedTable` when the resolved cell's target line is
  ...
  private resolveOffsetInNestedTable(
    ...
    while (true) {
      ...
      // Further nesting — continue into it instead of stopping here.
      if (targetLine?.nestedTable) {
        const nestedBlock = cellData.blocks[blockIdx];
        if (nestedBlock?.tableData) {
          tableBlock = nestedBlock;
          layoutTable = targetLine.nestedTable;
          tableAbsY = cellOriginY + targetLine.y;
          localX = cellLocalX;
          continue;
        }
