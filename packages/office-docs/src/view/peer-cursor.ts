 ...
 * Usernames wider than the canvas are projected with an ellipsis (U+2026).
 * The complete name remains with the caller and is redrawn on a wider canvas.
 *
...
export function drawPeerLabel(
  ...
  const paddingX = 4;
  const paddingY = 2;
  const maxWidth = Math.max(0, canvasWidth - paddingX * 2);
  const radius = 2;
  ...
  if (textWidth > maxWidth) {
    ...
    while (textWidth > maxWidth && characters.length > 0) {
      characters.pop();
