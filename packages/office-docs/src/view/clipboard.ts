...
export function serializeBlocks(blocks: Block[]): string {
  ...
  return stringifyJsonData(payload);
}
...
export function deserializeBlocks(json: string): Block[] {
  try {
    ...
    const work: ClipboardWork = [];
    const blocks = sanitizeBlocks(payload.blocks, work);
    drainClipboardWork(work);
    return blocks;
  } catch {
    return [];
  }
...
// Parsed JSON is acyclic. An explicit work stack visits every nested table
// without spending the JavaScript call stack or dropping a subtree by depth.
type ClipboardWork = Array<() => void>;
function drainClipboardWork(work: ClipboardWork): void {
...
function sanitizeCell(value: unknown, work: ClipboardWork): TableCell {
  ...
  work.push(() => {
    cell.blocks = sanitizeBlocks(record.blocks, work);
    if (cell.blocks.length === 0) cell.blocks = [sanitizeBlock({}, work)!];
  });
...
function sanitizeCellRows(value: unknown, work: ClipboardWork): TableCell[][] | undefined {
  if (!Array.isArray(value)) return undefined;
  const rows: TableCell[][] = [];
  for (const rawRow of value) {
    if (!Array.isArray(rawRow)) continue;
    rows.push(rawRow.map((cell) => sanitizeCell(cell, work)));
  }
  return rows.length > 0 ? rows : undefined;
}

function sanitizeTableData(value: unknown, work: ClipboardWork): TableData | undefined {
  if (!isRecord(value)) return undefined;
  ...
  for (let i = 0; i < rawRows.length; i++) {
    ...
    rows.push({ cells: rawRow.cells.map((cell) => sanitizeCell(cell, work)) });
    sourceIndices.push(i);
  }
  if (rows.length === 0) return undefined;
  const cols = rows.reduce((width, row) => Math.max(width, row.cells.length), 0);
  ...
  for (const row of rows) {
    while (row.cells.length < cols) row.cells.push(sanitizeCell({}, work));
  }
 ...
 */
function sanitizeBlock(value: unknown, work: ClipboardWork): Block | null {
  const record = isRecord(value) ? value : {};
  ...
  if (type === 'table') {
    const tableData = sanitizeTableData(record.tableData, work);
...
function sanitizeBlocks(value: unknown, work: ClipboardWork): Block[] {
  if (!Array.isArray(value)) return [];
  const blocks: Block[] = [];
  for (const raw of value) {
    const block = sanitizeBlock(raw, work);
...
export function deserializeClipboard(json: string): ClipboardData {
  try {
    ...
    const work: ClipboardWork = [];
    const result = {
      blocks: sanitizeBlocks(payload.blocks, work),
      tableCells: sanitizeCellRows(payload.tableCells, work),
    };
    drainClipboardWork(work);
    return result;
  } catch {
...
export function cloneTableCells(cells: TableCell[][]): TableCell[][] {
  const copy = cloneJsonData(cells);
  const roots: Block[][] = [];
  for (const row of copy) for (const cell of row) roots.push(cell.blocks);
  for (const blocks of walkBlockArrays(roots)) {
    for (const block of blocks) block.id = generateBlockId();
  }
  return copy;
}
