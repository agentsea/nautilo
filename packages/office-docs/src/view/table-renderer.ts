...
import {
  completeNestedWork, type NestedWork } from './nested-work.js';
import {
...
export { computeMergedCellLineLayouts } from './table-geometry.js';
...
export function renderTableBackgrounds(
  ...args: Parameters<typeof renderTableBackgroundsWork>
): void {
  completeNestedWork(renderTableBackgroundsWork(...args));
}

function* renderTableBackgroundsWork(
  ctx: CanvasRenderingContext2D,
  tableData: TableData,
  tableLayout: LayoutTable,
  tableX: number,
      ...
      for (let li = 0; li < layoutCell.lines.length; li++) {
        ...
        if (line.nestedTable) {
          ...
          if (nestedBlock?.tableData) {
            yield renderTableBackgroundsWork(
...
export function renderTableContent(
  ...args: Parameters<typeof renderTableContentWork>
): void {
  completeNestedWork(renderTableContentWork(...args));
}

function* renderTableContentWork(
  ctx: CanvasRenderingContext2D,
  tableData: TableData,
  tableLayout: LayoutTable,
  tableX: number,
