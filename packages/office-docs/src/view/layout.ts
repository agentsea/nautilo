...
export function computeListCounters(blocks: Block[]): Map<string, string> {
  ...
  for (const block of blocks) {
    if (block.type !== 'list-item' || block.listKind !== 'ordered') {
      levelCounters.clear(); // Reset on non-list block
      continue;
    }
    const level = block.listLevel ?? 0;
    const count = (levelCounters.get(level) ?? 0) + 1;
    levelCounters.set(level, count);
    // Reset only represented deeper levels, without allocating or walking
    // an array whose length is controlled by the numeric indentation level.
    for (const key of levelCounters.keys()) {
      if (key > level) levelCounters.delete(key);
    }
    counters.set(block.id, formatOrderedMarker(count, level));
  }
  return counters;
}
