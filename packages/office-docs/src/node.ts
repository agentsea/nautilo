...
// DOM-free public surface for `@nautilo/office-docs`.
//
...
//
// Wired in two places:
//   - `packages/backend/tsconfig.json` paths map `@nautilo/office-docs`
...
//     subpath (`@nautilo/office-docs/node`) for downstream consumers that
...
// Store contracts and search results are data-only; browser construction remains injected.
export type { DocStore } from './store/store.js';
export type { SearchMatch } from './model/types.js';
export type {
  Document,
  Block,
  // `@nautilo/office-slides`'s PPTX import/export reads it from this entry (it is
...
export {
  ...
  // path both resolve `@nautilo/office-docs` to this file), so leaving it out left
...
// slides PPTX exporter (a Node consumer via `@nautilo/office-slides/node`)
...
// (frontend) imports these from `@nautilo/office-docs`; the docs comments
