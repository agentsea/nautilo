 ...
 * so `@nautilo/office-docs` gains no `@yorkie-js/sdk` dependency. There are two
...
export function docsTreeToDocument(
  ...
  if (typeof opts?.stylesJson === 'string' && opts.stylesJson.length > 0) {
    try {
      const parsed: unknown = JSON.parse(opts.stylesJson);
      if (parsed !== null && typeof parsed === 'object' && !Array.isArray(parsed)) {
        doc.styles = parsed as NonNullable<Document['styles']>;
      }
    } catch {
