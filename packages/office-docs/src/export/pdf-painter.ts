...
export class PdfPainter {
  ...
  static async embedAllFonts(
    ...
    for (const key of Object.keys(LATIN_STANDARD_FONTS)) {
      out[key as PdfFontKey] = pdfDoc.embedStandardFont(
  ...
  static paintPage(
    ...
    completeNestedWork(PdfPainter.paintPageWork(
      page, layoutPage, pageSetup, fonts, ctx,
    ));
  }

  private static *paintPageWork(
    page: PDFPage,
    layoutPage: LayoutPage,
    pageSetup: PageSetup,
    fonts: EmbeddedFonts,
    ...
    while (i < layoutPage.lines.length) {
      ...
      if (lb && lb.block.type === 'table' && lb.layoutTable) {
        if (shouldStartTableRender(layoutPage, i)) {
          yield paintTablePageRangeWork(
            page, layoutPage, pl, i, lb, pageHeightPt,
            function* (cell, layoutCell, layoutTable, r, _c, rect, pageStartRow, endRowIndex) {
              yield PdfPainter.paintCellContentWork(
  ...
  static paintCellContent(
    ...
    completeNestedWork(PdfPainter.paintCellContentWork(
      page, cell, layoutCell, layoutTable, row, rect, pageStartRow,
      endRowIndex, pageHeightPt, fonts, ctx,
    ));
  }

  private static *paintCellContentWork(
    page: PDFPage,
    cell: TableCell,
    layoutCell: LayoutTableCell,
    layoutTable: LayoutTable,
...
function toPdfUriLiteral(href: string): string {
  ...
  return new URL(href).href
    .replace(/[^\p{ASCII}]+/gu, (run) => encodeURIComponent(run))
    .replace(/[\\()]/g, (ch) => '\\' + ch);
}
