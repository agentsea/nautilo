...
${bodyXml}
  ...
  private static blockToXml(
    ...
    return completeNestedWork(DocxExporter.blockToXmlWork(block, imageEntries));
  }

  private static *blockToXmlWork(
    block: Block,
    imageEntries: ImageEntry[],
  ): NestedWork<string> {
    if (block.type === 'table' && block.tableData) {
      return yield DocxExporter.tableToXmlWork(block.tableData, imageEntries);
    }
  ...
  private static *tableToXmlWork(
    ...
    for (let rowIdx = 0; rowIdx < tableData.rows.length; rowIdx++) {
      ...
      for (let c = gridBefore; c < nCols - gridAfter; c++) {
        ...
        for (const child of cell.blocks) {
          cellParts.push(yield DocxExporter.blockToXmlWork(child, imageEntries));
        }
        const cellContent = cellParts.join('');
    ...
    return `    <w:tbl><w:tblPr><w:tblW w:w="0" w:type="auto"/></w:tblPr><w:tblGrid>${gridCols}</w:tblGrid>${rows.join('')}</w:tbl>`;
  }
...
${rels}
  ...
  private static countImageSrcs(blocks: Block[] | undefined): number {
    ...
    for (const blockArray of walkBlockArrays([blocks])) {
  ...
  private static async collectImages(
    ...
    for (const blockArray of walkBlockArrays([[block]])) {
      for (const current of blockArray) {
        for (const inline of current.inlines) {
          if (!inline.style.image) continue;
