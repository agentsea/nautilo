...
describe('drawPeerLabel', () => {
  ...
  it('uses the actual canvas width and restores the full name when resized', () => {
    const name = 'A readable complete peer name';
    const fillText = vi.fn<(text: string, x: number, y: number) => void>();
    const ctx = { ...makeCtx(), fillText,
      measureText: (text: string) => ({ width: text.length * 10 }),
    } as unknown as CanvasRenderingContext2D;
    const pixel = { x: 0, y: 50, height: 20 };
    drawPeerLabel(ctx, pixel, name, '#EF4444', 0, 100);
    expect(fillText.mock.calls[0][0]).toMatch(/…$/u);
    expect(fillText.mock.calls[0][0].length * 10).toBeLessThanOrEqual(92);
    drawPeerLabel(ctx, pixel, name, '#EF4444', 0, 600);
    expect(fillText.mock.calls[1][0]).toBe(name);
    drawPeerLabel(ctx, pixel, name, '#EF4444', 0, 5);
