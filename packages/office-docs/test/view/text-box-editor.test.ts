...
describe('TextBoxEditorAPI — formatting surface', () => {
  ...
  it('advances deep list indentation through the Slides text-box API', () => {
    const { api } = mount();
    api.setBlockType('list-item', { listKind: 'unordered', listLevel: 12 });
    api.indent();
    expect(api.getBlockType().listLevel).toBe(13);
    api.outdent();
    expect(api.getBlockType().listLevel).toBe(12);
    api.detach();
  });
