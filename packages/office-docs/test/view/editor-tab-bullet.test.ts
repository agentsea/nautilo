...
describe('Tab / Shift+Tab on multi-bullet selection', () => {
  ...
  test('Tab preserves and advances levels beyond eight; Shift+Tab stops at zero', () => {
    const { editor, container } = setupEditor([
      makeListItem('b1', 'deep', 8),
    ...
    expect(blocks.find((b) => b.id === 'b1')?.listLevel).toBe(9);
    ...
    expect(blocks.find((b) => b.id === 'b1')?.listLevel).toBe(7);
    expect(blocks.find((b) => b.id === 'b2')?.listLevel).toBe(6);
    expect(blocks.find((b) => b.id === 'b3')?.listLevel).toBe(0);
    editor.dispose();
  });

  test('toolbar and keyboard indent agree above the former ceiling and Undo restores it', () => {
    const { editor, container } = setupEditor([makeListItem('deep', 'nested item', 12)]);
    editor.indent();
    expect(editor.getDoc().document.blocks[0].listLevel).toBe(13);
    const textarea = container.querySelector('textarea')!;
    textarea.dispatchEvent(new KeyboardEvent('keydown', {
      key: ']', ctrlKey: true, bubbles: true, cancelable: true,
    }));
    expect(editor.getDoc().document.blocks[0].listLevel).toBe(14);
    editor.undo();
    expect(editor.getDoc().document.blocks[0].listLevel).toBe(13);
    editor.undo();
    expect(editor.getDoc().document.blocks[0].listLevel).toBe(12);
    editor.dispose();
  });
