...
/**
 * HTML of at least `chars` characters with real paragraphs. Large fixtures
 * pad the raw clipboard payload while keeping indicator tests inexpensive.
 */
function htmlOfSize(chars: number): string {
  if (chars >= THRESHOLD) {
    // The admission path measures the raw clipboard payload. A fixed number
    // of blocks plus inert padding exercises that boundary without making
    // every indicator assertion repeat a 20,000-node jsdom layout.
    const paragraphs = Array.from(
      { length: 128 },
      (_, i) => `<p>Paragraph ${i}</p>`,
    ).join('');
    const padding = 'x'.repeat(Math.max(0, chars - paragraphs.length - 7));
