...
describe('read-only image resize drag', () => {
  ...
  function resizeCursorCount(container: HTMLElement): number {
    return [...container.querySelectorAll('canvas')]
      .filter((c) => (c).style.cursor.endsWith('-resize'))
      .length;
  }
...
describe('read-only store and doc handles', () => {
  ...
  test('a prototype cannot be planted to recover the raw store', () => {
    ...
    let stolen: unknown;
    const capture = (value: unknown) => { stolen = value; };
    const maliciousPrototype = {};
    Object.defineProperty(maliciousPrototype, 'probe', {
      get(this: object) {
        capture(this);
        return undefined;
      },
    });
    expect((): void => { Object.setPrototypeOf(handle, maliciousPrototype); }).toThrow(TypeError);
