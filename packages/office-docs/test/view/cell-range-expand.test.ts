...
import type { Block, TableCell, TableData, TableCellRange } from '../../src/model/types.js';

function plainCell(): TableCell {
  return {
    blocks: [{ id: 'b', type: 'paragraph', inlines: [{ text: '', style: {} }], style: { ...DEFAULT_BLOCK_STYLE } }],
    style: {},
  };
}
...
function makeTable(rows: number, cols: number, overrides: Record<string, TableCell> = {}): TableData {
  const data: TableData = { rows: [], columnWidths: Array.from<number>({ length: cols }).fill(1 / cols) };
...
describe('Selection.getNormalizedRange — cell range expansion at read time', () => {
  it('expands a partially-overlapping cell range using layout TableData', () => {
    ...
    const tableBlock: Block = {
      id: 't', type: 'table', inlines: [], style: { ...DEFAULT_BLOCK_STYLE },
      tableData: t,
    };
