...
const para = (text: string, style: InlineStyle = {}, fontFamily?: string): Document => ({
  blocks: [{
    id: generateBlockId(),
    type: 'paragraph',
...
describe('scanFontsUsed', () => {
  ...
  it('scans fonts through deeply nested tables without consuming call stack', () => {
    let child = para('한글').blocks;
    for (let depth = 0; depth < 20_000; depth++) {
      child = [{
        ...
        style: { ...DEFAULT_BLOCK_STYLE },
        tableData: {
          rows: [{ cells: [{ blocks: child, style: {} }] }],
          columnWidths: [1],
        },
      }];
    }
    expect(scanFontsUsed({ blocks: child }).needsKR).toBe(true);
  });
});
...
describe('PdfFonts', () => {
  ...
  it('caches a font after first load (no second source call)', async () => {
    ...
    const fonts = new PdfFonts({
      sources: {
        'kr-sans-regular': () => { calls++; return Promise.resolve(TEST_FONT.buffer); },
      },
    });
  ...
  it('loads a custom font key from an injected source', async () => {
    const fonts = new PdfFonts({
      sources: {
        'custom:Roboto:regular': () => Promise.resolve(TEST_FONT.buffer),
      },
    });
  ...
  it('throws a clear error when source is missing', async () => {
    ...
    const fonts = new PdfFonts({ sources: {} });
    await expect(fonts.load('sans-regular' as unknown as import('../../src/export/pdf-fonts.js').PdfFontKey)).rejects.toThrow(/no source/i);
  });
});
