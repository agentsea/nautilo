...
describe('DocxExporter', () => {
  it('serializes deeply nested tables without consuming call stack', async () => {
    let child: Document['blocks'] = [{
      id: 'leaf',
      type: 'paragraph',
      inlines: [{ text: 'deep leaf', style: {} }],
    ...
    const blob = await DocxExporter.export({ blocks: child });
    const zip = await (await import('jszip')).default.loadAsync(await blob.arrayBuffer());
    const xml = await zip.file('word/document.xml')!.async('string');
    expect(xml).toContain('deep leaf');
    expect(xml.match(/<w:tbl>/g)).toHaveLength(3_000);
  });
