...
describe('PdfPainter table regressions', () => {
  ...
  it('paints a deeply nested table leaf without consuming call stack', async () => {
    const leaf = createTableBlock(1, 1);
    leaf.tableData!.rows[0].cells[0].blocks[0].inlines = [
      { text: 'Deep PDF leaf', style: {} },
    ];
    let outer = leaf;
    for (let depth = 0; depth < 1_500; depth++) {
      const parent = createTableBlock(1, 1);
      parent.tableData!.rows[0].cells[0].blocks = [outer];
      outer = parent;
    }
    const paintedText = await collectPaintedText({
      blocks: [outer],
      pageSetup: { ...DEFAULT_PAGE_SETUP },
    });
    expect(paintedText.join('')).toContain('Deep PDF leaf');
  });
...
describe('PdfPainter inline styles', () => {
  ...
  it('emits link annotations for href runs', async () => {
    ...
    const size = annotsRef instanceof PDFArray ? annotsRef.size() : 0;
