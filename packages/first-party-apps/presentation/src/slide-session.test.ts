...
import type { DocumentChange, DocumentEnvelope, SlidesBridge, SlidesRecoveryDraft, WriteResult } from "./slide-bridge";
import { SlideSession } from "./slide-session";
import type { SlidesSaveStatus } from "./slide-session";
...
function createFixture(initial = envelope("initial", 1)) {
  ...
  const writes: Array<{ content: string; base: Parameters<SlidesBridge["document"]["write"]>[1] }> = [];
  const copies: string[] = [];
  const replacements: string[] = [];
  const statuses: Array<{ state: SlidesSaveStatus; message?: string }> = [];
  ...
  const bridge: SlidesBridge = {
    document: {
      ...
      saveCopy: async (snapshot) => { copies.push(snapshot); return { path: "Recovery.presentation.html" }; },
  ...
  const view = {
    ...
    status: (state: SlidesSaveStatus, message?: string) => { statuses.push({ state, message }); },
...
function recoveryFixture(initial: SlidesRecoveryDraft | null = null) {
  let draft = initial;
  let revision: string | null = initial ? "journal-1" : null;
  let next = 1;
  const writes: Array<SlidesRecoveryDraft | null> = [];
  return {
    ...
    bridge: {
      ...
      async write(input: { expectedRevision: string | null; draft: SlidesRecoveryDraft | null }) {
...
describe("SlideSession crash recovery", () => {
  const record = (content: string, exact = true): SlidesRecoveryDraft => ({
  ...
  test("opens a recovery draft when the saved file is unavailable and saves only a copy", async () => {
    ...
    const session = new SlideSession(f.bridge, f.view);
    await session.start();
  ...
  test("checkpoints focused text independently of an unacknowledged canonical save", async () => {
    ...
    const session = new SlideSession(f.bridge, { ...f.view, recoverySnapshot: () => ({ content: pendingText, exact: true }) });
    await session.start();
  ...
  test("an unfinished media operation remains visible even when its last snapshot equals the saved file", async () => {
    const f = createFixture();
  ...
  test.each([false, true])("save retries a failed journal opening without replacing a discovered older draft (older=%s)", async older => {
    ...
    const session = new SlideSession(f.bridge, { ...f.view, recoverySnapshot: () => ({ content: "new focused text", exact: true }) });
    await session.start();
    session.setLocalEditing(true);
    session.changed();
    await session.save();
  ...
  test("focused typing retries transient recovery opening without a blur or canonical save", async () => {
    ...
    const session = new SlideSession(f.bridge, { ...f.view, recoverySnapshot: () => ({ content: "latest focused text", exact: true }) });
    await session.start();
    session.setLocalEditing(true);
    session.changed();
    session.changed();
  ...
  test("an acknowledged lifecycle copy clears recovery and does not restore into the original", async () => {
    ...
    const next = new SlideSession(reopened.bridge, reopened.view);
    await next.start();
    await settle();
  ...
  test("a delayed pre-copy snapshot cannot resurrect recovery after finalization", async () => {
    ...
    const session = new SlideSession(f.bridge, view);
    await session.start();
    session.setLocalEditing(true);
    await settle();
...
async function started(initial?: DocumentEnvelope) {
  ...
  const session = new SlideSession(fixture.bridge, fixture.view);
  await session.start();
...
describe("SlideSession", () => {
  ...
  test("saves a host copy without writing the open document", async () => {
    ...
    expect(fixture.statuses.at(-1)).toEqual({
      state: "saved",
      message: "Copy saved as Recovery.presentation.html. Your open file is unchanged.",
    });
  });
