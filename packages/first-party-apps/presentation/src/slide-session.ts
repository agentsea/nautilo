// Adapted from the qualified Sheets serial persistence lane; document authority stays in the host bridge.
import type { DocumentEnvelope, SlidesBridge } from "./slide-bridge";
import { SlideRecovery } from "./slide-recovery";
export type SlidesSaveStatus = "saved" | "unsaved" | "saving" | "conflict" | "error";
...
type SessionView = {
  ...
  status(state: SlidesSaveStatus, message?: string): void;
...
export class SlideSession {
  ...
  private recovery?: SlideRecovery;
  ...
  private lastStatus: SlidesSaveStatus = "saved";
  private lastStatusMessage?: string;

  constructor(private bridge: SlidesBridge, private view: SessionView) {
    if (bridge.context.mode !== "preview" && bridge.recovery) this.recovery = new SlideRecovery(bridge.recovery);
  }
  ...
  private status(state: SlidesSaveStatus, message?: string): void {
  ...
  private recoveryFailed(error: unknown): void {
    this.recoveryError = `Crash recovery could not be updated. Keep this presentation open until it saves. ${error instanceof Error ? error.message : ""}`.trim();
  ...
  private retryRecovery(): void {
    ...
    this.recoveryOpening = this.recovery.read().then(draft => {
      ...
      if (draft && (!draft.exact || draft.content !== this.base?.content)) {
        ...
        this.recoveryError = "A previous recovery copy is still present. Save your current edits or a copy before reopening this presentation to recover it.";
        this.renderStatus();
        return;
      }
  ...
  saveCopy(commit?: () => void | Promise<void>): Promise<void> {
    return this.enqueue(async () => {
      if (!this.bridge.document.saveCopy) throw new Error("Save a copy is unavailable. Keep this presentation open and retry saving.");
  ...
  finalizeRecoveryCopy(content: string): Promise<void> {
    return this.enqueue(async () => {
      ...
      if (!copied || copied.content !== content) throw new Error("The recovery copy no longer matches this presentation. Save a new copy before closing.");
      const generation = this.generation;
      const snapshot = this.view.recoverySnapshot?.() ?? { content: await this.view.snapshot(), exact: !this.localEditing };
      if (this.disposed) return;
      if (!snapshot.exact || snapshot.content !== content || generation !== copied.generation || generation !== this.generation) {
        throw new Error("The presentation changed after the recovery copy was saved. Save a new copy before closing.");
      }
      ...
      if (!this.recoveryReady) throw new Error("Crash recovery is unavailable. Keep this presentation open and retry Save Copy.");
      ...
      if (!current.exact || current.content !== content || generation !== this.generation) {
        this.scheduleRecovery();
        throw new Error("The presentation changed while its recovery copy was being finalized. Save a new copy before closing.");
      }
      this.recoveryError = undefined;
    });
  }
