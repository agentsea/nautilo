...
import type { Block, BlockMarker, BlockStyle, Inline, InlineStyle } from '@nautilo/office-docs';
import { DEFAULT_BLOCK_STYLE, generateBlockId } from '@nautilo/office-docs';
...
function parseParagraph(
  ...
  // DrawingML's paragraph-level defRPr supplies the base character
  // properties for every run in this paragraph. Keep it as an inherited
  // style so explicit rPr attributes can override individual axes without
  // losing the remaining paragraph defaults.
  const inherited = ctx.defaultRunStyle ? { ...ctx.defaultRunStyle, ...(ctx.defaultRunStyle.fontSize !== undefined ? { fontSize: ctx.defaultRunStyle.fontSize * fontScale } : {}) } : undefined;
  const paragraphRunStyle = parseRunStyle(pPr ? child(pPr, 'defRPr') : undefined, fontScale, ctx, inherited);
  ...
  for (let i = 0; i < p.childNodes.length; i++) {
    ...
    if (el.localName === 'r') {
      const inline = parseRun(el, fontScale, ctx, undefined, paragraphRunStyle);
      inlines.push(inline);
      ...
      const style = parseRunStyle(rPr, fontScale, ctx, paragraphRunStyle);
      if ((!rPr || attrInt(rPr, 'sz') == null) && lastRunFontSize != null) {
        style.fontSize = lastRunFontSize;
      }
      inlines.push({ text: '\n', style });
      ...
      const t = child(el, 't');
      if (t) inlines.push(parseRun(el, fontScale, ctx, textOf(t), paragraphRunStyle));
    }
  }
  ...
  if (!inlines.some((i) => i.text !== '')) {
    ...
    const style = sized
      ? sized.style
      : parseRunStyle(child(p, 'endParaRPr'), fontScale, ctx, paragraphRunStyle);
    inlines.length = 0;
...
function parseRun(
  ...
  const style = parseRunStyle(child(r, 'rPr'), fontScale, ctx, inheritedStyle);
  const text = textOverride ?? textOf(child(r, 't') ?? r);

  // No script-specific fontFamily override here. The renderer
  // (`@nautilo/office-docs` `resolveFontFamily`) splices a Korean-capable
...
export function parseRunStyle(
  ...
  const style: InlineStyle = { ...inheritedStyle };

  // Apply placeholder-derived default first so an explicit `sz` below
  // can override it.
  if (style.fontSize == null && ctx.defaultFontSize != null) {
  ...
  if (rPr) {
    ...
    if (spc != null) {
      if (spc === 0) delete style.letterSpacing;
      else style.letterSpacing = spc / 100;
    }
