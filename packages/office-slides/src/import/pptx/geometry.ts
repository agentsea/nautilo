...
/** Resolve an OOXML transform over inherited placeholder geometry per axis. */
export function resolveInheritedXfrm(
  xfrm: Element | undefined,
  inherited: Frame | undefined,
  scale: EmuScale,
): Frame {
  if (!xfrm) return inherited ? { ...inherited } : parseXfrm(undefined, scale);
  const frame = parseXfrm(xfrm, scale);
  if (!inherited) return frame;
  if (!child(xfrm, 'off')) {
    frame.x = inherited.x;
    frame.y = inherited.y;
  }
