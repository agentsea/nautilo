...
export class ImportReport {
  ...
  omittedTemplateObjects = 0;
  skippedImages = 0;
  tableStyleOptionsDropped = 0;
  themeEffectsOmitted = 0;
  ...
  summary(): string {
    ...
    if (this.omittedTemplateObjects) parts.push(`${this.omittedTemplateObjects} master/layout object(s) omitted`);
    if (this.skippedImages) parts.push(`${this.skippedImages} image(s) skipped`);
    if (this.tableStyleOptionsDropped)
      parts.push(`${this.tableStyleOptionsDropped} table(s) lost style options (first/last row, first/last column, or banding)`);
    if (this.themeEffectsOmitted)
      parts.push(`${this.themeEffectsOmitted} shape theme effect(s) omitted`);
