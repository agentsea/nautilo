...
import type { Block, BlockStyle, InlineStyle } from '@nautilo/office-docs';
...
export async function parseSlide(opts: ParseSlideOptions): Promise<Slide | undefined> {
  ...
  const background = bgEl
    ? await parseSlideBackground(bgEl, layoutInfo?.clrMap ?? opts.clrMap, imageCtx)
  ...
  const ctx: SlideParseContext = {
    ...
    placeholderSizes,
    placeholderRunStyles: layoutInfo?.placeholderRunStyles,
    placeholderAlignments: layoutInfo?.placeholderAlignments ?? new Map(),
    placeholderFrames: layoutInfo?.placeholderFrames ?? new Map(),
    clrMap: layoutInfo?.clrMap ?? opts.clrMap,
    txStylesMarkers: layoutInfo?.txStylesMarkers ?? opts.txStylesMarkers,
    txStylesAlignments: layoutInfo?.txStylesAlignments ?? opts.txStylesAlignments,
  };
