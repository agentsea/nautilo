...
import { emuToStrokePx, parseXfrm, prstToShapeKind, resolveInheritedXfrm } from './geometry';
import {
  applyGroupTransform,
...
async function parseChild(
  ...
  switch (el.localName) {
    case 'sp': {
      ...
      const effectRef = style ? child(style, 'effectRef') : undefined;
      if (!effects && effectRef && (attrInt(effectRef, 'idx') ?? 0) > 0) {
        ctx.report.themeEffectsOmitted += 1;
      }
...
async function parseSp(sp: Element, ctx: SlideParseContext): Promise<SlideElement[]> {
  ...
  const frame = resolveInheritedXfrm(xfrm, layoutFrame, ctx.scale);
...
function buildTextBody(
  ...
  return {
    ...
    blocks: parseTextBody(txBody, {
      ...
      report: ctx.report,
      defaultFontSize,
      defaultRunStyle: layoutSizeKey ? ctx.placeholderRunStyles?.get(layoutSizeKey) : undefined,
      clrMap: ctx.clrMap,
      markerDefaults,
...
function buildShapeElement(
  ...
  const kind = prstToShapeKind(prst) ?? 'rect';
