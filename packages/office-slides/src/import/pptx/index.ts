// Modified by Nautilo: retain the default master referenced by built-in layouts.
...
import { parseLayout, parsePlaceholderFrames } from './layout';
import { attrInt, child, children, descendant, parseXml, NS } from './xml';
 ...
 * then.
 *
 * XML parsing uses the browser's native DOMParser when available and a
 * package-local parser in Node; callers do not need to install a global.
 */
export async function importPptx(
  ...
  // is the primary; each retained layout also carries its own master link
  // and parsing defaults for the slides that reference it.
  ...
  for (const masterTarget of masterTargets) {
    ...
    if (loaded.master) importedMasters.push(loaded.master);
  ...
  const themes: Theme[] = importedTheme
    ...
    : [...BUILT_IN_THEMES];
  const masters: Master[] = [...importedMasters, DEFAULT_MASTER];
  // Built-ins remain available for new slides; source layouts retain distinct
  // identities and already use the imported deck's coordinate space.
...
async function loadMasterAndLayouts(
  ...
  const masterRoot = descendant(masterDoc, 'sldMaster');
  const masterPlaceholderFrames = masterRoot
    ? parsePlaceholderFrames(masterRoot, scale)
    : new Map();
  const masterCommon = masterRoot ? child(masterRoot, 'cSld') : undefined;
  const masterTree = masterCommon ? child(masterCommon, 'spTree') : undefined;
  if (masterTree) {
    for (const node of Array.from(masterTree.childNodes)) {
      if (node.nodeType !== 1) continue;
      const element = node as Element;
      if (!['sp', 'pic', 'grpSp', 'graphicFrame', 'cxnSp'].includes(element.localName)) continue;
      const nv = child(element, 'nvSpPr');
      const nvPr = nv ? child(nv, 'nvPr') : undefined;
      if (!nvPr || !child(nvPr, 'ph')) report.omittedTemplateObjects += 1;
    }
  }
  ...
  for (const rel of rels.values()) {
    ...
    const imported = await parseLayout(layoutXml, layoutPath, report, {
      ...
      clrMap, masterId: master.id, txStylesMarkers, txStylesAlignments,
      masterPlaceholderFrames,
    });
    ...
    layoutMap.set(layoutPath, {
      ...
      clrMap, txStylesMarkers, txStylesAlignments,
      placeholderSizes: imported.placeholderSizes,
      placeholderRunStyles: imported.placeholderRunStyles,
