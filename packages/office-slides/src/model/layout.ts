...
import { DEFAULT_BLOCK_STYLE, type Block } from '@nautilo/office-docs';
...
export function slotRefsForLayout(layout: Layout): PlaceholderRef[] {
  ...
  return layout.placeholders.map((p) => {
    ...
    const index = p.placeholder.index ?? counts.get(type) ?? 0;
  ...
  for (let si = 0; si < slots.length; si++) {
    ...
    if (context && cloned.type === 'text') {
      ...
      if (placeholderStyle) {
        ...
        cloned.data = {
          ...cloned.data,
          blocks: seedPlaceholderBlocks(placeholderStyle, context.theme, cloned.data.blocks),
        };
      }
    }
    const fresh = {
