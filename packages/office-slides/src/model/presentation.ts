...
export type Slide = {
  ...
  layoutId: string;
  /**
   * Optional per-slide theme used by independently imported template slides.
   * Absent preserves the historical deck-wide `meta.themeId` behavior.
   */
  themeId?: string;
...
export type PlaceholderSpec = ElementInit & {
  placeholder: { type: PlaceholderType; index?: number };
};
 ...
 * color, de-duping case-insensitively without discarding older colors.
...
export function pushRecent(list: readonly string[], hex: string): string[] {
  ...
  return [norm, ...list.filter((c) => c.toLowerCase() !== norm)];
}
...
/** Resolve the theme that paints one slide, preserving legacy deck-wide themes. */
export function getThemeForSlide(doc: SlidesDocument, slide: Slide): Theme {
  const id = slide.themeId ?? doc.meta.themeId;
  const theme = doc.themes.find((candidate) => candidate.id === id);
  if (!theme) {
    throw new Error(
      `[slides] theme '${id}' for slide '${slide.id}' not found in document; ` +
        `themes: ${doc.themes.map((candidate) => candidate.id).join(', ') || '(none)'}`,
    );
  }
  return theme;
}
