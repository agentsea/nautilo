...
export type PlaceholderType =
  "title" | "subtitle" | "body" | "caption" | "big-number";
...
/** @internal @deprecated Use SlidesTableRow. */
export type TableRow = SlidesTableRow;

/** @internal @deprecated Use SlidesCellStyle. */
export type CellStyle = SlidesCellStyle;

/** @internal @deprecated Use SlidesTableCell. */
export type TableCell = SlidesTableCell;
...
export type ChartValueAxis = {
  /** Explicit OOXML value-axis scale bounds. Absent means automatic. */
  min?: number;
  max?: number;
  /** Where the category axis crosses the value axis. */
...
export type ChartElement = ElementBase & {
  ...
  data: {
    kind: ChartKind;
    /** Cartesian grouping; ignored for pie. Absent ⇒ family default. */
    grouping?: ChartGrouping;
    title?: string;
    /** Shared category-axis labels (x for column/line/area, y for bar). */
    categories: string[];
    /**
     * Original OOXML `<c:pt idx>` coordinates, parallel to `categories` and
     * every `series.values` array. Present only when imported points are
     * sparse; absent means dense coordinates `0..categories.length - 1`.
     */
