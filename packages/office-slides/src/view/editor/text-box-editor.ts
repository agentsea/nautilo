...
/**
 * Slides-side wrapper around `initializeTextBox` from @nautilo/office-docs.
 *
 ...
 */
const HOVER_TEXT_REGION_INSET_PX = 6;
...
export interface MountSlidesTextBoxOptions {
  ...
  /** Fired when the non-committed recovery snapshot changes. */
  onDraftChange?: () => void;
  /** Reports rich content that slide text boxes cannot render. */
...
export interface SlidesTextBoxEditor {
   ...
   */
  commit(): void;
  /** Read live blocks (including IME preedit) and their fitted height without committing. */
...
export function mountSlidesTextBox(opts: MountSlidesTextBoxOptions): SlidesTextBoxEditor {
  const { overlay, frame, scale, blocks, onCommit, onDraftChange, onCancel, onLinkRequest, onContentHeightChange, colorResolver, autofit, verticalAnchor, growMode, initialText, overflowBounds } = opts;
  ...
  const textBoxOptions: Parameters<typeof initializeTextBox>[0] = {
    container,
    canvas,
    blocks,
    ...
    onPasteRejected: opts.onPasteRejected,
    onLinkRequest,
  ...
  const api: TextBoxEditorAPI = initializeTextBox(textBoxOptions);
