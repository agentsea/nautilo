    ...
    const buildChunk = (): void => {
      ...
      if (cursor < slides.length) {
        ...
        if (typeof requestAnimationFrame !== 'undefined') {
          ...
          // No rAF available — finish synchronously without one stack
          // frame per chunk. Headless and SSR-style hosts can contain an
          // arbitrarily large deck, so recursive chunk advancement would
          // eventually exhaust the JavaScript call stack.
          while (cursor < slides.length) {
            if (disposed || activeRenderToken !== myToken) return;
            const fallbackEnd = Math.min(
              cursor + RENDER_CHUNK_SIZE,
              slides.length,
            );
            for (; cursor < fallbackEnd; cursor++) buildItem(slides[cursor]);
            if (scrollParent && scrollParent.scrollTop < savedScrollTop) {
              scrollParent.scrollTop = savedScrollTop;
            }
          }
