...
export interface NotesPanelHandle {
  ...
  dispose(): void;
  /**
   * Read the textarea's current value without blurring or writing the store.
   * This remains authoritative while focused even if a remote store update is
   * deliberately withheld from the textarea to protect the local caret.
   */
  getDraftSnapshot(): { slideId: string; blocks: Block[] } | null;
}
...
export function mountNotesPanel(
  ...
  return {
    getDraftSnapshot: () => {
      ...
      const slide = store.read().slides.find((candidate) => candidate.id === slideId);
      // Preserve canonical block ids/styles when the textarea already mirrors
      // the store. Only synthesize plain-text blocks for the focused divergence
      // that the store-change guard deliberately leaves view-local.
      const blocks = slide && blocksToText(slide.notes) === ta.value
        ? slide.notes
        : textToBlocks(ta.value);
      return { slideId, blocks };
    },
    dispose: () => {
      offSelection();
      offSlide();
