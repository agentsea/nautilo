...
describe('dragThresholdFor', () => {
  it('keeps the precise threshold for a mouse', () => {
    expect(dragThresholdFor('mouse')).toBe(SLOW_DOUBLE_CLICK_MAX_DISTANCE_PX);
  });
  ...
  it('keeps the precise threshold for a stylus', () => {
    ...
    expect(dragThresholdFor('pen')).toBe(SLOW_DOUBLE_CLICK_MAX_DISTANCE_PX);
  });

  it('widens the threshold for a fingertip', () => {
    expect(dragThresholdFor('touch')).toBe(TOUCH_DRAG_THRESHOLD_PX);
    expect(TOUCH_DRAG_THRESHOLD_PX).toBeGreaterThan(SLOW_DOUBLE_CLICK_MAX_DISTANCE_PX);
  });
  ...
  it('falls back to the precise threshold when the device is unknown', () => {
    ...
    expect(dragThresholdFor(undefined)).toBe(SLOW_DOUBLE_CLICK_MAX_DISTANCE_PX);
  });
});
