...
import { isBlocksEmpty } from '../../model/element';
import {
  cropToFull,
  windowToCrop,
  applyCropHandle,
...
import { clearMeasureCache } from '@nautilo/office-docs';
...
export interface SlidesEditorOptions extends SlideRendererOptions {
  ...
  onFontsLoaded?: () => void;
  /**
   * Fired when view-local editor state changes the recoverable document
   * returned by `getDraftSnapshot()`. This includes rich text/IME preedit and
   * live image-crop geometry; it never commits those changes to the store.
   */
  onDraftChange?: () => void;
  /**
   ...
   * package (`@nautilo/office-board`) reuses this editor unmodified via
...
export interface SlidesEditor {
  ...
  /**
   * Return a detached presentation snapshot with any active text or crop edit
   * overlaid. Reading it does not blur, commit, or mutate the SlidesStore.
   */
...
class SlidesEditorImpl implements SlidesEditor {
  ...
  /** Routing metadata needed to overlay a live text draft onto a store clone. */
  private editingDraftTarget:
    | {
        slideId: string;
        elementId: string;
        ...
        enterFrameH: number;
        growApplicable: boolean;
        shapeHadTextBody: boolean;
      }
    | null = null;
  ...
  constructor(options: SlidesEditorOptions) {
    this.options = { ...options, showPlaceholderHints: !options.readOnly };
    const ctx = options.canvas.getContext('2d');
    if (!ctx) throw new Error('SlidesEditor: canvas has no 2D context');
    this.hitCtx = ctx;
    this.renderer = new SlideRenderer(ctx, this.options);
    if (
  ...
  /** Paint chrome without disconnecting the active text input or peer layer.
   * Removing and re-appending a focused textarea loses browser focus and can
   * restart type-to-edit at the beginning of the note on the next keystroke.
   */
  private paintOverlay(
    ...
    const chrome = overlay.ownerDocument.createElement('div');
    renderOverlay(chrome, selectedElements, options);
    const textBox = this.editingTextBox?.container;
    for (const child of Array.from(overlay.childNodes)) {
      if (child !== textBox && child !== this.peerCursorLayer) child.remove();
    }
    const fragment = overlay.ownerDocument.createDocumentFragment();
    while (chrome.firstChild) fragment.appendChild(chrome.firstChild);
    overlay.insertBefore(fragment, overlay.firstChild);
  ...
  insertTable(
    ...
    this.options.store.batch(() => {
      id = this.options.store.addElement(slideId, {
        ...
        data: {
          columnWidths: new Array<number>(cols).fill(colWidth),
          rows: rowsData,
        },
      });
    });
  ...
  getDraftSnapshot(): SlidesDocument {
    ...
    if (crop) {
      const slide = snapshot.slides.find((candidate) => candidate.id === crop.slideId);
      const element = slide?.elements.find((candidate) => candidate.id === crop.elementId);
      if (element?.type === 'image') {
        const frame = windowToFrame(crop.window, crop.center, crop.cos, crop.sin);
        element.frame = {
          ...element.frame,
          x: frame.x,
          y: frame.y,
        ...
        element.data = {
          ...element.data,
          crop: normalizeCrop(windowToCrop(crop.full, crop.window)),
        };
      }
    }

    const target = this.editingDraftTarget;
    ...
    const element = slide ? findElement(slide.elements, target.elementId) : undefined;
    if (!element) return snapshot;
    const draft = active.getDraftSnapshot();

    if (target.kind === 'text' && element.type === 'text') {
      element.data = { ...element.data, blocks: draft.blocks };
      if (target.growApplicable) {
        const height = Math.max(MIN_TEXT_BOX_H, draft.contentHeight);
        if (height !== target.enterFrameH) element.frame = { ...element.frame, h: height };
      }
    } else if (target.kind === 'shape' && element.type === 'shape') {
      // Match `withShapeText`: entering and leaving a previously textless
      // shape without content must not materialise an empty body.
      if (target.shapeHadTextBody || !isBlocksEmpty(draft.blocks)) {
        element.data = {
          ...element.data,
          text: { ...element.data.text, blocks: draft.blocks },
        };
      }
    } else if (target.kind === 'cell' && element.type === 'table' && target.cell) {
      const cell = element.data.rows[target.cell.row]?.cells[target.cell.col];
      if (cell && cell.gridSpan !== 0 && cell.rowSpan !== 0) {
        cell.body = { ...cell.body, blocks: draft.blocks };
      }
    }
  ...
  private enterEditMode(
    ...
    this.editingDraftTarget = {
      slideId,
      elementId,
      kind: target.kind,
      cell: target.cell,
      enterFrameH,
      growApplicable: this.editingGrowApplicable,
      shapeHadTextBody: localElement.type === 'shape' && localElement.data.text !== undefined,
    };
    ...
    const tb = this.mountTextBox({
      ...
      onDraftChange: this.options.onDraftChange,
      onPasteRejected: this.options.onToast,
      ...
      colorResolver: makeColorResolver(getThemeForSlide(doc, slide)),
      ...
      overflowBounds:
        target.kind === 'text'
          ...
          : this.options.viewport
            ? {
                // An infinite board includes negative world coordinates.
                // Paint into its visible viewport, not the slide's 0,0 rect.
                left: target.editFrame.x + this.options.viewport.panX / this.scale(),
                top: target.editFrame.y + this.options.viewport.panY / this.scale(),
                width: this.options.hostWidth / this.scale(),
                height: this.options.hostHeight / this.scale(),
              }
            : {
  ...
  private startTableEdgeResize(
    ...
    const clampPosition = (proposed: number): number => {
      if (kind === 'col') {
        return clampInsidePair(
          proposed,
          sumPrefix(originalWidths, idx - 1),
          sumPrefix(originalWidths, idx + 1),
          edge.position,
        );
      }
      return clampInsidePair(
        proposed,
        sumPrefix(originalHeights, idx - 1),
        sumPrefix(originalHeights, idx + 1),
        edge.position,
      );
    };
    ...
    const finish = (commit: boolean): void => {
      ...
      if (commit && pending !== null && pending.position !== edge.position) {
        if (pending.kind === 'col') {
  ...
  private startCellRangeDrag(
    ...
    const onMove = (ev: PointerEvent): void => {
      ...
      const clampedX = projectIntoBoundaryIntervals(localX, layout.colX);
      const clampedY = projectIntoBoundaryIntervals(localY, layout.rowY);
