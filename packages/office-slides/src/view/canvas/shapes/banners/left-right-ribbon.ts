...
export const LEFT_RIGHT_RIBBON_HANDLES: readonly AdjustmentHandle[] = [
  ...
  // tracks a1 and the position↔apply round-trip is stable.
  {
    ...
    apply: (size, start, pointer) => {
      ...
      // OOXML: maxAdj1 = 100000 - a3.  Use the current fold adjustment,
      // rather than the static outer metadata domain, so a handle drag cannot
      // commit a value that the preset renderer immediately pins away.
      const { maxAdj1 } = geometry(size, start);
      return [
        Math.max(0, Math.min(maxAdj1, raw)),
        start[1] ?? 50000,
        start[2] ?? 16667,
      ];
    },
  ...
  // corner (x1, ly1). Drag right → longer arrowhead.
  {
    ...
    apply: (size, start, pointer) => {
      ...
      // OOXML: maxAdj2 = 100000 * (wd2 - wd32) / ss.  This varies with
      // aspect ratio and can be either below or above the static 100000
      // metadata value.
      const { maxAdj2 } = geometry(size, start);
      return [
        start[0] ?? 50000,
        Math.max(0, Math.min(maxAdj2, raw)),
        start[2] ?? 16667,
      ];
    },
  },
  // Fold height — diamond on the center fold top at (x3, ry2).
  {
    ...
    apply: ({ h }, start, pointer) => {
      ...
      const nextA3 = Math.max(spec.min, Math.min(spec.max, raw));
      // Increasing the fold lowers adj1's dependent maximum.  Keep the
      // complete adjustment tuple valid at commit time.
      const nextA1 = Math.max(
        0,
      Math.min(100000 - nextA3, start[0]?? 50000),
      );
    return [
        nextA1,
        start[1] ?? 50000,
