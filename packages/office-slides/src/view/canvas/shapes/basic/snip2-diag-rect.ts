...
export const buildSnip2DiagRect: PathBuilder = ({ w, h }, adjustments) => {
  const a1 = Math.max(0, Math.min(50000, adj(adjustments, 0, SNIP2_DIAG_RECT_ADJUSTMENTS[0].defaultValue)));
  const a2 = Math.max(0, Math.min(50000, adj(adjustments, 1, SNIP2_DIAG_RECT_ADJUSTMENTS[1].defaultValue)));
