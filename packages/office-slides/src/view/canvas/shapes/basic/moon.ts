...
export const buildMoon: PathBuilder = ({ w, h }, adjustments) => {
  // The preset geometry evaluates `a = pin 0 adj 87500`. Preserve the raw
  // imported adjustment in the model, while evaluating its rendered guide
  // with the authoritative preset bounds.
  const raw = adj(adjustments, 0, MOON_ADJUSTMENTS[0].defaultValue);
  const a = Math.max(0, Math.min(87500, raw));
  const cx = w / 2;
  const cy = h / 2;
  const rx = w / 2;
  const ry = h / 2;
