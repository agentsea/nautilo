 ...
 *
 * The OOXML constraint `maxAdj1 = q2*h/(2*ss)` (where `q2` depends
 * on notch position) is enforced at runtime by clamping the radius
...
export function braceCornerRadius(
  ...
  // OOXML `maxAdj1 = q2*h/(2*ss)` where q2 is the smaller distance
  // from the notch to an end. This caps the rendered radius at
  // h*q2/200000; at the default notch the cap is h/4.
  const pinnedNotch = Math.max(0, Math.min(100000, notchA));
  const q2 = Math.min(pinnedNotch, 100000 - pinnedNotch);
  const maxAdj = ss > 0 ? (q2 / 2) * h / ss : 0;
...
export const buildLeftBrace: PathBuilder = (size, adjustments) => {
  ...
  const a1 = Math.max(0, Math.min(100000, adj(adjustments, 1, DEF_BRACE_NOTCH)));
...
export const LEFT_BRACE_HANDLES: readonly AdjustmentHandle[] = [
  {
    position: (size, adjustments) => {
      const a1 = Math.max(0, Math.min(100000, adjustments[1] ?? DEF_BRACE_NOTCH));
      return {
