...
export const buildDonut: PathBuilder = ({ w, h }, adjustments) => {
  // The preset geometry evaluates `a = pin 0 adj 50000`. Imported avLst
  // values remain stored verbatim for round-trip, but painting must evaluate
  // the same guide rather than letting an out-of-range value invert the hole.
  const a = Math.max(0, Math.min(50000, adj(adjustments, 0, 25000) ));
