...
export const buildArc: PathBuilder = (size, adjustments) => {
  const start = Math.max(
    ARC_ADJUSTMENTS[0].min,
    Math.min(
      ARC_ADJUSTMENTS[0].max,
      adj(adjustments, 0, ARC_ADJUSTMENTS[0].defaultValue),
    ),
  );
  const end = Math.max(
    ARC_ADJUSTMENTS[1].min,
    Math.min(
      ARC_ADJUSTMENTS[1].max,
      adj(adjustments, 1, ARC_ADJUSTMENTS[1].defaultValue),
    ),
  );
