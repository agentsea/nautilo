...
export const buildRound2SameRect: PathBuilder = ({ w, h }, adjustments) => {
  const a1 = Math.max(0, Math.min(50000, adj(adjustments, 0, ROUND2_SAME_RECT_ADJUSTMENTS[0].defaultValue)));
  const a2 = Math.max(0, Math.min(50000, adj(adjustments, 1, ROUND2_SAME_RECT_ADJUSTMENTS[1].defaultValue)));
