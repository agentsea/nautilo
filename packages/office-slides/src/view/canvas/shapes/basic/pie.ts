...
export const buildPie: PathBuilder = (size, adjustments) => {
  const start = Math.max(
    PIE_ADJUSTMENTS[0].min,
    Math.min(
      PIE_ADJUSTMENTS[0].max,
      adj(adjustments, 0, PIE_ADJUSTMENTS[0].defaultValue),
    ),
  );
  const end = Math.max(
    PIE_ADJUSTMENTS[1].min,
    Math.min(
      PIE_ADJUSTMENTS[1].max,
      adj(adjustments, 1, PIE_ADJUSTMENTS[1].defaultValue),
    ),
  );
