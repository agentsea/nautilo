...
const chevronAdjustmentMax = (w: number, h: number) => {
...
export const CHEVRON_HANDLES: readonly AdjustmentHandle[] = [
  {
    ...
    position: ({ w, h }, adjustments) => {
      // The preset permits 100000*w/ss so tall and wide frames both reach
      // the edge without collapsing early at a square-frame maximum.
      const a = Math.max(0, Math.min(chevronAdjustmentMax(w, h), adjustments[0] ?? 50000));
    ...
    apply: ({ w, h }, _start, pointer) => {
      ...
      return [Math.max(0, Math.min(chevronAdjustmentMax(w, h), value))];
    },
  },
];
...
export const buildChevron: PathBuilder = ({ w, h }, adjustments) => {
  ...
  const ss = Math.min(w, h);
  // Pin to the preset's frame-dependent maximum before deriving x1.
  const a = Math.max(0, Math.min(chevronAdjustmentMax(w, h), adj(adjustments, 0, 50000)));
