...
export function curvedArrowHandles(
  ...
  return pos.map((p, index) =>
    presetNumericHandle({
      ...
      // Effective paint pins: adj1 uses pinned a2 (including curvedDown,
      // whose XML handle metadata names raw adj2), then maxAdj2/maxAdj3.
      bounds: { min: '0', max: ['a2', 'maxAdj2', 'maxAdj3'][index] },
    }),
  );
}
