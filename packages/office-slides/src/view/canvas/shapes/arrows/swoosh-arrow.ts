...
const swooshHeadSizeMax = (w: number, h: number) => {
  const ss = Math.min(w, h);
  return ss > 0 ? (70000 * w) / ss : 0;
};

const swooshHeadHandle = presetNumericHandle({
...
export const SWOOSH_ARROW_HANDLES: readonly AdjustmentHandle[] = [
  ...
  {
    position: swooshHeadHandle.position,
    apply: ({ w, h }, start, pointer) => {
      const ss = Math.min(w, h);
      const raw = ss > 0 ? Math.round(((w - pointer.x) / ss) * 100000) : 0;
      const next = [...start];
      next[1] = Math.max(0, Math.min(swooshHeadSizeMax(w, h), raw));
      return next;
    },
  },
];
