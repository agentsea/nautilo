...
export const buildBentUpArrow: PathBuilder = ({ w, h }, adjustments) => {
  const pin = (value: number) => Math.max(0, Math.min(50000, value));
  const a1 = pin(adj(adjustments, 0, BENT_UP_ARROW_ADJUSTMENTS[0].defaultValue));
  const a2 = pin(adj(adjustments, 1, BENT_UP_ARROW_ADJUSTMENTS[1].defaultValue));
  const a3 = pin(adj(adjustments, 2, BENT_UP_ARROW_ADJUSTMENTS[2].defaultValue));
