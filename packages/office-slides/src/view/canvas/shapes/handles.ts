...
export function angularHandle(opts: {
  ...
  return {
    ...
    apply: (frame, start, pointer) => {
      ...
      // preserves both exact +/-180° ties, matching the former strict
      // loops and existing arc/pie behavior. Outside that closed interval,
      // the derived turn count works for any finite spec range.
      const delta = degrees - startDegrees;
      const unwrappedDegrees =
        delta > 180
          ? degrees - Math.ceil((delta - 180) / 360) * 360
          : delta < -180
            ? degrees + Math.ceil((-180 - delta) / 360) * 360
            : degrees;
      const ooxml = Math.round(unwrappedDegrees * 60000);
