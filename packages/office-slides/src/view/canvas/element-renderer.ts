...
export function drawElement(
  ...
  if (!hasAnim) {
    drawElementBody(
      ...
      elementsLookup, parentFlip, parentTransform, showPlaceholderHints,
    );
    return;
  }
  ctx.save();
    ...
    if (element.type === 'group') {
      ...
      for (const child of element.data.children) {
        drawElement(
          ...
          elementsLookup, totalFlip, childTransform, undefined, showPlaceholderHints,
        );
      }
    } else {
      ...
      switch (element.type) {
        ...
        case 'text': {
          ...
          let placeholderHint:
            ...
            | undefined;
          if (showPlaceholderHints && element.placeholderRef) {
            const slide = doc.slides.find((candidate) =>
              candidate.elements.some((candidateElement) => candidateElement.id === element.id),
            );
            const layout = slide
              ? doc.layouts.find((candidate) => candidate.id === slide.layoutId)
              : undefined;
            const master =
              doc.masters.find((m) => m.id === layout?.masterId)
