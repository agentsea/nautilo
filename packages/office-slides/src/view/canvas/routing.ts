...
import type { Point } from '@nautilo/office-core/geometry';
...
function resolveCurveBend(bend: number | undefined): number {
  if (bend === undefined) return CURVE_BEND_DEFAULT;
  if (!Number.isFinite(bend)) {
    throw new Error('Curved connector bend must be finite');
  }
  return bend;
}
 ...
 * PowerPoint's `curvedConnector*` look). Signed finite values are preserved;
 * values whose derived control points overflow are refused.
 */
export function routeCurved(
  ...
  const dx = b.x - a.x;
  const dy = b.y - a.y;
  const k = (Math.hypot(dx, dy) / 3) * resolveCurveBend(bend);
  const path = {
    p0: { ...a },
  ...
  if (!coordinates.every(Number.isFinite)) {
    throw new Error('Curved connector bend produces non-finite geometry');
  }
  return path;
}
