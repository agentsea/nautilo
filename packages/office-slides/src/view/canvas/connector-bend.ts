 ...
 *
 * Curved: solve for the signed bend factor that places the bezier's
...
export function bendFromCursor(
  ...
  if (dist < 1e-6) {
    return connector.curveBend ?? CURVE_BEND_DEFAULT;
  }
  ...
  if (Math.abs(sinSum) < 1e-3) {
    // The midpoint is independent of bend when the endpoint contributions
    // cancel. Preserve the current value rather than inventing a solution.
    return connector.curveBend ?? CURVE_BEND_DEFAULT;
  }
  ...
  return Number.isFinite(bend) ? bend : null;
}
