...
import type { Block } from '@nautilo/office-docs';
import type {
  ArrowheadStyle,
  ConnectorRouting,
  Endpoint,
...
import { deckFontScale, deckSlideHeight, getThemeForSlide, pushRecent } from '../model/presentation';
import { scaleElementHeight } from '../model/slide-size';
import { scaleBlocks } from '../model/autofit';
...
function assertFiniteConnectorFrame(frame: Frame): void {
  const values = [frame.x, frame.y, frame.w, frame.h, frame.rotation];
  if (!values.every(Number.isFinite) || frame.w < 0 || frame.h < 0) {
    throw new Error('Connector bend produces a non-finite frame');
  }
}
...
function collectStringIds(value: unknown, out: Set<string>): void {
  ...
  if (Array.isArray(value)) {
    for (const item of value) collectStringIds(item, out);
    return;
  }
  const record = value as Record<string, unknown>;
  if (typeof record.id === 'string') out.add(record.id);
  for (const child of Object.values(record)) collectStringIds(child, out);
}
...
function refreshBlocks(blocks: Block[], fontRatio: number, used: Set<string>): Block[] {
  const refreshed = scaleBlocks(clone(blocks), fontRatio);
  const work = [...refreshed];
  while (work.length > 0) {
    const block = work.pop()!;
    block.id = freshId(used);
    if (!block.tableData) continue;
    for (const row of block.tableData.rows) {
      for (const cell of row.cells) {
        const scaled = scaleBlocks(cell.blocks, fontRatio);
        cell.blocks = scaled;
        work.push(...scaled);
      }
    }
...
function refreshElementIdsAndText(
  elements: Element[],
  fontRatio: number,
  used: Set<string>,
): Map<string, string> {
  ...
  const visit = (element: Element): void => {
    ...
    element.id = freshId(used);
    idMap.set(oldId, element.id);
    all.push(element);
    if (element.type === 'text') {
      element.data.blocks = refreshBlocks(element.data.blocks, fontRatio, used);
    } else if (element.type === 'shape' && element.data.text) {
      element.data.text.blocks = refreshBlocks(element.data.text.blocks, fontRatio, used);
    } else if (element.type === 'table') {
      for (const row of element.data.rows) {
        for (const cell of row.cells) {
          cell.body.blocks = refreshBlocks(cell.body.blocks, fontRatio, used);
        }
      }
  ...
  for (const element of all) {
    ...
    for (const side of ['start', 'end'] as const) {
      ...
      const mapped = idMap.get(endpoint.elementId);
      if (mapped) element[side] = { ...endpoint, elementId: mapped };
    }
  }
  return idMap;
}
...
export class MemSlidesStore implements SlidesStore {
  ...
  addSlide(layoutId: string, atIndex?: number): string {
    ...
    const master =
      this.doc.masters.find((m) => m.id === this.resolveLayout(layoutId).masterId)
      ...
      elements: layout.placeholders.map((p, i) => {
        ...
        if (cloned.type === 'text' && master && theme) {
          ...
          if (placeholderStyle) {
            ...
            cloned.data = {
              ...cloned.data,
              blocks: seedPlaceholderBlocks(placeholderStyle, theme, cloned.data.blocks),
            };
          }
        }
        return {
  ...
  /**
   * Import a one-slide template document as independent resources and content.
   * The caller owns the surrounding batch so the complete import is one undo.
   */
  importSlideTemplate(templateDocument: SlidesDocument, afterSlideId?: string): string {
    this.requireBatch();
    const template = migrateDocument(clone(templateDocument));
    if (template.slides.length !== 1) {
      throw new Error('A slide template must contain exactly one slide');
    }
    const sourceSlide = template.slides[0];
    const sourceLayout = template.layouts.find((layout) => layout.id === sourceSlide.layoutId);
    if (!sourceLayout) throw new Error(`Slide template layout not found: ${sourceSlide.layoutId}`);
    const sourceMaster = template.masters.find((master) => master.id === sourceLayout.masterId);
    if (!sourceMaster) throw new Error(`Slide template master not found: ${sourceLayout.masterId}`);
    const sourceTheme = getThemeForSlide(template, sourceSlide);

    const used = new Set<string>();
    collectStringIds(this.doc, used);
    collectStringIds(template, used);
    const theme = clone(sourceTheme);
    theme.id = freshId(used);
    const master = clone(sourceMaster);
    master.id = freshId(used);
    master.themeId = theme.id;
    const layout: Layout = clone(sourceLayout);
    layout.id = freshId(used);
    layout.masterId = master.id;
    const slide: Slide = clone(sourceSlide);
    slide.id = freshId(used);
    slide.layoutId = layout.id;
    slide.themeId = theme.id;

    const heightRatio = deckSlideHeight(this.doc.meta) / deckSlideHeight(template.meta);
    const fontRatio = deckFontScale(template.meta) / deckFontScale(this.doc.meta);
    for (const style of Object.values(master.placeholderStyles)) {
      style.fontSize *= fontRatio;
    }

    layout.placeholders = layout.placeholders.map((placeholder) => {
      const withId = { ...placeholder, id: freshId(used) } as Element;
      refreshElementIdsAndText([withId], fontRatio, used);
      scaleElementHeight(withId, heightRatio);
      const { id: _discarded, ...withoutId } = withId;
      return withoutId as typeof placeholder;
    });
    refreshElementIdsAndText(layout.staticElements, fontRatio, used);
    for (const element of layout.staticElements) scaleElementHeight(element, heightRatio);

    const elementIds = refreshElementIdsAndText(slide.elements, fontRatio, used);
    for (const element of slide.elements) scaleElementHeight(element, heightRatio);
    const lookup = buildElementWorldLookup(slide.elements);
    for (const element of slide.elements) {
      if (element.type === 'connector') element.frame = computeConnectorFrame(element, lookup);
    }
    slide.notes = refreshBlocks(slide.notes, 1, used);
    if (slide.animations) {
      slide.animations = slide.animations.map((animation) => ({
        ...animation,
        id: freshId(used),
        elementId: elementIds.get(animation.elementId) ?? animation.elementId,
      }));
    }

    const insertAt = afterSlideId === undefined
      ? this.doc.slides.length
      : this.requireSlideIndex(afterSlideId) + 1;
    this.doc.themes.push(theme);
    this.doc.masters.push(master);
  ...
  applyTheme(themeId: string): void {
    ...
    for (const master of this.doc.masters) master.themeId = themeId;
    for (const slide of this.doc.slides) delete slide.themeId;
  }
  ...
  setUnit(unit: 'in' | 'cm'): void {
    this.requireBatch();
    if (unit !== 'in' && unit !== 'cm') {
      throw new Error(`[slides] invalid unit '${String(unit)}'`);
    }
  ...
  setSlideHeight(height: number): void {
    ...
    if (!Number.isFinite(height) || height <= 0) {
      throw new Error(`[slides] invalid slide height '${String(height)}'`);
    }
  ...
  updateConnectorElbowBend(
    ...
    if (bend !== undefined && !Number.isFinite(bend)) {
      throw new Error('Connector elbow bend must be finite');
    }
    const candidate = clone(e);
    if (bend === undefined) delete candidate.elbowBend;
    else candidate.elbowBend = bend;
    const frame = computeConnectorFrame(candidate, this.elementsLookup(slideId));
    assertFiniteConnectorFrame(frame);
    if (bend === undefined) delete e.elbowBend;
    else e.elbowBend = bend;
    e.frame = frame;
  }
  ...
  updateConnectorCurveBend(
    ...
    if (bend !== undefined && !Number.isFinite(bend)) {
      throw new Error('Connector curve bend must be finite');
    }
    const candidate = clone(e);
    if (bend === undefined) delete candidate.curveBend;
    else candidate.curveBend = bend;
    const frame = computeConnectorFrame(candidate, this.elementsLookup(slideId));
    assertFiniteConnectorFrame(frame);
    if (bend === undefined) delete e.curveBend;
    else e.curveBend = bend;
    e.frame = frame;
  }
  ...
  ungroup(slideId: string, groupId: string): string[] {
    ...
    const groupTx = groupToTransform(group);
    const bakedChildren: Element[] = group.data.children.map(child => {
      const cloned = clone(child);
      cloned.frame = applyGroupTransform(child.frame, group);
      if (cloned.type === 'connector') {
        for (const side of ['start', 'end'] as const) {
          const ep = cloned[side];
          if (ep.kind === 'free') {
