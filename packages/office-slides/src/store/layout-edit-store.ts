...
export class LayoutEditStore implements SlidesStore {
  ...
  read(): SlidesDocument {
    ...
    const master = this.resolveMaster(doc, layout.masterId);
  ...
  private resolveMaster(doc: SlidesDocument, layoutMasterId?: string): Master {
    return (
      doc.masters.find((m) => m.id === layoutMasterId)
