...
export interface SlidesStore {
  ...
  /** Apply a theme deck-wide, clearing per-slide template overrides. Theme must already be in `themes[]`. */
   ...
   * most-recent-first and de-duped without discarding older colors.
   ...
   * default 1 (auto-routed control-point distance). Implementations preserve
   * exact signed finite values and refuse bends whose derived geometry is
   * not finite.
   */
