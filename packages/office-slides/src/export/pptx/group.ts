 ...
 */
import type { ChartElement, Element, Frame, GroupElement, ImageElement } from '../../model/element.js';
...
import { chartFrameToXml } from './chart.js';
...
export interface ElementXmlCtx {
   ...
   */
  resolveHyperlinkRId(this: void, href: string): string;
  resolveChartRId?(el: ChartElement): string;
}
...
export function elementToXml(el: Element, ctx: ElementXmlCtx): string {
  switch (el.type) {
    ...
    case 'image': {
      // A dropped image is intentionally omitted after the caller's media
      // resolution policy decides its bytes cannot be included.
    ...
    case 'chart':
      if (!ctx.resolveChartRId) throw new Error(`Cannot export chart "${el.id}": chart relationship resolver is unavailable.`);
      return chartFrameToXml(el, ctx.resolveChartRId(el));
  }
}
...
export function groupToXml(el: GroupElement, ctx: ElementXmlCtx): string {
  ...
  // Every child serialized away (dropped images). A
