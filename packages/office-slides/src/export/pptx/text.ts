...
import type { Block, BlockMarker, Inline } from '@nautilo/office-docs';
import type { StoredColor } from '@nautilo/office-docs';
import { toRgbHexColor } from '@nautilo/office-docs';
...
/** OOXML ST_TextIndentLevelType permits only levels 0 through 8.
 * Keep deeper model content intact and refuse lossy conversion explicitly.
 * Authority: dotnet/Open-XML-SDK data/schemas/
 * schemas_openxmlformats_org_drawingml_2006_main.json, TextParagraphPropertiesType.Level.
 */
const PPTX_MAX_LIST_LEVEL = 8;
function listLevelAttr(listLevel: number | undefined): string {
  const n = Math.trunc(Number(listLevel));
  if (!Number.isFinite(n) || n <= 0) return '';
  if (n > PPTX_MAX_LIST_LEVEL) {
    throw new RangeError('PPTX supports nine list levels. This document contains deeper lists; keep the original document or reduce their indentation before exporting.');
  }
  return ` lvl="${n}"`;
}
...
function storedColorToThemeColor(
  ...
  if ((Object.keys(ROLE_TO_SCHEME)).includes(c.role)) {
