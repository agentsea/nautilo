...
import { attachConnectors } from './connector-attachments.js';
 ...
 *
 * Returns the renumbered spTree body and the raw model ID→numeric ID map.
 * XML escaping belongs only to serialization, never animation/attachment lookup.
 */
function renumberCNvPrIds(body: string, elements: Slide['elements']): { renumbered: string; spidMap: Map<string, number> } {
  const originalIds = new Map(flattenElements(elements).map(el => [escapeXmlAttr(el.id), el.id]));
  ...
  const renumbered = body.replace(
    ...
    (_match, name: string) => {
      counter += 1;
      const originalId = originalIds.get(name);
      if (originalId === undefined) throw new Error('Cannot resolve a serialized shape ID.');
      spidMap.set(originalId, counter);
...
export function slideToXml(slide: Slide, ctx: ElementXmlCtx): string {
  ...
  const { renumbered: bodyWithIds, spidMap } = renumberCNvPrIds(rawBody, slide.elements);
  const connectedBody = attachConnectors(bodyWithIds, slide.elements, spidMap);
