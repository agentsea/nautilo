...
export class PptxWriter {
  ...
  private mediaSeq = 0;
  private binaryParts = new Map<string, Uint8Array>();
  ...
  addBinaryPart(path: string, bytes: Uint8Array, contentType: string): void {
    this.binaryParts.set(path, bytes);
    this.addOverride(`/${path}`, contentType);
  }
  ...
  async build(): Promise<Uint8Array> {
    ...
    for (const [path, bytes] of this.binaryParts) zip.file(path, bytes);
