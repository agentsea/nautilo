...
export function themeToXml(theme: Theme, index: number): string {
  ...
  const fontScheme =
    ...
    `<a:majorFont><a:latin typeface="${majorTypeface}"/><a:ea typeface=""/><a:cs typeface=""/></a:majorFont>` +
    `<a:minorFont><a:latin typeface="${minorTypeface}"/><a:ea typeface=""/><a:cs typeface=""/></a:minorFont>` +
    `</a:fontScheme>`;
