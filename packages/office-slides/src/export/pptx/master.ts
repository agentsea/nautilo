 ...
 *
 * v1 scope:
 * - Solid or gradient background fill from `master.background.fill`.
 * - Standard `<p:clrMap>` with the OOXML identity mapping.
 * - Layout IDs and relationships supplied by the orchestrator.
 * - No `<p:txStyles>` — placeholder typography overrides are deferred to v1.5.
 */
export function masterToXml(master: Master, _index: number, layoutRIds: Array<{ rId: string; id: number }> = []): string {
  const bgFill = fillXml(master.background.fill);
