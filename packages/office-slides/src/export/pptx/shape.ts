import type { Frame, PlaceholderRef, ShapeElement, ShapeKind, Stroke, TextElement } from '../../model/element.js';
...
const PLACEHOLDER_TYPE = new Map<PlaceholderRef['type'], string>([
  ['title', 'title'], ['subtitle', 'subTitle'], ['body', 'body'],
  ['caption', 'body'], ['big-number', 'body'],
]);

function placeholderNvPr(ref: PlaceholderRef | undefined): string {
  if (!ref) return '<p:nvPr/>';
  const type = PLACEHOLDER_TYPE.get(ref.type);
  if (!type || !Number.isSafeInteger(ref.index) || ref.index < 0) {
    throw new Error('Cannot export an invalid slide placeholder reference.');
  }
  return `<p:nvPr><p:ph type="${type}" idx="${ref.index}"/></p:nvPr>`;
}
...
export function shapeToXml(
  ...
  const nv =
    ...
    `<p:cNvSpPr/>` +
    placeholderNvPr(el.placeholderRef) +
    `</p:nvSpPr>`;
