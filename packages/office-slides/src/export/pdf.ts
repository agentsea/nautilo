...
export interface ExportSlidesPdfOptions {
  ...
  /** Refuse missing, failed, or timed-out images instead of painting a blank fallback. */
...
export async function exportSlidesPdf(
  ...
  const { map, temp } = await resolveDeckImages(doc, opts.imageFetcher, opts.strictAssets === true);
  ...
  try {
    ...
    for (const slide of doc.slides) {
      ...
      const imageStates = await Promise.all(srcs.map((s) => awaitImageLoaded(s, assetTimeoutMs)));
      if (opts.strictAssets) {
        const unavailable = imageStates.find((state) => state !== 'loaded');
        if (unavailable) {
          throw new Error(
            unavailable === 'timed-out'
              ? 'An image did not finish loading before PDF export.'
              : 'An image could not be loaded for PDF export.',
          );
        }
      }
...
async function resolveDeckImages(
  ...
  const entries = await Promise.all(
    Array.from(originals).map(async (src) => {
      ...
      try {
        ...
        if (strictAssets) return { src, error: true as const };
  ...
  if (failed && 'error' in failed) {
    const allocated = entries.flatMap((entry) =>
      'url' in entry && entry.temp ? [entry.url] : [],
    );
    if (allocated.length > 0) {
      evictImageSrcs(allocated);
      for (const url of allocated) URL.revokeObjectURL(url);
    }
    throw new Error(`An image could not be fetched for PDF export: ${failed.src}`);
  }
...
function prepareExportSlide(
  ...
  if (cloned.background.image) {
    ...
    if (inheritedBg) {
      const clonedBg = structuredClone(inheritedBg);
 ...
 */
function awaitImageLoaded(src: string, timeoutMs: number): Promise<'loaded' | 'failed' | 'timed-out'> {
  return new Promise((resolve) => {
    let settled = false;
    let timer: ReturnType<typeof setTimeout> | undefined;
    const finish = (state: 'loaded' | 'failed' | 'timed-out'): void => {
      if (!settled) {
        settled = true;
    ...
    const img = getOrLoadImage(src, () => finish(isImageFailed(src) ? 'failed' : 'loaded'));
    ...
    if (!settled && timeoutMs > 0) timer = setTimeout(() => finish('timed-out'), timeoutMs);
  });
}
