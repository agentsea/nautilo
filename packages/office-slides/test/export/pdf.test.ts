...
describe('exportSlidesPdf', () => {
  ...
  it('strictAssets refuses a fetch failure instead of substituting a blank image', async () => {
    const { exportSlidesPdf } = await import('../../src/export/pdf');
    const doc = baseDoc([
      { ...blankSlide('s1'), elements: [imageElement('i1', 'http://cdn/missing.png')] },
    ]);

    await expect(exportSlidesPdf(doc, {
      scale: 1,
      strictAssets: true,
      imageFetcher: async () => { throw new Error('missing'); },
    })).rejects.toThrow('could not be fetched');
  });

  it('keeps the compatible blank-image fallback when strictAssets is omitted', async () => {
    const { exportSlidesPdf } = await import('../../src/export/pdf');
    const doc = baseDoc([
      { ...blankSlide('s1'), elements: [imageElement('i1', 'http://cdn/missing.png')] },
    ]);
  ...
  it('strictAssets refuses an image that fails browser decoding', async () => {
    ...
    const { exportSlidesPdf } = await import('../../src/export/pdf');
    const doc = baseDoc([
      { ...blankSlide('s1'), elements: [imageElement('i1', `data:image/png;base64,${PNG_1x1_B64}`)] },
    ]);

    await expect(exportSlidesPdf(doc, { scale: 1, strictAssets: true }))
