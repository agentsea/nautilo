...
describe('PptxWriter', () => {
  ...
  it('writes an exact binary package part and its content-type override', async () => {
    const w = new PptxWriter();
    const payload = new Uint8Array([0, 255, 17, 128]);
    w.addBinaryPart('ppt/embeddings/data.bin', payload, 'application/x-test-binary');
    const zip = await JSZip.loadAsync(await w.build());

    expect(await zip.file('ppt/embeddings/data.bin')!.async('uint8array')).toEqual(payload);
    expect(await zip.file('[Content_Types].xml')!.async('string')).toContain(
      '<Override PartName="/ppt/embeddings/data.bin" ContentType="application/x-test-binary"/>',
    );
  });
});
