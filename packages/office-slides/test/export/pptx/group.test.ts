...
describe('group', () => {
  ...
  it('rejects chart export when its relationship resolver is missing', () => {
    ...
    expect(() => elementToXml(chart, ctx)).toThrow(
      'Cannot export chart "ch": chart relationship resolver is unavailable.',
    );
  });

  it('dispatches a chart nested in a group through its relationship resolver', () => {
    const chart: ChartElement = {
      id: 'ch2',
      frame: { x: 0, y: 0, w: 10, h: 10, rotation: 0 },
      type: 'chart',
    ...
    const xml = groupToXml(g, { ...ctx, resolveChartRId: () => 'rIdChart' });
    expect(xml).toContain('<p:graphicFrame>');
    expect(xml).toContain('r:id="rIdChart"');
