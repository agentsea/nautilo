...
describe('LayoutEditStore', () => {
  ...
  it('seeds an edited layout from its own master, with document-master fallback', () => {
    const source = new MemSlidesStore().read();
    source.masters.push({
      ...structuredClone(source.masters[0]),
      id: 'secondary',
      placeholderStyles: {
        ...structuredClone(source.masters[0].placeholderStyles),
        title: { ...source.masters[0].placeholderStyles.title, fontSize: 73 },
      },
    });
    source.layouts.find((layout) => layout.id === 'title-body')!.masterId = 'secondary';
    const real = new MemSlidesStore(source);

    const secondary = new LayoutEditStore(real, 'title-body').read();
    const title = secondary.slides[0].elements.find((e) => e.placeholderRef?.type === 'title');
    expect(title?.type === 'text' ? title.data.blocks[0].inlines[0].style.fontSize : undefined).toBe(73);

    const fallbackSource = real.read();
    fallbackSource.layouts.find((layout) => layout.id === 'title-body')!.masterId = 'missing';
    const fallbackStore = new MemSlidesStore(fallbackSource);
    const fallback = new LayoutEditStore(fallbackStore, 'title-body').read();
    const fallbackTitle = fallback.slides[0].elements.find((e) => e.placeholderRef?.type === 'title');
    expect(fallbackTitle?.type === 'text'
      ? fallbackTitle.data.blocks[0].inlines[0].style.fontSize
      : undefined).toBe(real.read().masters[0].placeholderStyles.title.fontSize);
  });
