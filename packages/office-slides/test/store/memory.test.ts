...
describe('MemSlidesStore — connector methods', () => {
  ...
  it('updateConnectorElbowBend preserves exact values and clears on undefined', () => {
    const { store, slideId, connectorId } = setup();
    store.batch(() => store.updateConnectorRouting(slideId, connectorId, 'elbow'));
    ...
    let c = store.read().slides
      .find((s) => s.id === slideId)!.elements
      .find((e) => e.id === connectorId);
    if (c?.type === 'connector') expect(c.elbowBend).toBe(0.123456);
  ...
  it('updateConnectorCurveBend preserves signed values exactly across serialization', () => {
    const { store, slideId, connectorId } = setup();
    store.batch(() => store.updateConnectorRouting(slideId, connectorId, 'curved'));
    store.batch(() => store.updateConnectorCurveBend(slideId, connectorId, -12.3456789));
    const serialized = JSON.stringify(store.read());
    const restored = new MemSlidesStore(JSON.parse(serialized) as SlidesDocument);
    let c = restored.read().slides
      .find((s) => s.id === slideId)!.elements
      .find((e) => e.id === connectorId);
    if (c?.type === 'connector') expect(c.curveBend).toBe(-12.3456789);
  ...
  it('rejects non-finite derived curve geometry without mutating the connector', () => {
    const { store, slideId, connectorId } = setup();
    store.batch(() => store.updateConnectorRouting(slideId, connectorId, 'curved'));
    store.batch(() => store.updateConnectorCurveBend(slideId, connectorId, -7));
    const before = store.read().slides
      .find((s) => s.id === slideId)!.elements
      .find((e) => e.id === connectorId);

    expect(() => store.batch(() => {
      store.updateConnectorCurveBend(slideId, connectorId, Number.MAX_VALUE);
    })).toThrow('non-finite geometry');
