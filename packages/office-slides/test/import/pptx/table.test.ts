...
describe('parseTable — structured TableElement', () => {
  it('reports table style options that the native model cannot retain', () => {
    const report = new ImportReport();
    const parsed = parseTable(frame(`<p:graphicFrame><p:xfrm/><a:graphic><a:graphicData><a:tbl><a:tblPr firstRow="1" bandRow="1"><a:tableStyleId>style</a:tableStyleId></a:tblPr><a:tblGrid/><a:tr h="1"/></a:tbl></a:graphicData></a:graphic></p:graphicFrame>`), ctx(report));
    expect(parsed).toHaveLength(1);
    expect(report.summary()).toContain('1 table(s) lost style options');
  });

  it('recognizes the OOXML boolean lexical value true for table style options', () => {
    const report = new ImportReport();
    parseTable(frame(`<p:graphicFrame><p:xfrm/><a:graphic><a:graphicData><a:tbl><a:tblPr firstRow="true"><a:tableStyleId>style</a:tableStyleId></a:tblPr><a:tblGrid/><a:tr h="1"/></a:tbl></a:graphicData></a:graphic></p:graphicFrame>`), ctx(report));
    expect(report.tableStyleOptionsDropped).toBe(1);
  });
  ...
  it('parses each <a:lnL/R/T/B> into a per-side CellBorder with width and color', () => {
    ...
    expect(typeof border?.left?.width).toBe('number');
