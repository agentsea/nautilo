...
import { textBodyToXml } from '../../../src/export/pptx/text';
...
describe('parseTextBody — runs', () => {
  it('inherits paragraph defRPr and lets explicit runs override individual properties', () => {
    const t = txBody(`<a:txBody>
      <a:bodyPr/>
      <a:p>
        <a:pPr><a:defRPr sz="2000" b="1"><a:latin typeface="Georgia"/></a:defRPr></a:pPr>
        <a:r><a:t>default</a:t></a:r>
        <a:br/>
        <a:r><a:rPr sz="1600" b="0"><a:latin typeface="Arial"/></a:rPr><a:t>override</a:t></a:r>
      </a:p>
    </a:txBody>`);
    const block = parseTextBody(t, { report: new ImportReport() })[0];

    expect(block.inlines).toEqual([
      { text: 'default', style: { fontSize: 20, bold: true, fontFamily: 'Georgia' } },
      { text: '\n', style: { fontSize: 20, bold: true, fontFamily: 'Georgia' } },
      { text: 'override', style: { fontSize: 16, bold: false, fontFamily: 'Arial' } },
    ]);

    const exported = textBodyToXml({ blocks: [block] });
    const reparsed = parseTextBody(txBody(exported), { report: new ImportReport() })[0];
    expect(reparsed.inlines.map(({ text, style }) => ({
      text,
      fontFamily: style.fontFamily,
      fontSize: style.fontSize,
      bold: style.bold ?? false,
    }))).toEqual(block.inlines.map(({ text, style }) => ({
      text,
      fontFamily: style.fontFamily,
      fontSize: style.fontSize,
      bold: style.bold ?? false,
  ...
  it('clears inherited strike, baseline and spacing with explicit neutral run values', () => {
    const t = txBody(`<a:txBody>
      <a:bodyPr/>
      <a:p>
        <a:pPr><a:defRPr strike="dblStrike" baseline="30000" spc="250"/></a:pPr>
        <a:r><a:rPr strike="noStrike" baseline="0" spc="0"/><a:t>neutral</a:t></a:r>
        <a:r><a:rPr strike="sngStrike" baseline="-25000"/><a:t>cross-axis</a:t></a:r>
      </a:p>
    </a:txBody>`);
    const block = parseTextBody(t, { report: new ImportReport() })[0];

    expect(block.inlines).toEqual([
      { text: 'neutral', style: { strikethrough: false } },
      {
        text: 'cross-axis',
        style: { strikethrough: true, subscript: true, letterSpacing: 2.5 },
      },
    ]);
  });
