...
function makeCapturingMount(captured: {
  ...
  return function mount(opts: MountSlidesTextBoxOptions): SlidesTextBoxEditor {
    ...
    return {
      ...
      getDraftSnapshot: () => ({
        blocks: structuredClone(captured.draft ?? opts.blocks),
        contentHeight: opts.frame.h,
      }),
...
describe('table cell text-edit entry — box sizing', () => {
  ...
  it('overlays a live cell draft without changing the table store', () => {
    const { canvas, overlay, store } = setup();
    let sid = '';
    let tableId = '';
    store.batch(() => {
      sid = store.addSlide('blank');
      tableId = store.addElement(sid, {
        type: 'table',
        frame: { x: 200, y: 200, w: 200, h: 200, rotation: 0 },
        data: tableData(),
      });
    ...
    const captured: { opts?: MountSlidesTextBoxOptions; draft?: Block[] } = {};
    editor = initialize({
      canvas, overlay, store,
      hostWidth: 1920, hostHeight: 1080, dpr: 1,
      mountTextBox: makeCapturingMount(captured),
    ...
    captured.draft = [{
      id: 'live-cell',
      type: 'paragraph',
      inlines: [{ text: 'live cell draft', style: { italic: true } }],
      style: {},
    } as Block];

    const table = editor.getDraftSnapshot().slides[0].elements.find((item) => item.id === tableId) as TableElement;
    expect(table.data.rows[0].cells[0].body.blocks).toEqual(captured.draft);
    const canonical = store.read().slides[0].elements.find((item) => item.id === tableId) as TableElement;
    expect(canonical.data.rows[0].cells[0].body.blocks).toEqual([]);
