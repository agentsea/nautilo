...
describe('mountNotesPanel', () => {
  ...
  it('does not clobber the local value while the textarea is focused', () => {
    const { notes, store, editor } = makeFixture();
    const panel = mountNotesPanel(notes, store, editor);
    const ta = notes.querySelector<HTMLTextAreaElement>('textarea')!;
    const id = editor.getCurrentSlideId()!;
    ...
    const draft = panel.getDraftSnapshot();
    expect(draft?.slideId).toBe(id);
    expect(draft?.blocks[0].inlines[0].text).toBe('local in progress');
    expect((store.read().slides[0].notes[0].inlines[0] as { text: string }).text).toBe('from peer');
