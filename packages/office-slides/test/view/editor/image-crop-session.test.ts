...
describe('image crop session', () => {
  ...
  it('snapshots live crop geometry without committing the image', () => {
    const { canvas, overlay, store } = setup();
    let sid = '';
    store.batch(() => { sid = store.addSlide('blank'); });
    const id = addImage(store, sid, { x: 200, y: 200, w: 400, h: 300 });
    const changed = vi.fn();
    editor = makeEditor(store, canvas, overlay, changed);

    editor.enterImageCrop(id);
    const e = handleCenter(overlay, 'e');
    drag(overlay, e, { x: e.x - 100, y: e.y });

    const canonical = image(store, sid, id);
    expect(canonical.frame.w).toBeCloseTo(400);
    expect(canonical.data.crop).toBeUndefined();
    const draftSlide = editor.getDraftSnapshot().slides.find((slide) => slide.id === sid)!;
    const draft = draftSlide.elements.find((element) => element.id === id) as ImageElement;
    expect(draft.frame.x).toBeCloseTo(200);
    expect(draft.frame.w).toBeCloseTo(300);
    expect(draft.data.crop?.w).toBeCloseTo(0.75);
    expect(draft.data.crop?.h).toBeCloseTo(1);
    expect(editor.isCropping()).toBe(true);
