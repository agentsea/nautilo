...
describe('mountThumbnailPanel — scroll preservation across re-render', () => {
  it('restores parent scrollTop after innerHTML wipe (simulated browser clamp)', () => {
    ...
    Object.defineProperty(panel, 'innerHTML', {
      get() { return innerHTMLDesc.get!.call(panel) as unknown as string; },
      set(value: string) {
...
describe('mountThumbnailPanel — chunked render for large decks', () => {
  ...
  it('completes a large deck iteratively when requestAnimationFrame is unavailable', () => {
    vi.stubGlobal('requestAnimationFrame', undefined);
    let observed = 0;
    let firstBuildChunkFrames: number | undefined;
    let lastBuildChunkFrames: number | undefined;
    class CountingIntersectionObserver {
      constructor(_callback: IntersectionObserverCallback) {}
      observe(): void {
        observed++;
        const buildChunkFrames = new Error().stack
          ?.split('\n')
          .filter((line) => line.includes('buildChunk')).length ?? 0;
        if (observed === 1) firstBuildChunkFrames = buildChunkFrames;
        if (observed === slideCount) lastBuildChunkFrames = buildChunkFrames;
      }
      unobserve(): void {}
      disconnect(): void {}
    ...
    const slideCount = 2_005;
    const { panel, store, editor } = makeBigDeck(slideCount);

    mountThumbnailPanel(panel, store, editor);

    expect(panel.querySelectorAll('[data-slide-id]')).toHaveLength(slideCount);
    expect(observed).toBe(slideCount);
    // Recursive fallback advancement adds one buildChunk frame per batch.
    // Iteration keeps late thumbnails at the same call depth as the first.
    expect(firstBuildChunkFrames).toBeGreaterThan(0);
    expect(lastBuildChunkFrames).toBe(firstBuildChunkFrames);
    expect(rafQueue).toHaveLength(0);
  });
