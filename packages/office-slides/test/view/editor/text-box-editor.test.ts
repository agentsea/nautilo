...
interface MockTextBox extends SlidesTextBoxEditor {
  ...
  /** Replace the view-local recovery draft without committing it. */
  fireDraft(blocks: Block[], contentHeight: number): void;
...
function makeMockMount(): {
  ...
  function mount(opts: MountSlidesTextBoxOptions): SlidesTextBoxEditor {
    ...
    let mounted = true;
    let draftBlocks = structuredClone(opts.blocks);
    let draftContentHeight = opts.frame.h;
    ...
    const tb: MockTextBox = {
      ...
      getDraftSnapshot: () => ({
        blocks: structuredClone(draftBlocks),
        contentHeight: draftContentHeight,
      }),
      detach(): void {
        if (!mounted) return;
        mounted = false;
      ...
      fireDraft(blocks: Block[], contentHeight: number): void {
        draftBlocks = structuredClone(blocks);
        draftContentHeight = contentHeight;
        opts.onDraftChange?.();
      },
      opts,
...
describe('slides text-box editor wiring', () => {
  ...
  it('returns live rich text and grow-frame height without committing or blurring', () => {
    const { canvas, overlay, store } = makeFixture();
    const original = [paragraph('committed')];
    const { elementId } = addTextElement(store, original);
    const changed = vi.fn();
    const { mount, current } = makeMockMount();
    editor = initialize({
      canvas, overlay, store,
    ...
    dispatchDblClick(canvas, 200, 200);
    const live = [paragraph('live formatted')];
    live[0].inlines[0].style = { bold: true, fontSize: 28 };
    current()!.fireDraft(live, 275);

    const snapshot = editor.getDraftSnapshot();
    const drafted = snapshot.slides[0].elements.find((item) => item.id === elementId) as TextElement;
    const canonical = store.read().slides[0].elements.find((item) => item.id === elementId) as TextElement;
    expect(drafted.data.blocks).toEqual(live);
    expect(drafted.frame.h).toBe(275);
    expect(canonical.data.blocks).toEqual(original);
    expect(canonical.frame.h).toBe(200);
    expect(editor.isTextEditing()).toBe(true);
    expect(changed).toHaveBeenCalledTimes(1);

    drafted.data.blocks[0].inlines[0].text = 'mutated snapshot';
    expect(editor.getDraftSnapshot().slides[0].elements.find((item) => item.id === elementId)).toMatchObject({
      data: { blocks: live },
    });
  });
  ...
  it('shape edit-mode commits typed text into ShapeElement.data.text via withShapeText', () => {
    ...
    dispatchDblClick(canvas, 150, 150);
    expect(editor.getEditingElementId()).toBe(shapeId);
    current()!.fireDraft([paragraph('live shape draft')], 100);
    const draft = editor.getDraftSnapshot().slides[0].elements[0] as {
      data: { text?: { blocks: Block[] } };
    };
    expect(draft.data.text?.blocks[0].inlines[0].text).toBe('live shape draft');
    expect((store.read().slides[0].elements[0] as { data: { text?: unknown } }).data.text).toBeUndefined();
