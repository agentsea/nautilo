...
describe('snapToDefaults', () => {
  ...
  it('all adjustments must qualify (multi-index)', () => {
    // wedgeRectCallout defaults [-20833, 62500], one frame axis is 100000 units → 5% = 5000
    // both close → snap
  ...
  it('does not snap distant legal signed-coordinate callout tails', () => {
    for (const kind of ['wedgeRectCallout', 'wedgeEllipseCallout', 'wedgeRoundRectCallout', 'cloudCallout'] as const) {
      expect(snapToDefaults(kind, [100000, 62500])).toEqual([100000, 62500]);
      expect(snapToDefaults(kind, [-20833 + 5000, 62500])).toEqual(defaultAdjustmentsFor(kind));
      expect(snapToDefaults(kind, [-20833 + 5001, 62500])).toEqual([-20833 + 5001, 62500]);
    }
  });
