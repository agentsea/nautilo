...
describe('ThumbnailScheduler', () => {
  ...
  it('batches different slide ids into a single flush', () => {
    ...
    expect(onFlush).toHaveBeenCalledTimes(1);
    const flushedIds: unknown = onFlush.mock.calls[0]?.[0];
    expect(Array.isArray(flushedIds)).toBe(true);
    const rawIds: unknown[] = Array.isArray(flushedIds) ? flushedIds : [];
    expect(rawIds.sort()).toEqual(['s1', 's2']);
  });
