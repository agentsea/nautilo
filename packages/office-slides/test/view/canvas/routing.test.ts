...
describe('routeCurved bend', () => {
  ...
  it('preserves negative, zero, and large finite bend values', () => {
    const a = { x: 0, y: 0 };
    const b = { x: 300, y: 0 };
    expect(routeCurved(a, 0, b, Math.PI, -2).c1.x).toBeCloseTo(-200, 5);
    expect(routeCurved(a, 0, b, Math.PI, 0).c1.x).toBe(0);
    expect(routeCurved(a, 0, b, Math.PI, 99).c1.x).toBeCloseTo(9900, 5);
  });

  it('refuses bends whose derived control points overflow', () => {
    expect(() => routeCurved(
      { x: 0, y: 0 },
      0,
      { x: 300, y: 0 },
