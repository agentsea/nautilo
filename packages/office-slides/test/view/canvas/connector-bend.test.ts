...
describe('bendFromCursor', () => {
  ...
  it('curved: preserves the current bend when the midpoint solver is singular', () => {
    const c = curved({ x: 0, y: 0 }, { x: 300, y: 0 }, -4.25);
    expect(bendFromCursor(c, { x: 150, y: 100 }, EMPTY)).toBe(-4.25);
  });

  it('curved: analytic solver returns signed, unclamped bend values', () => {
    ...
    const c: ConnectorElement = {
      ...curved({ x: 0, y: 0 }, { x: 300, y: 200 }),
      start: { kind: 'attached', elementId: 'target', siteIndex: 1 },
    };
    const elements = new Map<string, Element>([['target', target]]);
    const positive = bendFromCursor(c, { x: 200, y: 300 }, elements);
    const negative = bendFromCursor(c, { x: 200, y: -300 }, elements);
    expect(positive).not.toBeNull();
    expect(negative).not.toBeNull();
    expect(Math.sign(positive!)).toBe(-Math.sign(negative!));
    expect(Math.max(Math.abs(positive!), Math.abs(negative!))).toBeGreaterThan(3);
  });
});
