...
describe('LEFT_RIGHT_RIBBON_HANDLES', () => {
  ...
  it('clamps head spread to the preset maximum derived from fold height', () => {
    const next = LEFT_RIGHT_RIBBON_HANDLES[0].apply(
      { w: 200, h: 100 },
      [50000, 50000, 30000],
      { x: 0, y: -100 },
    );
    expect(next[0]).toBe(70000); // maxAdj1 = 100000 - a3
  });

  it('clamps tail length to the preset maximum derived from frame aspect', () => {
    const next = LEFT_RIGHT_RIBBON_HANDLES[1].apply(
      { w: 200, h: 100 },
      [50000, 50000, 16667],
      { x: 200, y: 0 },
    );
    expect(next[1]).toBe(93750); // 100000 * (wd2 - wd32) / ss
  });

  it('keeps head spread valid when a fold drag lowers its dependent maximum', () => {
    const next = LEFT_RIGHT_RIBBON_HANDLES[2].apply(
      { w: 200, h: 100 },
      [90000, 50000, 0],
      { x: 0, y: 0 },
    );
    expect(next[2]).toBe(33333);
    expect(next[0]).toBe(66667);
  });
});
