import { describe, it, expect } from 'vitest';
import '../../../../../src/view/canvas/test-canvas-env';
import { createTestCanvas } from '../../../../../src/view/canvas/test-canvas-env';
import { buildDonut } from '../../../../../src/view/canvas/shapes/basic/donut';

describe('buildDonut', () => {
  it('produces concentric ellipses fillable as a ring with evenodd rule', () => {
    const path = buildDonut({ w: 100, h: 60 });
    const ctx = createTestCanvas(200, 200).getContext('2d');
    // Outer ring point — inside the donut.
    expect(ctx.isPointInPath(path, 5, 30, 'evenodd')).toBe(true);
    // Centre — inside the hole, NOT filled.
    expect(ctx.isPointInPath(path, 50, 30, 'evenodd')).toBe(false);
    // Outside everything.
    expect(ctx.isPointInPath(path, -5, 30, 'evenodd')).toBe(false);
  });

  it('evaluates imported out-of-range adjustments through the preset pin guide', () => {
    const ctx = createTestCanvas(200, 200).getContext('2d');
    const below = buildDonut({ w: 100, h: 60 }, [-25000]);
    const atMin = buildDonut({ w: 100, h: 60 }, [0]);
    const above = buildDonut({ w: 100, h: 60 }, [100000]);
    const atMax = buildDonut({ w: 100, h: 60 }, [50000]);
    for (let y = 5; y < 60; y += 10) {
      for (let x = 5; x < 100; x += 10) {
        expect(ctx.isPointInPath(below, x, y, 'evenodd')).toBe(
          ctx.isPointInPath(atMin, x, y, 'evenodd'),
        );
        expect(ctx.isPointInPath(above, x, y, 'evenodd')).toBe(
          ctx.isPointInPath(atMax, x, y, 'evenodd'),
        );
      }
    }
  });
});
