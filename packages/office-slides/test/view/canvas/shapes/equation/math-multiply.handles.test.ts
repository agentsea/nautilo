...
import {
  MATH_MULTIPLY_ADJUSTMENTS,
  MATH_MULTIPLY_HANDLES,
} from "../../../../../src/view/canvas/shapes/equation/math-multiply";
...
describe("MATH_MULTIPLY_HANDLES", () => {
  ...
  it("clamps at the mathMultiply preset maximum and preserves other slots", () => {
    expect(MATH_MULTIPLY_ADJUSTMENTS[0].max).toBe(51965);
    const next = MATH_MULTIPLY_HANDLES[0].apply(
      { w: 200, h: 100 },
      [23520, 42],
      { x: 0, y: 100 },
    );
    expect(next).toEqual([51965, 42]);
  });
});
