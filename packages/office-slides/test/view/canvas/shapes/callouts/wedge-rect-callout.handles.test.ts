...
describe('WEDGE_RECT_CALLOUT_HANDLES', () => {
  ...
  describe('apply', () => {
    ...
    it('preserves legal positive signed DrawingML coordinates', () => {
      const next = handle.apply(FRAME, [0, 0], { x: 9999, y: 9999 });
      expect(next).toEqual([4949500, 9949000]);
    });

    it('preserves legal negative signed DrawingML coordinates', () => {
      const next = handle.apply(FRAME, [0, 0], { x: -9999, y: -9999 });
      expect(next).toEqual([-5049500, -10049000]);
    });
