...
import {
  angularHandle,
  linearTopEdgeHandle,
} from "../../../../src/view/canvas/shapes/handles";
...
describe("angularHandle", () => {
  ...
  describe("apply", () => {
    it("pointer at right edge → 0°", () => {
      const next = angHandle.apply({ w: 200, h: 200 }, [0], { x: 200, y: 100 });
      expect(next[0]).toBe(0);
    });

    it("pointer below centre → 90°", () => {
      const next = angHandle.apply({ w: 200, h: 200 }, [0], { x: 100, y: 200 });
    ...
    it("pointer above centre at start 0° → unwraps to ~−90° then clamps to 0", () => {
      ...
      const next = angHandle.apply({ w: 200, h: 200 }, [0], { x: 100, y: 0 });
      expect(next[0]).toBe(0);
    });
    ...
    it("crosses 0°/360° boundary without snapping (start 5°, drag to atan2 = -5°)", () => {
      ...
      const next = wide.apply({ w: 200, h: 200 }, start, {
        x: 200,
        y: 100 + (200 * Math.tan((5 * Math.PI) / 180)) / 2,
      });
    ...
    it("unwraps across more than four turns without an iteration ceiling", () => {
      const manyTurnSpec: AdjustmentSpec = {
        name: "Many turns",
        defaultValue: 0,
        min: 0,
        max: 10 * 360 * 60000,
      };
      const manyTurn = angularHandle({
        center: ({ w, h }) => ({ x: w / 2, y: h / 2 }),
        radius: ({ w, h }) => ({ rx: w / 2, ry: h / 2 }),
        index: 0,
        spec: manyTurnSpec,
      });
      const next = manyTurn.apply(
        { w: 200, h: 200 },
        [(8 * 360 + 355) * 60000],
        { x: 200, y: 100 + 100 * Math.tan((5 * Math.PI) / 180) },
      );
      expect(next[0]).toBeCloseTo((9 * 360 + 5) * 60000, -3);
    });

    it("preserves both exact 180° ties before the spec clamp", () => {
      expect(
        angHandle.apply({ w: 200, h: 200 }, [0], { x: 0, y: 100 })[0],
      ).toBe(180 * 60000);
      expect(
        angHandle.apply({ w: 200, h: 200 }, [270 * 60000], {
          x: 100,
          y: 200,
        })[0],
      ).toBe(90 * 60000);
    ...
    it("clamps above spec.max", () => {
      ...
      const next = narrow.apply({ w: 200, h: 200 }, [0], { x: 100, y: 200 });
      expect(next[0]).toBe(90 * 60000);

      // Pointer at left edge → atan2 = 180°. Should clamp to 90°.
      const past = narrow.apply({ w: 200, h: 200 }, [0], { x: 0, y: 100 });
    ...
    it("passes through other indices unchanged", () => {
      ...
      const next = second.apply({ w: 200, h: 200 }, [12345, 0], {
        x: 200,
        y: 100,
      });
