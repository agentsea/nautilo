...
describe('buildBentUpArrow', () => {
  ...
  it('pins imported adjustments to the preset domain', () => {
    const maxed = buildBentUpArrow({ w: 200, h: 200 }, [50000, 50000, 50000]);
    const overflow = buildBentUpArrow({ w: 200, h: 200 }, [999999, 999999, 999999]);
    const ctx = createTestCanvas(400, 400).getContext('2d');
    for (const point of [[20, 20], [100, 100], [180, 180]] as const) {
      expect(ctx.isPointInPath(overflow, point[0], point[1])).toBe(
        ctx.isPointInPath(maxed, point[0], point[1]),
      );
    }
  });
});
