...
describe('CHEVRON_HANDLES', () => {
  ...
  describe('position', () => {
    ...
    it('uses the frame-dependent OOXML maximum', () => {
      expect(handle.position(FRAME, [150000]).x).toBeCloseTo(50, 5);
      expect(handle.position(FRAME, [999999]).x).toBeCloseTo(0, 5);
  ...
  describe('apply', () => {
    ...
    it('pointer near the left edge preserves the valid wide-frame range', () => {
      const next = handle.apply(FRAME, [50000], { x: 25, y: 50 });
      expect(next[0]).toBe(175000);
    });
