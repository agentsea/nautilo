...
describe('computeConnectorFrame — non-straight routings', () => {
  ...
  it.each([-8, 0, 1_000_000])(
    'curved: bend %s produces a finite frame',
    (curveBend) => {
      const target: Element = {
        id: 't1',
        type: 'shape',
      ...
      const c: ConnectorElement = {
        ...baseConnector(
          { kind: 'attached', elementId: 't1', siteIndex: 1 },
          { kind: 'free', x: 100, y: 250 },
        ),
        routing: 'curved',
        curveBend,
      };
      const frame = computeConnectorFrame(
        c,
        new Map<string, Element>([['t1', target]]),
      );
      expect([
        frame.x,
        frame.y,
