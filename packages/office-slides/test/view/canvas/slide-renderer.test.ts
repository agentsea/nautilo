...
import { buildLayoutSlide, BUILT_IN_LAYOUTS } from '../../../src/model/layout';
...
describe('SlideRenderer.render', () => {
  it('resolves an explicit slide theme independently of the deck theme', () => {
    const { renderer, ctx } = makeRenderer();
    const alternate: Theme = {
      ...THEME,
      id: 'alternate',
      colors: { ...THEME.colors, background: '#123456' },
    };
    const slide = {
      ...blankSlide(),
      themeId: alternate.id,
      background: { fill: { kind: 'role' as const, role: 'background' as const, tint: 0 } },
    };
    renderer.render(slide, { ...DOC, themes: [THEME, alternate] });
  ...
  describe('image-fill backgrounds', () => {
    ...
    it('paints the slide background image stretched to 1920x1080', async () => {
      ...
      const [, dx, dy, dw, dh] =
        ((ctx.drawImage as ReturnType<typeof vi.fn>).mock.calls[0] ?? []) as unknown[];
      expect(dx).toBe(0);
      expect(dy).toBe(0);
...
describe('authoring placeholder hints', () => {
  it('keeps empty slots out of output unless authoring hints are explicitly enabled', () => {
    const slide = buildLayoutSlide(BUILT_IN_LAYOUTS.find((layout) => layout.id === 'title-slide')!, DEFAULT_MASTER, THEME);
    const doc = { ...DOC, slides: [slide] };
    const before = JSON.stringify(doc);
    const ctx = createCtxSpy();
    const options = { hostWidth: 960, hostHeight: 540, dpr: 1 };
    drawSlide(asCtx(ctx), slide, doc, options);
    expect(ctx.fillText).not.toHaveBeenCalled();
    drawSlide(asCtx(ctx), slide, doc, { ...options, showPlaceholderHints: true });
    expect(ctx.fillText.mock.calls.map(([text]) => String(text))).toContain('Click to add title');
    expect(JSON.stringify(doc)).toBe(before);
  });

  it('preserves authored text even when it matches a hint', () => {
    const slide = buildLayoutSlide(BUILT_IN_LAYOUTS.find((layout) => layout.id === 'title-slide')!, DEFAULT_MASTER, THEME);
    const title = slide.elements.find((el) => el.type === 'text');
    if (!title || title.type !== 'text') throw new Error('Missing title fixture');
    title.data.blocks = [{ id: 'authored', type: 'paragraph', style: { ...DEFAULT_BLOCK_STYLE }, inlines: [{ text: 'Click to add title', style: {} }] }];
    const ctx = createCtxSpy();
    drawSlide(asCtx(ctx), slide, { ...DOC, slides: [slide] }, {
      hostWidth: 960, hostHeight: 540, dpr: 1,
    });
    expect(ctx.fillText.mock.calls.map(([text]) => String(text)).join('')).toContain('Click to add title');
  });
});
