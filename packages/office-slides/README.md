...
- **Renderer** — `SlideRenderer.render(slide, doc)` paints a slide to a Canvas 2D context. Every `ctx.fillStyle` / `ctx.strokeStyle` routes through `resolveColor(themeColor, theme)` so role-bound colors follow the deck's active theme. Text rendering delegates to `@nautilo/office-docs`'s layout/paint pipeline.
...
- **Slide Model** — `SlidesDocument → Theme/Master/Layout/Slide → Element` hierarchy. Slides are free-position canvases; elements are text boxes (rich text via `@nautilo/office-docs`), shapes (55+ OOXML presets), connectors, groups, tables, charts, and images.
...
pnpm --filter @nautilo/office-slides test         # Run unit tests (Vitest)
pnpm --filter @nautilo/office-slides test:watch   # Watch mode
pnpm --filter @nautilo/office-slides build        # Library build (ESM + CJS + .d.ts)
pnpm --filter @nautilo/office-slides typecheck    # TypeScript check
```
...
| Rich text | `@nautilo/office-docs` (text-box layout / paint reuses the docs engine) |
