ALTER TABLE "reflection_record_semantic_work" ADD COLUMN "quarantine_round" integer DEFAULT 0 NOT NULL;--> statement-breakpoint
ALTER TABLE "reflection_record_semantic_work" ADD COLUMN "recover_after" timestamp with time zone;--> statement-breakpoint
DROP TRIGGER "reflection_record_semantic_work_update_guard" ON "reflection_record_semantic_work";--> statement-breakpoint
UPDATE "reflection_record_semantic_work"
SET "quarantine_round" = 1,
    "recover_after" = now()
WHERE "state" = 'quarantined';--> statement-breakpoint
...
EXECUTE FUNCTION "public"."reflection_semantic_work_guard_update"();--> statement-breakpoint
CREATE INDEX "idx_reflection_record_semantic_work_recovery" ON "reflection_record_semantic_work" USING btree ("state","recover_after","due_since","record_id");--> statement-breakpoint
ALTER TABLE "reflection_record_semantic_work" ADD CONSTRAINT "reflection_record_semantic_work_quarantine_round_bound" CHECK ("reflection_record_semantic_work"."quarantine_round" >= 0);--> statement-breakpoint
ALTER TABLE "reflection_record_semantic_work" ADD CONSTRAINT "reflection_record_semantic_work_recovery_coherent" CHECK (("reflection_record_semantic_work"."state" = 'quarantined') = ("reflection_record_semantic_work"."recover_after" is not null)
        and ("reflection_record_semantic_work"."quarantine_round" > 0 or "reflection_record_semantic_work"."state" <> 'quarantined'));
