import { PROTECTED_TOP_LEVEL_ROOM_KINDS } from "@nautilo/types";

/** PostgreSQL trigger contract paired with the shared execution Drizzle table.
 * Drizzle Kit does not model PL/pgSQL functions. The repository finalizer emits
 * this schema definition into its generated forward custom migration.
 */
export function protectedRoomMessageAuthoritySql(): string {
  return `CREATE OR REPLACE FUNCTION "public"."validate_conversation_shared_agent_shadow_execution"()
RETURNS trigger
LANGUAGE plpgsql
SECURITY INVOKER
  ...
  PERFORM 1
   ...
   WHERE session_row.id = NEW.session_id
     ...
     AND ((source_room.parent_room_id IS NULL
            AND source_room.kind IN (${PROTECTED_TOP_LEVEL_ROOM_KINDS.map((kind) => `'${kind}'`).join(", ")}))
       OR (source_room.parent_room_id IS NOT NULL
            AND source_room.kind = 'subthread'
            AND authority_room.kind IN (${PROTECTED_TOP_LEVEL_ROOM_KINDS.map((kind) => `'${kind}'`).join(", ")})))
