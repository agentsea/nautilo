...
export const users = pgTable("users", {
  ...
  /**
   * Email address is nullable. A local single-household Nautilo install
   * identifies users by `handle` (Logto username) +
   ...
   * remain distinct.
   *
   * Existing rows may still contain an address; invite-based account
   * creation can leave it NULL.
   */
  email: varchar("email", { length: 255 }).unique(),
  /**
   * OIDC subject identifier (`sub` claim) issued by the Server's Logto
   * cluster. It is reserved exclusively for the OIDC identity link.
   *   - NULL → legacy PIN-mode user, or a fresh row not yet linked on its
   ...
   *
   * Decoupled from `handle` (the human-chosen part of @handle@server)
   * and `email` (used for first-time linking by account migration tooling).
   * If Logto's sub format ever changes, only this column is rekeyed.
   *
   * Uniqueness is enforced via a partial unique index in the schema
   * migration (`users_external_id_unique` WHERE external_id IS NOT
  ...
  /**
   * Federated-id local part. Combined with the server's
   * `NAUTILO_HOSTNAME` it forms `@<handle>@<server>` — the owner's
   * addressable identity. Nullable for compatibility with older
   * databases; `seedDefaultOwner` populates missing handles at boot
   * (derived from `users.name` via
   ...
   * `findActorByHandle`, `updateOwnerHandle`. The old unique-handle
   * derivation was retired with the Logto JIT path; invites now supply
   * handles directly.
   *
  ...
  server: text("server"),
  /**
   * Soft-delete (account disable) columns. The legacy `server_role`
   * enum was removed; server-wide authority is now derived purely
   * from Capabilities (Group→Role membership, REL-CAP-HUM); the enum was
   * a divergent second RBAC axis and was dropped from the schema.
   *
  ...
  /**
   * Historical authenticated HTTP activity, bumped best-effort and
   * coalesced per user. It does not represent a live connection or
   * Human interaction and must not classify online/idle/offline state.
   */
  lastSeenAt: timestamp("last_seen_at", { withTimezone: true }),
  /**
   * The Human user's own profile picture (square avatar). Distinct
  ...
  /**
   * Account-level timezone, IANA name (e.g. "Europe/Athens",
