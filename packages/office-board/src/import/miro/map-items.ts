...
function captionInit(
  ...
  ends:
    | { frame: Frame; siteIndex: number; kind: ShapeKind | undefined }
    | undefined,
  otherEnds:
    | { frame: Frame; siteIndex: number; kind: ShapeKind | undefined }
    | undefined,
  ...
  const blocks = miroHtmlToBlocks(caption.content, {
    ...textStyle,
    alignment: "center",
  });
  if (!blocks.some((b) => b.inlines.some((i) => i.text.trim() !== "")))
    return undefined;
...
export function mapMiroItems(input: MiroImportInput): MiroMapResult {
  ...
  const bump = (type: string) => {
    Object.defineProperty(skipped, type, {
      value: (Object.hasOwn(skipped, type) ? skipped[type] : 0) + 1,
      enumerable: true,
      configurable: true,
      writable: true,
    });
  ...
  const approx = (kind: string) => {
    approximated[kind] = (approximated[kind] ?? 0) + 1;
  };

  // Duplicate handles cannot be mapped faithfully: reject before minting IDs
  // or invoking the caller's image resolver, rather than attach to the last row.
  const ids = new Set<string>();
  for (const item of input.items) {
    if (!item.id || ids.has(item.id))
      throw new Error(`Invalid or duplicate Miro item id: ${item.id}`);
    ids.add(item.id);
  }
  ...
  for (const item of ordered) {
    ...
    if (item.type === "text") {
      ...
      const sized =
        item.geometry?.height === undefined
          ? resizeAboutCentre(
              frame,
              estimateTextHeight(blocks, num(style.fontSize)),
            )
          : frame;
    ...
    if (item.type === "image") {
      ...
      inits.push({
        ...
        type: "image",
        frame,
        data: { src: input.resolveImageUrl(src) },
      } as ElementInit & { __id: string });
      continue;
    }
    ...
    inits.push({
      ...
      data: {
        ...
        fill: { kind: "srgb", value: "#FFFFFF" },
        stroke: {
          color: str(style.cardTheme) ?? str(style.fillColor) ?? "#2d9bf0",
          width: 2,
        },
  ...
  for (const connector of input.connectors) {
    ...
    const startArrow = arrowheadOf(
      str(style.startStrokeCap),
      DEFAULT_START_CAP,
      bumpArrowhead,
    );
    const endArrow = arrowheadOf(
      str(style.endStrokeCap),
      DEFAULT_END_CAP,
      bumpArrowhead,
    );
    inits.push({
    ...
    for (const caption of connector.captions ?? []) {
      const init = captionInit(
        caption,
        startFrame
          ? { frame: startFrame, siteIndex: start.siteIndex, kind: startKind }
          : undefined,
        endFrame
          ? { frame: endFrame, siteIndex: end.siteIndex, kind: endKind }
          : undefined,
        style,
      );
      if (init) {
