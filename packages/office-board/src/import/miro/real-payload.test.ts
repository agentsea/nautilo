// Modified by Nautilo: typed assertions against the native model.
import type { ElementInit } from "@nautilo/office-slides/node";
import { asShape } from "../../../test/element-kind";
...
const CONNECTOR = {
  ...
  captions: [{ content: "<p>(1) create doc with name</p>", position: "49.5%" }],
};
...
describe("mapMiroItems on verbatim Miro API payload", () => {
  ...
  const textOf = (init: ElementInit): string => {
    const blocks =
      init.type === "text"
        ? init.data.blocks
        : init.type === "shape"
          ? (init.data.text?.blocks ?? [])
          : [];
    return blocks
      .flatMap((b) => b.inlines ?? [])
      .map((i) => i.text)
  ...
  const byText = (needle: string): ElementInit => {
    const found = inits.find((i) => textOf(i).includes(needle));
    if (!found) throw new Error(`Missing mapped text: ${needle}`);
    return found;
  };

  const shape = asShape(byText("Hello"));
  ...
  it("gives the connector its Miro colour, width and arrowhead", () => {
    ...
    expect(connector.arrowheads).toEqual({
  ...
  it("decodes a frame title, which Miro delivers already escaped", () => {
    expect(textOf(byText("Creating Document"))).toBe(
      "Creating Document & Auth Webhook",
    );
  });
