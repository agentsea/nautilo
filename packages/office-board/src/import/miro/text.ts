...
export function estimateTextHeight(
  ...
  const spacing =
    DEFAULT_BLOCK_STYLE.marginTop + DEFAULT_BLOCK_STYLE.marginBottom;
...
export function estimateTextWidth(
  ...
  const longest = blocks.reduce(
    (max, block) =>
      Math.max(
        max,
        block.inlines.reduce((n, i) => n + i.text.length, 0),
      ),
    0,
  );
...
export function miroAlignment(
  align: string | undefined,
): BlockStyle["alignment"] | undefined {
 ...
 */
export function miroHtmlToBlocks(
  ...
  // One HTML parser in browser and Node; no global DOM shim or HTML execution.
  ...
  const walk = (node: DefaultTreeAdapterMap["childNode"], marks: Marks) => {
    if (node.nodeName === "#text" && "value" in node) {
      const text = node.value;
      if (text)
        current.push({ text, style: { ...inlineBase, ...marks } } as Inline);
      return;
    }
  ...
  for (const child of doc.childNodes) walk(child, {});
  flush();
