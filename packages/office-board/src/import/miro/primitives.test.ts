...
describe("miroFrame", () => {
  it("converts a center position + degrees into a top-left frame in radians", () => {
    const f = miroFrame(
      { x: 100, y: 200 },
      { width: 40, height: 20, rotation: 90 },
    );
...
describe("resolveMiroFrames", () => {
  ...
  it("resolves a chain the same way whichever end of it arrives first", () => {
    ...
    const build = (deepestFirst: boolean) => {
      ...
      return resolveMiroFrames(
        deepestFirst ? items.reverse() : items,
      ).frames.get("s1");
    };
...
describe("miroShapeKind", () => {
  it("maps known Miro shape names to slides ShapeKinds", () => {
    ...
    expect(miroShapeKind("round_rectangle")).toEqual({
      kind: "roundRect",
      known: true,
    });
    ...
    expect(miroShapeKind("flow_chart_predefined_process")).toEqual({
      kind: "flowChartPredefinedProcess",
      known: true,
    });
  });
  ...
  it("does not resolve inherited Object.prototype keys from untrusted names", () => {
    expect(miroShapeKind("constructor")).toEqual({
      kind: "rect",
      known: false,
    });
...
describe("miroHtmlToBlocks", () => {
  ...
  it("applies the item-level size, color and alignment Miro keeps outside the HTML", () => {
    ...
    expect(blocks[0].inlines[0].style).toMatchObject({
      fontSize: 10.5,
      color: "#808080",
    });
  });
  ...
  it("lets inline markup win over the item-level base on the axis it names", () => {
    const blocks = miroHtmlToBlocks("<p>a<strong>b</strong></p>", {
      fontSize: 18,
      color: "#f00",
    });
