...
import type { ShapeKind } from "@nautilo/office-slides/node";
 ...
 */
export function miroShapeKind(name: string | undefined): {
  kind: ShapeKind;
  known: boolean;
} {
  const mapped =
    name && Object.prototype.hasOwnProperty.call(SHAPE_MAP, name)
      ? SHAPE_MAP[name]
      : undefined;
  return mapped
    ? { kind: mapped, known: true }
    : { kind: "rect", known: false };
}
