...
import type { Frame } from "@nautilo/office-slides/node";
import {
  pickConnectorSite,
  siteAnchor,
  siteIndexFor,
...
describe("pickConnectorSite — rule 1: explicit relative position", () => {
  it("maps the four cardinal offsets", () => {
    ...
    for (const [position, expected] of cases) {
      expect(pickConnectorSite({ position }, left, right, "rect")).toBe(
        expected,
      );
    }
  });
  ...
  it("falls through to snapTo when the position is malformed", () => {
    ...
    for (const position of malformed) {
      expect(
        pickConnectorSite({ snapTo: "bottom", position }, left, right, "rect"),
      ).toBe(SITE_S);
    }
  });
...
describe("pickConnectorSite — rule 2: snapTo", () => {
  it("maps each explicit side", () => {
    expect(pickConnectorSite({ snapTo: "top" }, left, right, "rect")).toBe(
      SITE_N,
    );
    expect(pickConnectorSite({ snapTo: "right" }, left, right, "rect")).toBe(
      SITE_E,
    );
    expect(pickConnectorSite({ snapTo: "bottom" }, left, right, "rect")).toBe(
      SITE_S,
    );
    expect(pickConnectorSite({ snapTo: "left" }, left, right, "rect")).toBe(
      SITE_W,
    );
  });
  ...
  it('falls through to geometry on "auto"', () => {
    ...
    expect(pickConnectorSite({ snapTo: "auto" }, left, below, "rect")).toBe(
      SITE_S,
    );
  });
  ...
  it("falls through to geometry on an unknown value", () => {
    const below = at(0, 500);
    expect(pickConnectorSite({ snapTo: "diagonal" }, left, below, "rect")).toBe(
      SITE_S,
    );
  });
});
