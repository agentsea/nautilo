...
describe('drawPeerLabel', () => {
  it('draws username text above the cell', () => {
    ...
    const fillTextY = vi.mocked(ctx.fillText).mock.calls[0][2];
  ...
  it('truncates long usernames with ellipsis', () => {
    ...
    const displayedText = vi.mocked(ctx.fillText).mock.calls[0][0];
  ...
  it('stacks labels for peers on the same cell', () => {
    ...
    const y0 = vi.mocked(ctx.fillText).mock.calls[0][2];

    drawPeerLabel(ctx, 'u2', '#4ECDC4', cellRect, port, 1);
    const y1 = vi.mocked(ctx.fillText).mock.calls[1][2];
  ...
  it('clamps label within viewport on right edge', () => {
    ...
    const fillTextX = vi.mocked(ctx.fillText).mock.calls[0][1];
