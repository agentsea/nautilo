...
export class Worksheet {
  ...
  private bindEventListener<K extends keyof HTMLElementEventMap>(
    element: EventTarget,
    type: K,
    handler: (this: typeof element, ev: HTMLElementEventMap[K]) => unknown,
  ...
  private handleDblClickAt(x: number, y: number): void {
    ...
    if (hiddenHit) {
      if (hiddenHit.axis === 'row') {
        void this.sheet!.showRows(hiddenHit.indices).then(() => this.render());
      } else {
        void this.sheet!.showColumns(hiddenHit.indices).then(() => this.render());
      }
      return;
    }
    ...
    if (resizeEdge) {
      void this.autoFitSize(resizeEdge.axis, resizeEdge.index);
      return;
    }
  ...
  private async showFilterPanel(
    ...
    this.filterPanelState = {
      ...
      search: '',
      visibleValueCount: FilterPanelMaxVisibleValues,
      mode,
      condition: {
        ...initialCondition,
  ...
  private syncFilterPanelApplyButtonState(): void {
    ...
    const apply = this.filterPanel.querySelector<HTMLButtonElement>(
  ...
  private syncFilterPanelValuesSelectionState(filteredValues: string[]): void {
    ...
    const selectAll = this.filterPanel.querySelector<HTMLInputElement>(
  ...
  private renderFilterPanel(): void {
    ...
    sortAsc.onclick = () => {
      void this.sheet!.sortFilterByColumn(state.col, 'asc').then((sorted) => {
        if (sorted) {
          this.hideFilterPanel();
          this.render();
        }
    ...
    sortDesc.onclick = () => {
      void this.sheet!.sortFilterByColumn(state.col, 'desc').then((sorted) => {
        if (sorted) {
          this.hideFilterPanel();
          this.render();
        }
    ...
    if (state.mode === 'values') {
      ...
      search.oninput = () => {
        ...
        this.filterPanelState.visibleValueCount = FilterPanelMaxVisibleValues;
        this.renderFilterPanel();
      };
      ...
      const visibleValues = filteredValues.slice(0, state.visibleValueCount);
      ...
      if (filteredValues.length === 0) {
        ...
        for (const value of visibleValues) {
          ...
          checkbox.dataset.wbFilterValue = value;
        ...
        if (visibleValues.length < filteredValues.length) {
          const continuation = document.createElement('div');
          continuation.style.display = 'flex';
          continuation.style.alignItems = 'center';
          continuation.style.justifyContent = 'space-between';
          continuation.style.gap = '8px';
          continuation.style.padding = '6px 8px';
          continuation.style.borderTop = `1px solid ${borderColor}`;
          const count = document.createElement('span');
          count.dataset.wbFilterVisibleCount = 'true';
          count.textContent = `Showing ${visibleValues.length} / ${filteredValues.length}`;
          count.style.fontSize = '11px';
          count.style.opacity = '0.7';
          const showMore = makeButton('Show more');
          showMore.dataset.wbFilterShowMore = 'true';
          showMore.onclick = () => {
            if (!this.filterPanelState) return;
            this.filterPanelState.visibleValueCount +=
              FilterPanelMaxVisibleValues;
            this.renderFilterPanel();
          };
          continuation.append(count, showMore);
          valuesWrap.appendChild(continuation);
        }
      }
    ...
    clear.onclick = () => {
      void this.sheet!.clearColumnFilter(state.col).then(() => {
        this.hideFilterPanel();
        this.render();
      });
    };
    ...
    if (wasSearchFocused && state.mode === 'values') {
      const restoredSearch = this.filterPanel.querySelector<HTMLInputElement>(
  ...
  private startFreezeDrag(
    ...
    this.startMouseDragSession({
      ...
      onComplete: () => {
        ...
        const currentFreeze = this.sheet!.getFreezePane();
        if (axis === 'row') {
          void this.setFreezePane(targetIndex, currentFreeze.frozenCols);
        } else {
          void this.setFreezePane(currentFreeze.frozenRows, targetIndex);
        }
      },
      onCleanup: () => {
  ...
  private async handleEditorKeydown(
    ...
    if (!handled && source === 'formulaBar' && !this.cellInput.isShown()) {
      void this.showCellInput(true, true);
    }
  }
      ...
      },
      {
        ...
        run: async (event) => {
          ...
          try {
            ...
            if (!(error instanceof RangeOperationError)) throw error;
            this.onValidationErrorCallback?.(error.message);
          }
        },
      },
      {
