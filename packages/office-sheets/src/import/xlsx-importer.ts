...
// XLSX worksheets have 16,384 columns (A through XFD). Column declarations may
// validly style any of them even when no cell is populated there.
...
function applyColumns(
  ...
  for (const col of childrenByLocalName(doc, 'col')) {
    ...
    if (
      ...
      min < 1 ||
      rawMax < min ||
      rawMax > XLSX_MAX_COLUMN
    ) {
      throw new Error(
        `Invalid .xlsx column range: min="${col.getAttribute('min') ?? ''}" max="${col.getAttribute('max') ?? ''}".`,
      );
    }
    ...
    const rawStyle = col.getAttribute('style');
    const columnStyle =
      rawStyle !== null
        ? styleTable.resolveCellStyle(Number(rawStyle))
        : undefined;
    for (let index = min; index <= rawMax; index += 1) {
      if (hasCustomWidth) {
      ...
      if (columnStyle) {
        worksheet.colStyles[String(index)] = { ...columnStyle };
      }
    }
  }
  if (hidden.length > 0) {
...
function parseWorksheet(
  ...
  applyColumns(worksheet, doc, styleTable);
