import { ARef, Ref, Range, Ranges, Reference, Sref, Srng } from './types';

/**
 * `isIntersect` returns whether the given Ranges are intersected.
 */
function isIntersect(range1: Range, range2: Range): boolean {
 ...
 */
function* toRefs(range: Range): Generator<Ref> {
  const [a, b] = range;
 ...
 */
function isCollapsedRange(range: Range): boolean {
  const [from, to] = range;
