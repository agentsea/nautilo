...
export type RangeOperationErrorReason = 'selection-too-large';
export type RangeOperation = 'set-range-borders' | 'toggle-checkboxes';

/**
 * A truthful temporary refusal while large range operations still require a
 * compact engine representation. The numeric boundary is debt, not authority.
 */
export class RangeOperationError extends Error {
  readonly name = 'RangeOperationError';

  constructor(
    readonly operation: RangeOperation,
    readonly reason: RangeOperationErrorReason,
    readonly cellCount: number,
    readonly temporaryMaximum: number,
  ) {
    super(
      operation === 'toggle-checkboxes'
        ? `This checkbox selection has ${cellCount.toLocaleString()} cells. Select a smaller range while large checkbox operations are being improved.`
        : `This border selection has ${cellCount.toLocaleString()} cells. Select a smaller range while large border operations are being improved.`,
    );
  }
}
...
export class Sheet {
  ...
  setRowHeight(row: number, height: number): void {
    ...
    void this.store.setDimensionSize('row', row, height);
  }
  ...
  setColumnWidth(col: number, width: number): void {
    ...
    void this.store.setDimensionSize('column', col, width);
  }
   ...
   */
  // Modified by Nautilo: Promise shape is part of the dependency-builder contract.
  ...
  private async buildGlobalDependantsMap(
    ...
    for (const sref of crossSheetFormulaSrefs) {
      ...
      for (const ref of toSrefs(references)) {
        ...
        const qualifiedRef = ref.toUpperCase();
    ...
    while (queue.length > 0) {
      ...
      for (const [localRef, formula] of formulas) {
        const qualifiedSref = `${sheetName}!${localRef}`;
        ...
        for (const ref of toSrefs(references)) {
          if (isCrossSheetRef(ref)) {
            ...
            const qualifiedRef = `${sheetName}!${ref}`;
            addEdge(qualifiedRef, qualifiedSref);
          }
        }
      }
  ...
  async selectAll(): Promise<void> {
    const prev = this.getRangeOrActiveCell();
   ...
   * whole-column checkbox rule beyond the temporary engine boundary is
   * explicitly refused before mutation rather than silently ignored.
  ...
  async toggleCheckboxesInRange(range: Range): Promise<boolean> {
    ...
    for (const rule of this.dataValidations) {
      ...
      for (const rr of rule.ranges) {
        ...
        const intersectionCellCount =
          (ir1 - ir0 + 1) * (ic1 - ic0 + 1);
        if (intersectionCellCount > MaxCheckboxToggleCells) {
          throw new RangeOperationError(
            'toggle-checkboxes',
            'selection-too-large',
            intersectionCellCount,
            MaxCheckboxToggleCells,
          );
        }
        ...
        if (candidates.size > MaxCheckboxToggleCells) {
          throw new RangeOperationError(
            'toggle-checkboxes',
            'selection-too-large',
            candidates.size,
