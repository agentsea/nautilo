...
import { RangeOperationError } from '../../src/model/worksheet/sheet';
...
type GridSheetMock = {
  ...
  getDataValidationAt: ReturnType<typeof vi.fn>;
  getRangeOrActiveCell: ReturnType<typeof vi.fn>;
  toggleCheckboxesInRange: ReturnType<typeof vi.fn>;
};
...
type GridContext = {
  ...
  onValidationErrorCallback?: (message: string) => void;
};
...
const createContext = (): GridContext => ({
  sheet: {
    ...
    getDataValidationAt: vi.fn().mockReturnValue(undefined),
    getRangeOrActiveCell: vi.fn().mockReturnValue([
      { r: 1, c: 1 },
      { r: 1, c: 1 },
    ]),
    toggleCheckboxesInRange: vi.fn().mockResolvedValue(false),
  },
  readOnly: false,
  render: vi.fn(),
...
describe('Worksheet grid keymap', () => {
  it('surfaces a large checkbox range refusal through validation feedback', async () => {
    const ctx = createContext();
    const onValidationError = vi.fn();
    ctx.onValidationErrorCallback = onValidationError;
    ctx.sheet.getDataValidationAt.mockReturnValue({
      id: 'checkboxes',
      kind: 'checkbox',
      ranges: [],
    });
