...
import type { FunctionInfo } from '../../src/formula/function-catalog';
...
describe('FunctionBrowser', () => {
  ...
  it('updates highlighted row on hover without rebuilding list rows', () => {
    ...
    const rows = browser
      .getContainer()
      .querySelectorAll<HTMLElement>('[data-func-name]');
  ...
  it('inserts selected function on Enter key', () => {
    const onInsert = vi.fn<(info: FunctionInfo) => void>();
  ...
  it('inserts selected function on row pointer down', () => {
    ...
    const row = browser
      .getContainer()
      .querySelector<HTMLElement>('[data-func-name="SUM"]');
