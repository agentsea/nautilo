// Modified by Nautilo: verify actual header paint colors in both grid modes.
import { describe, expect, it } from 'vitest';
import { getThemeColor, type ThemeKey } from '../../src/view/theme';
...
const renderHeader = (
  GridCanvas.prototype as unknown as {
    renderHeader: (...args: unknown[]) => void;
  }
).renderHeader;

describe('GridCanvas header readability', () => {
  for (const theme of ['light', 'dark'] as const) {
    for (const [selected, fullSelected] of [[false, false], [true, false], [true, true]]) {
      it(`paints ${theme} header selected=${selected} fullSelected=${fullSelected}`, () => {
        const paints: { operation: string; color: string }[] = [];
        const ctx = {
          fillStyle: '',
          fillRect() { paints.push({ operation: 'background', color: this.fillStyle }); },
          strokeRect() {},
          fillText() { paints.push({ operation: 'text', color: this.fillStyle }); },
        };
        renderHeader.call({ getThemeColor: (key: ThemeKey) => getThemeColor(theme, key) },
          ctx, 0, 0, 80, 24, 'A', selected, fullSelected);
        const active = selected && !fullSelected;
        expect(paints).toEqual([
          { operation: 'background', color: getThemeColor(theme, fullSelected ? 'headerSelectedBGColor' : active ? 'headerActiveBGColor' : 'headerBGColor') },
          { operation: 'text', color: active ? '#000000' : getThemeColor(theme, 'cellTextColor') },
        ]);
        if (active) expect(paints[1].color).not.toBe('#FFFFFF');
      });
    }
  }
});
