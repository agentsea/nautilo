...
describe('xlsx style import', () => {
  ...
  it('preserves a valid whole-sheet column width span', async () => {
    const sheetXml = `<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
      <cols>
    ...
    expect(Object.keys(sheet.worksheet.colWidths)).toHaveLength(16_384);
    expect(sheet.worksheet.colWidths['1']).toBeGreaterThan(0);
    expect(sheet.worksheet.colWidths['16384']).toBeGreaterThan(0);
  });

  it('preserves distant column formatting on an otherwise empty sheet', async () => {
    const sheetXml = `<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
      <cols>
        <col min="1000" max="1002" width="21" customWidth="1" hidden="1" style="3"/>
      </cols>
      <sheetData/>
    </worksheet>`;
    const [sheet] = await importXlsxWorkbook(
    ...
    expect(sheet.worksheet.colWidths['1000']).toBe(152);
    expect(sheet.worksheet.colWidths['1002']).toBe(152);
    expect(sheet.worksheet.hiddenColumns).toEqual([1000, 1001, 1002]);
    expect(sheet.worksheet.colStyles['1000']?.nf).toBe('percent');
    expect(sheet.worksheet.colStyles['1002']?.nf).toBe('percent');
  });
  ...
  it.each([
    ...
    ['non-numeric', 'first', '2'],
    ['fractional', '1.5', '2'],
    ['beyond XFD', '16384', '16385'],
  ])('rejects an invalid %s column range', async (_name, min, max) => {
    const sheetXml = `<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
      <cols><col min="${min}" max="${max}" width="12" customWidth="1"/></cols>
      <sheetData/>
    </worksheet>`;

    await expect(
      importXlsxWorkbook(await buildStyledWorkbook(sheetXml)),
    ).rejects.toThrow(`Invalid .xlsx column range: min="${min}" max="${max}".`);
  });
