...
const TEST_MAX_CELLS = 40;
const limitedImport = (rows: ReadonlyArray<ReadonlyArray<unknown>>) =>
  importTable(rows, { maxCells: TEST_MAX_CELLS });
...
describe('cell budget (boundary value analysis)', () => {
  // The explicit caller limit is exact and checked before a row is written, so the
  // interesting inputs are the three around it plus the row that straddles it.
  const COLS = 10;
  const wholeRows = TEST_MAX_CELLS / COLS;

  it('imports a table exactly at the explicit cell limit untruncated', () => {
    const result = limitedImport(table(COLS, wholeRows));
  ...
  it('imports one cell under the budget untruncated', () => {
    ...
    expect(limitedImport(rows).truncated).toBe(false);
  });

  it('truncates one row over the budget', () => {
    const result = limitedImport(table(COLS, wholeRows + 1));
  ...
  it('counts cells, not rows, so a narrow table gets proportionally more rows', () => {
    const narrow = limitedImport(table(2, TEST_MAX_CELLS));
    expect(narrow.rowCount).toBe(TEST_MAX_CELLS / 2);
  ...
  it('charges only cells it wrote, so a sparse table gets far more rows', () => {
    ...
    const result = limitedImport(sparse);
    expect(result.truncated).toBe(false);
  ...
  it('spends the whole budget on a sparse table rather than stopping short', () => {
    ...
    const sparse = Array.from({ length: TEST_MAX_CELLS + 1 }, () =>
      Array.from({ length: COLS }, (_, c) => (c === 0 ? 'x' : '')),
    );
    const result = limitedImport(sparse);
    expect(result.cellCount).toBe(TEST_MAX_CELLS);
    expect(result.truncated).toBe(true);
  });
  ...
  it('does not let a trailing blank row truncate an exact-budget import', () => {
    ...
    // row's field count made a file exactly at its caller limit report "Only
    // the first N rows were imported" while holding every row it had.
    const rows = table(COLS, wholeRows);
    rows.push(['']);
    const result = limitedImport(rows);
    expect(result.cellCount).toBe(TEST_MAX_CELLS);
    expect(result.rowCount).toBe(wholeRows);
    expect(result.truncated).toBe(false);
  });

  it('imports more than 40,000 cells completely by default', () => {
    const result = importTable(table(1, 40_001));
    expect(result.cellCount).toBe(40_001);
    expect(result.rowCount).toBe(40_001);
    expect(result.truncated).toBe(false);
  });
});
...
describe('error paths', () => {
  ...
  it('identifies the explicit caller policy when the first row cannot fit', () => {
    ...
    const width = TEST_MAX_CELLS + 1;
    expect(() =>
      importTable(table(width, 1), { maxCells: TEST_MAX_CELLS }),
    ).toThrow(`This file exceeds the caller cell limit (${TEST_MAX_CELLS}).`);
  });
});

describe('TableWriter — streaming contract', () => {
  it('returns true while the budget holds and false once it is exhausted', () => {
    const writer = createTableWriter({ maxCells: TEST_MAX_CELLS });
    const row = Array.from({ length: TEST_MAX_CELLS }, () => 'x');
  ...
  it('writes nothing for the row that returned false', () => {
    const writer = createTableWriter({ maxCells: TEST_MAX_CELLS });
    writer.push(Array.from({ length: TEST_MAX_CELLS }, () => 'x'));
  ...
  it('agrees cell-for-cell with importTable on the same input', () => {
    ...
    const writer = createTableWriter({ maxCells: 5 });
    for (const row of rows) writer.push(row);
    const streamed = writer.finish();
    const bulk = importTable(rows, { maxCells: 5 });
  ...
  it('cannot resume with a smaller row after rejecting a wide row', () => {
    const writer = createTableWriter({ maxCells: 3 });
    expect(writer.push(['kept'])).toBe(true);
    expect(writer.push(['too', 'wide', 'now'])).toBe(false);
    expect(writer.push(['would-fit'])).toBe(false);
