...
import {
  RangeOperationError,
  Sheet,
} from '../../src/model/worksheet/sheet';
...
describe('Sheet.Formatting', () => {
  ...
  it('explicitly refuses an unsupported large border operation before writes', async () => {
    const store = new MemStore();
    const sheet = new Sheet(store);
    sheet.selectStart({ r: 1, c: 1 });
    sheet.selectEnd({ r: 501, c: 100 });
    const setSpy = vi.spyOn(store, 'set');

    const operation = sheet.setRangeBorders('all');
    await expect(operation).rejects.toBeInstanceOf(RangeOperationError);
    await expect(operation).rejects.toMatchObject({
      name: 'RangeOperationError',
      operation: 'set-range-borders',
      reason: 'selection-too-large',
      cellCount: 50100,
      temporaryMaximum: 50000,
    });
    expect(setSpy).not.toHaveBeenCalled();
    expect(await store.getRangeStyles()).toEqual([]);
  });
