...
describe('Pivot table end-to-end', () => {
  ...
  describe('single row field + SUM value', () => {
    ...
    it('materializes to styled cells', () => {
      ...
      expect(grid.get(toSref({ r: 1, c: 2 }))?.s?.b).toBe(true);
      // Row header should be bold
      expect(cellValue(grid, 2, 1)).toBe('East');
      expect(grid.get(toSref({ r: 2, c: 1 }))?.s?.b).toBe(true);
      // Value cell should not be bold
      expect(cellValue(grid, 2, 2)).toBe('3900');
      const valueStyle = grid.get(toSref({ r: 2, c: 2 }))?.s;
      ...
      expect(grid.get(toSref({ r: 4, c: 1 }))?.s?.b).toBe(true);
      expect(grid.get(toSref({ r: 4, c: 1 }))?.s?.bg).toBeUndefined();
    });
  });
