// Modified by Nautilo: keep harness-only implementation private.
...
function createTestSheet(): Promise<{ sheet: Sheet; store: MemStore }> {
  const store = new MemStore();
  return Promise.resolve({ sheet: new Sheet(store), store });
}

async function initializeSheetState(sheet: Sheet): Promise<void> {
...
async function applyConcurrencyOp(
  sheet: Sheet,
  op: ConcurrencyOp,
): Promise<void> {
  switch (op.kind) {
...
async function captureConcurrencySnapshot(
  sheet: Sheet,
  store: MemStore,
