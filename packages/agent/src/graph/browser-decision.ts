...
export function browserDecisionHandoffContent(reason: string, target?: ConnectedBrowserDecisionTarget, decision?: BrowserDecisionState): string {
  ...
  const visualEvidence = decision?.observation?.visual === undefined ? ""
    : "\nDelegated screenshot pixels were processed locally and not retained. The latest browser_screenshot result has the extracted visual state only. If you need to inspect pixels, take a fresh ordinary browser_screenshot without decisionPlan before acting.";
...
function browserDecisionHandoffInstruction(reason: string, target?: ConnectedBrowserDecisionTarget, visual = false): string {
  ...
  if (reason === "needs_visual_evidence" && visual) return "Routine screenshot control returned because the extracted visual state did not identify a required fact or target. Inspect the visual snapshot to determine what information is missing, then revise the remaining plan or use ordinary browser controls. Do not repeatedly delegate against unchanged evidence as a substitute for resolving the missing information.";
