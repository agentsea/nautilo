/**
 * D429 Phase 7.1 — bounded remote model-catalog fetch with signed pointer.
 *
 ...
 * is fixed at construction time. In pointer mode it fetches the configured
 * pointer URL plus the immutable manifest URL derived from that pointer's own
 * directory and a validated `catalogVersion` — never a pointer-provided path.
 * No caller-supplied URL is ever accepted.
 *
 * Verification order (all failures retain the prior known-good or bootstrap):
 ...
 *   5. strict manifest schema (ModelCatalogSchema);
 *   6. pointer and manifest catalogVersion match;
 *   7. post-D560 catalogs preserve security-research Task routing metadata;
 *   8. atomic snapshot replace.
 *
 * Transport guarantees (mirrors the shipped D416 explainer loader posture):
 ...
 *   - local checked-in fallback so chat is never unavailable;
 *   - truthful, non-secret provenance in a narrowly typed result.
 *
 * This module is intentionally NOT wired into any sync catalog consumer
 * directly; the runtime seam (`./runtime-catalog`) wraps it and exposes a
 * synchronous atomic snapshot. Inject `fetchImpl` and `now` in tests; unit
 * tests never use the network.
 */
import {
  ARTIFACT_SHA256_PATTERN,
  ModelCatalogReleasePointerSchema,
  ModelCatalogSchema,
  canonicalModelCatalogSigningPayload,
  immutableModelCatalogFilename,
  type ModelCatalog,
  type ModelCatalogResultSource,
} from "@nautilo/types";
import { createHash } from "node:crypto";
import { verify as ed25519Verify, createPublicKey } from "node:crypto";
import { localModelCatalog } from "./catalog";
import { getTrustedModelCatalogPublicKey } from "./trusted-keys";

/** Where a served catalog came from, in narrowly typed terms. */
export type RemoteModelCatalogSource = ModelCatalogResultSource;

/** Truthful, bounded metadata about a served catalog. */
export interface RemoteModelCatalogResult {
  readonly catalog: ModelCatalog;
  readonly source: RemoteModelCatalogSource;
 ...
 */
export interface RemoteModelCatalogConfig {
  /**
   * Allowlisted official release-pointer URL (e.g. `models/latest.json`). No
...
export interface RemoteModelCatalogLoader {
  /** Resolve the best available catalog with truthful source metadata. */
  get(): Promise<RemoteModelCatalogResult>;
  /**
   ...
   */
  refresh(): Promise<RemoteModelCatalogResult | void>;
...
const SECURITY_RESEARCH_PREFERENCE_REQUIRED_SINCE = [2026, 8, 28, 1] as const;

function catalogVersionParts(version: string): readonly number[] {
  return version.split(".").map(Number);
}

function catalogVersionAtLeast(
  version: string,
  floor: readonly number[],
): boolean {
  const parts = catalogVersionParts(version);
  for (let index = 0; index < floor.length; index += 1) {
    const delta = (parts[index] ?? 0) - (floor[index] ?? 0);
    if (delta !== 0) return delta > 0;
  }
  return true;
}

function assertRequiredTaskPreferences(catalog: ModelCatalog): void {
  if (!catalogVersionAtLeast(
    catalog.catalogVersion,
    SECURITY_RESEARCH_PREFERENCE_REQUIRED_SINCE,
  )) return;
  const hasSecurityResearchRoute = catalog.entries.some((entry) =>
    "taskPreferences" in entry
    && entry.taskPreferences?.includes("security_research")
  );
  if (!hasSecurityResearchRoute) {
    throw new Error(
      "official model catalog omitted required security_research Task routing metadata",
    );
  }
}
...
function assertAllowedOrigin(url: string, allowedHosts: string[]): URL {
  ...
  try {
    ...
    throw new Error("official model catalog URL is not a valid URL");
  }
  if (parsed.protocol !== "https:") {
    throw new Error("official model catalog URL must use the https: scheme");
  }
  if (parsed.username || parsed.password) {
    throw new Error("official model catalog URL must not carry credentials");
  }
  if (parsed.hash) {
    throw new Error("official model catalog URL must not carry a fragment");
  }
  if (parsed.search) {
    throw new Error("official model catalog URL must not carry a query string");
  }
  const host = parsed.hostname.toLowerCase();
  const forbiddenIp = forbiddenIpLiteral(host);
  if (forbiddenIp) {
    throw new Error(`official model catalog host "${host}" is a forbidden IP literal (${forbiddenIp})`);
  }
  if (!allowedHosts.includes(host)) {
    throw new Error(`official model catalog host "${host}" is not in the allowlist`);
  }
  return parsed;
}
...
async function readBoundedText(res: Response, maxBytes: number): Promise<string> {
  if (res.body === null) {
    throw new Error("official model catalog response has no body");
  }
  ...
  try {
    while (true) {
      ...
      if (received > maxBytes) {
        throw new Error(
          `official model catalog response exceeded byte limit (${received} > ${maxBytes})`,
        );
      }
      try {
        text += decoder.decode(value, { stream: true });
      } catch {
        throw new Error("official model catalog response is not valid UTF-8");
      }
    }
    try {
      text += decoder.decode();
...
function verifyPointerSignature(
  ...
  const publicKeyB64 = trustedKeys[pointer.signingKeyId] ?? getTrustedModelCatalogPublicKey(pointer.signingKeyId);
  if (!publicKeyB64) {
  ...
  const payload = Buffer.from(
    canonicalModelCatalogSigningPayload(pointer.catalogVersion, pointer.artifactSha256),
    "utf8",
  );
...
export function createRemoteModelCatalogLoader(
  ...
  let refreshInflight: Promise<RemoteModelCatalogResult> | null = null;
  ...
  async function fetchStrictText(url: string): Promise<string> {
    ...
    const timeoutPromise = new Promise<never>((_, reject) => {
      timer = setTimeout(() => {
        controller.abort();
        reject(new Error(`official model catalog fetch timed out after ${timeoutMs}ms`));
      }, timeoutMs);
    });
    const work = async (): Promise<string> => {
      ...
      if (res.status !== 200) {
        throw new Error(`official model catalog fetch failed status=${res.status}`);
      }
      const contentType = res.headers.get("content-type");
      if (!isJsonContentType(contentType)) {
        throw new Error(
          `official model catalog response has invalid Content-Type (${contentType ?? "missing"})`,
        );
      }
      ...
      if (contentLengthHeader) {
        ...
        if (Number.isFinite(declared) && declared > maxBytes) {
          throw new Error(
            `official model catalog Content-Length exceeds byte limit (${declared} > ${maxBytes})`,
          );
        }
      }
  ...
  function parseJson(text: string): unknown {
    try {
      ...
      throw new Error("official model catalog response is not valid JSON");
    }
  }
  ...
  function deriveManifestUrl(pointerUrl: string, catalogVersion: string): string {
    ...
    if (lastSlash < 0) {
      throw new Error("official model catalog pointer URL must reside in a directory");
    }
    base.pathname = path.slice(0, lastSlash + 1) + immutableModelCatalogFilename(catalogVersion);
    const derived = base.toString();
    const validated = assertAllowedOrigin(derived, allowedHosts);
    if (validated.origin !== base.origin) {
      throw new Error("official model catalog manifest URL must be same-origin as the pointer");
    }
    return derived;
  }
  ...
  async function performFetch(now: number): Promise<CacheEntry> {
    if (originUrl === null) {
      throw new Error("remote model catalog is disabled (no pointer URL configured)");
    }
    ...
    try {
      pointer = ModelCatalogReleasePointerSchema.parse(parseJson(pointerText));
    } catch {
      throw new Error("official model catalog pointer is not a valid signed release pointer");
    }
    ...
    if (artifactSha256 !== pointer.artifactSha256) {
      throw new Error("official model catalog immutable artifact SHA-256 mismatch");
    }
    // 5. Strict manifest schema.
    const catalog = ModelCatalogSchema.parse(parseJson(manifestText));
    // 6. Pointer and manifest catalogVersion must match.
    if (catalog.catalogVersion !== pointer.catalogVersion) {
      throw new Error("official model catalog pointer and manifest catalogVersion do not match");
    }
    // D560 made this operational metadata part of the security-scan contract.
    // A newer release that drops every route would otherwise activate cleanly
    // and silently send GLM-designated scans to the Agent's unrelated base
    // model. Retain the prior remote LKG (or checked-in D560 fallback) instead.
    assertRequiredTaskPreferences(catalog);
    return {
      catalog,
      fetchedAt: now,
      expiresAt: now + ttlMs,
  ...
  async function refresh(): Promise<RemoteModelCatalogResult> {
    ...
    const promise = (async () => {
      try {
        ...
        // A fetch failure leaves the existing last-known-good cache in place;
        // return that LKG truthfully, or the checked-in fallback when no remote
        // payload has ever activated. Do not make a second network attempt.
        const cached = cacheEntry;
        if (cached) return fromCache(cached, "remote-stale");
        return localFallback(`official model catalog unavailable: ${errorMessage(error)}`);
      }
    })();
  ...
  function localFallback(reason: string): RemoteModelCatalogResult {
    return {
  ...
  async function get(): Promise<RemoteModelCatalogResult> {
    if (originUrl === null) {
      return localFallback("remote model catalog disabled — no pointer URL configured");
    }
    const now = clock();
    const cached = cacheEntry;
