...
export function parseFrontmatter(raw: string): {
  ...
  if (!raw.startsWith("---\n")) {
    throw new Error("Bundled skill markdown must start with a frontmatter block (---)");
  }

  const closingIndex = raw.indexOf("\n---\n", 4);
  if (closingIndex === -1) {
    throw new Error("Bundled skill markdown has a malformed or unclosed frontmatter block");
  }
  ...
  for (const key of REQUIRED_FRONTMATTER_KEYS) {
    if (frontmatter[key] === undefined) {
      throw new Error(`Bundled skill frontmatter is missing required key: ${key}`);
    }
  }
