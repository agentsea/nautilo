...
import { createAnthropic, createFireworks, createOpenAI } from "../../src/providers/factory";
...
beforeAll(async () => {
  ...
  const catalog = ModelCatalogSchema.parse({ ...baseline, entries: baseline.entries.map((entry) =>
    entry.id === "openrouter:deepseek/deepseek-v4-pro-0813" ? { ...entry, controls: { reasoning: {
      levels: ["low", "medium", "high", "max"], defaultLevel: "high", canDisable: true, mandatory: false,
...
describe("createAnthropic reasoning output (uniform adaptive — D331)", () => {
  ...
  it("skips thinking when maxTokens is too small (health-check guard)", async () => {
    const llm = await createAnthropic({
      modelId: "anthropic:claude-opus-4-8",
      apiKey: "test-key",
      maxTokens: 5,
...
describe("OpenAI-compatible reasoning output (D331 provider strengthening)", () => {
  it("does not pass direct OpenAI reasoning_effort on Chat Completions", async () => {
    ...
    // tools are bound for GPT-5.5. D334 owns the Responses API migration.
  ...
  it("uses Responses API with reasoning.effort when explicitly opted in (D334)", async () => {
    const llm = await createOpenAI({
      modelId: "openai:gpt-5.5-2026-04-23",
      apiKey: "test-key",
      maxTokens: 8192,
  ...
  for (const [provider, create] of [
    ...
    ["OpenRouter", () => createOpenAI({
      modelId: "openrouter:deepseek/deepseek-v4-pro-0813", apiKey: "test-key", baseUrl: "https://openrouter.ai/api/v1",
      maxTokens: 8192, reasoningOutput: true, reasoningEffort: "xhigh",
    })],
    ["Venice", () => createOpenAI({
      modelId: "venice:zai-org-glm-5-2", apiKey: "test-key", baseUrl: "https://api.venice.ai/api/v1",
      maxTokens: 8192, reasoningOutput: true, reasoningEffort: "minimal",
    })],
    ["Fireworks", () => createFireworks({
      modelId: "fireworks:accounts/fireworks/models/glm-5p2", apiKey: "test-key", maxTokens: 8192,
  ...
  it("stays on Chat Completions when useOpenAIResponsesApi is false (D334)", async () => {
    const llm = await createOpenAI({
      modelId: "openai:gpt-5.5-2026-04-23",
      apiKey: "test-key",
      maxTokens: 8192,
