...
export const MODEL_LAB_USAGE = "Use --help, --list, or --live --case NAME --model CATALOG_ID --entry router|controller [--controller-model CATALOG_ID] [--reasoning off|provider-default] --max-requests N --max-output-tokens N --duration-ms N. An optional controller model handles decision-model handoffs through the same executor. Decision models use controller entry and provider-default reasoning; output-token budget applies only to generative models. Live incurs provider charges on synthetic data only. Budgets bound model invocations/output/duration, NOT an exact dollar amount or provider-internal retries. No GUI access. Existing provider credentials are resolved by Nautilo, never CLI arguments.";
...
export function parseModelLabArguments(args: string[]) {
  ...
  return { kind: "live" as const, scenario, modelId: values.model, controllerModelId: values["controller-model"] ?? null,
    entry, reasoning, maxRequests, maxOutputTokens, durationMs };
}
  ...
  try {
    ...
    try {
      ...
      const report = await runControllerLab({ ...options, signal: controller.signal,
        ...
        decide: model ? createModelDecider(model, measurements) : () => Promise.reject(new Error("unexpected_generative_call")),
      ...
      console.log(JSON.stringify({ ...report, modelId: options.modelId, controllerModelId: options.controllerModelId, reasoning: options.reasoning, catalogue: catalogue.provenance,
