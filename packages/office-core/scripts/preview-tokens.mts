...
async function handle(req: Request) {
  ...
  try {
    ...
    // Modified by Nautilo: imported scratch modules are typed at this boundary.
    const load = async <K extends keyof TokenSources>(
      n: K,
    ): Promise<Record<K, TokenSources[K]>> =>
      (await import(pathToFileURL(join(dir, `${n}.ts`)).href)) as unknown as Record<
        K,
        TokenSources[K]
      >;
