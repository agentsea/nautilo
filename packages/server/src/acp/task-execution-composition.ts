...
import { createHermesAcpTaskExecutionRouteSelector, createHermesAcpTaskHarnessExecutionRouteRegistration, type HermesAcpTaskExecutionDeps } from "./task-execution";
import { projectHermesAcpRoomOutput } from "./room-output";
...
export function createHermesAcpTaskExecutionComposition(
  db: DirectDatabase,
  ...
  const reader: HermesAcpTaskExecutionDeps["tasks"] = { async getTask(taskId) {
  ...
  const facts: HermesAcpTaskExecutionDeps["facts"] = {
  ...
  const lifecycleDeps: HermesAcpTaskRunLifecycleDeps = {
    ...
    reportBack: {
      ...
      async fail(input) { if (input.code !== ACP_EXECUTION_FAILED) throw new Error("ACP_REPORT_BACK_INVALID"); const args = { taskId: input.taskId, runId: input.taskRunId, scheduleKind: input.scheduleKind, error: ACP_EXECUTION_FAILED, failureResultText: SAFE_DELEGATED_TASK_FAILURE_RESULT } as const; try { await reportBack.fail({ db }, args); } catch { await reportBack.fail({ db }, args); } },
    },
  };
...
export function createHermesAcpTaskExecutionRouteRegistration(
  db: DirectDatabase,
  relay: HermesAcpTaskExecutionDeps["relay"],
): TaskHarnessExecutionRouteRegistration {
  const { reader, facts, taskRuns } = createHermesAcpTaskExecutionComposition(db);
  return createHermesAcpTaskHarnessExecutionRouteRegistration(() => createHermesAcpTaskExecutionRouteSelector({ tasks: reader, facts, relay, taskRuns, projector: { project: (event) => projectHermesAcpRoomOutput(event, { ownerId: event.scope.binding.ownerId, taskId: event.scope.binding.taskId, taskRunId: event.scope.binding.taskRunId }) }, diagnostics: { onFailureStage: (stage) => log(`[hermes-acp] execution_failure_stage=${stage}`) } }));
}
