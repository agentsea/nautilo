/**
 * D452 C2 — real durable Task dispatch through the generic harness router and
 * the v14 in-memory ACP transport.  This intentionally owns its small DB seed:
...
import { createHermesAcpTaskExecutionRouteRegistration } from "../../src/acp/task-execution-composition";
...
const RELAY_ID = "d452-hermes-relay";
const DESKTOP_SESSION_ID = "d452-hermes-desktop";
const PAIRING_GENERATION = "d452-hermes-pair";
...
async function seedFixture(label: string): Promise<Fixture> {
  ...
  const [user] = await db.insert(users).values({
    name: `D452 ${label}`,
    email: `d452-${label}-${suffix}@test.local`,
    handle: `d452${suffix}`.slice(0, 48),
    externalId: `d452-${suffix}`,
  }).returning({ id: users.id });
  if (!user) throw new Error("D452 fixture user insert failed");
  await seedTrustPersonal(user.id, `D452 ${label}`);
  const [userActor] = await db.select({ id: actors.id }).from(actors)
    .where(and(eq(actors.ownerId, user.id), eq(actors.kind, "user"))).limit(1);
  if (!userActor) throw new Error("D452 fixture user actor missing");
  const [agent] = await db.insert(agents).values({ handle: `d452-agent-${suffix}` }).returning({ id: agents.id });
  if (!agent) throw new Error("D452 fixture agent insert failed");
  const [agentActor] = await db.insert(actors).values({
    ownerId: user.id,
    displayName: "D452 Hermes agent",
    trustState: "verified",
    kind: "agent",
    agentId: agent.id,
  }).returning({ id: actors.id });
  if (!agentActor) throw new Error("D452 fixture agent actor insert failed");
  const [namespace] = await db.insert(namespaces).values({
    scope: "private",
    label: `d452-hermes-${suffix}`,
  }).returning({ id: namespaces.id });
  if (!namespace) throw new Error("D452 fixture namespace insert failed");
  const [room] = await db.insert(rooms).values({
    ownerId: user.id,
    type: "private",
    kind: "private",
    label: `D452 Hermes ${label}`,
    graphThreadId: `room:d452:${suffix}`,
    namespaceId: namespace.id,
    humanActorIds: [userActor.id],
    createdBy: userActor.id,
  }).returning({ id: rooms.id });
  if (!room) throw new Error("D452 fixture room insert failed");
...
function hermesMetadata(readiness: NonNullable<ReturnType<InstanceType<typeof InMemoryRelayRegistry>["getAcpSession"]>>) {
  return {
    execution: {
      version: 1,
      ...
      source: "genie",
      readiness: {
        relayId: readiness.relayId,
        relaySessionId: readiness.relaySessionId,
        pairingGenerationRef: readiness.pairingGenerationRef,
        desktopSessionId: readiness.desktopSessionId,
        selectedProtocolVersion: readiness.selectedProtocolVersion,
        capabilityRevision: readiness.capabilityRevision,
      },
    },
  } as const;
}
...
function realJobManager(): JobManager {
  return new JobManager({
    ...
    persist: async (payload) => {
      ...
      if (!job) throw new Error("D452 Job.persist returned no row");
      return job.id;
    },
...
async function eventually<T>(description: string, read: () => Promise<T | undefined>): Promise<T> {
  ...
  for (;;) {
    ...
    if (Date.now() >= deadline) throw new Error(`D452 timed out waiting for ${description}`);
...
async function registerHermesTransport(
  fixture: Fixture,
  mode: TransportMode,
  timeline: string[] = [],
  ...
  await registry.register(RELAY_ID, fixture.userId, capabilities, (command) => {
    ...
    if (command.type === "relay:acp-readiness") {
      expect(accept({ type: "relay:acp-readiness-result", requestId: command.requestId, registrationId: "hermes-acp", scope: command.scope, state: "ready" })).toEqual({ ok: true });
      return;
    }
    if (command.type === "relay:acp-prepare") {
      expect(accept({ type: "relay:acp-prepared", requestId: command.requestId, registrationId: "hermes-acp", scope: command.scope, binding: command.binding, workspace: { workspaceReceiptId: "d452-receipt", workspaceRevision: "d452-revision", workspaceFingerprint: "d452-fingerprint", workspaceExpiresAt: "2035-01-01T00:00:00.000Z" } })).toEqual({ ok: true });
      return;
    }
    if (command.type !== "relay:acp-start") return;
    const process = { connectionId: "d452-connection", processGeneration: 1, acpSessionId: "d452-acp", turnGeneration: 1, turnRef: "d452-turn" } as const;
    expect(accept({ type: "relay:acp-started", registrationId: "hermes-acp", scope: command.scope, process, capabilities: { requests: "unsupported" }, eventId: "started", eventSequence: 1 })).toEqual({ ok: true });
    if (mode === "failed") {
      timeline.push("failed-terminal-received");
      expect(accept({ type: "relay:acp-terminal", registrationId: "hermes-acp", scope: command.scope, process, status: "failed", code: "upstream_failure", eventId: "failed", eventSequence: 2 })).toEqual({ ok: true });
      return;
    }
    expect(accept({ type: "relay:acp-semantic", registrationId: "hermes-acp", scope: command.scope, process, capabilities: { requests: "unsupported" }, eventId: "delta", eventSequence: 2, payload: { kind: "output_delta", vendorItemId: "d452-delta", text: "Inspecting task" } })).toEqual({ ok: true });
    expect(accept({ type: "relay:acp-semantic", registrationId: "hermes-acp", scope: command.scope, process, capabilities: { requests: "unsupported" }, eventId: "command", eventSequence: 3, payload: { kind: "command_summary", vendorItemId: "d452-command", commands: [{ summary: "Checked the workspace", status: "completed" }] } })).toEqual({ ok: true });
    expect(accept({ type: "relay:acp-semantic", registrationId: "hermes-acp", scope: command.scope, process, capabilities: { requests: "unsupported" }, eventId: "candidate", eventSequence: 4, payload: { kind: "assistant_completed", vendorItemId: "d452-final", text: "Hermes completed the requested work." } })).toEqual({ ok: true });
    expect(accept({ type: "relay:acp-terminal", registrationId: "hermes-acp", scope: command.scope, process, status: "completed", eventId: "completed", eventSequence: 5 })).toEqual({ ok: true });
  }, 14, DESKTOP_SESSION_ID, 7, PAIRING_GENERATION);
...
async function registerStartDisconnectingHermesTransport(
  fixture: Fixture,
): Promise<{
  ...
  await registry.register(RELAY_ID, fixture.userId, capabilities, (command) => {
    ...
    if (command.type === "relay:acp-prepare") {
      expect(accept({ type: "relay:acp-prepared", requestId: command.requestId, registrationId: "hermes-acp", scope: command.scope, binding: command.binding, workspace: { workspaceReceiptId: "d452-disconnect-receipt", workspaceRevision: "d452-disconnect-revision", workspaceFingerprint: "d452-disconnect-fingerprint", workspaceExpiresAt: "2035-01-01T00:00:00.000Z" } })).toEqual({ ok: true });
      return;
    }
    if (command.type !== "relay:acp-start") return;
    const process = { connectionId: "d452-disconnect-connection", processGeneration: 1, acpSessionId: "d452-disconnect-acp", turnGeneration: 1, turnRef: "d452-disconnect-turn" } as const;
    active = { scope: command.scope, process };
    expect(accept({ type: "relay:acp-started", registrationId: "hermes-acp", scope: command.scope, process, capabilities: { requests: "unsupported" }, eventId: "disconnect-started", eventSequence: 1 })).toEqual({ ok: true });
...
/**
 * A deterministic v14 Hermes peer which deliberately holds each terminal
 ...
 */
async function registerQueuedHermesTransport(
  fixture: Fixture,
  timeline: string[],
): Promise<{
  registry: InMemoryRelayRegistry;
  ...
  await registry.register(RELAY_ID, fixture.userId, capabilities, (command) => {
    ...
    if (command.type === "relay:acp-prepare") {
      expect(accept({ type: "relay:acp-prepared", requestId: command.requestId, registrationId: "hermes-acp", scope: command.scope, binding: command.binding, workspace: { workspaceReceiptId: "d452-queued-receipt", workspaceRevision: "d452-queued-revision", workspaceFingerprint: "d452-queued-fingerprint", workspaceExpiresAt: "2035-01-01T00:00:00.000Z" } })).toEqual({ ok: true });
      return;
    }
    if (command.type !== "relay:acp-start") return;
    const ordinal = starts.length + 1;
    const process = {
      connectionId: `d452-queued-connection-${ordinal}`,
      processGeneration: ordinal,
      acpSessionId: `d452-queued-acp-${ordinal}`,
      turnGeneration: ordinal,
      turnRef: `d452-queued-turn-${ordinal}`,
    } as const;
    starts.push(command);
    ...
    expect(accept({ type: "relay:acp-started", registrationId: "hermes-acp", scope: command.scope, process, capabilities: { requests: "unsupported" }, eventId: startedEventId, eventSequence: 1 })).toEqual({ ok: true });
    expect(accept({ type: "relay:acp-semantic", registrationId: "hermes-acp", scope: command.scope, process, capabilities: { requests: "unsupported" }, eventId: deltaEventId, eventSequence: 2, payload: { kind: "output_delta", vendorItemId: `d452-queued-delta-${ordinal}`, text: `Inspecting queued task ${ordinal}` } })).toEqual({ ok: true });
    expect(accept({ type: "relay:acp-semantic", registrationId: "hermes-acp", scope: command.scope, process, capabilities: { requests: "unsupported" }, eventId: candidateEventId, eventSequence: 3, payload: { kind: "assistant_completed", vendorItemId: `d452-queued-final-${ordinal}`, text: `Hermes queued turn ${ordinal} completed.` } })).toEqual({ ok: true });
  ...
  return {
    ...
    complete: (ordinal) => {
      ...
      if (!active) throw new Error(`D452 queued Hermes turn ${ordinal} is not pending`);
      pending.delete(ordinal);
      expect(accept({ type: "relay:acp-terminal", registrationId: "hermes-acp", scope: active.command.scope, process: active.process, status: "completed", eventId: `completed-${ordinal}`, eventSequence: 4 })).toEqual({ ok: true });
    },
  };
}
...
function observeTerminalOrder(registry: InMemoryRelayRegistry, timeline: string[]) {
  ...
  const subscribeSpy = spyOn(registry, "subscribeAcpExecution").mockImplementation((scope, process) => {
    const subscription = subscribe(scope, process);
  ...
  const containSpy = spyOn(registry, "containAcpExecution").mockImplementation((scope, process) => {
    timeline.push("contain-invoked");
    return contain(scope, process);
  });
...
function selectorFor(registry: InMemoryRelayRegistry) {
  return createTaskHarnessExecutionRouteSelector({
    ...
    registrations: [createHermesAcpTaskExecutionRouteRegistration(db, registry)],
  });
}

async function seedHermesTask(
  fixture: Fixture,
  metadata: Record<string, unknown>,
  prompt = "Complete the local Hermes task.",
) {
  return createTask(db, {
    ...
    targetRoomId: fixture.roomId,
    targetUserIds: [fixture.userId],
...
describe("D452 C2 Hermes ACP durable Task execution", () => {
  test("terminalizes the exact durable TaskRun when Electron disconnects between start and subscription, and fences late old-generation events", async () => {
    ...
    eventBus.on(observe);
    try {
      const transport = await registerStartDisconnectingHermesTransport(fixture);
      const readiness = transport.registry.getAcpSession(RELAY_ID, fixture.userId);
      if (!readiness) throw new Error("D452 authenticated v14 ACP session missing");
      const task = await seedHermesTask(fixture, hermesMetadata(readiness));
      ...
      expect(run).toMatchObject({ taskId: task.id, jobId: job.id, status: "errored", lastError: "ACP_EXECUTION_FAILED" });
      ...
      expect(raw).toEqual([{ content: SAFE_DELEGATED_TASK_FAILURE_RESULT }]);

      const started = transport.started();
      if (!started) throw new Error("D452 disconnect start missing");
      await transport.registry.register(RELAY_ID, fixture.userId, capabilities, () => undefined, 14, DESKTOP_SESSION_ID, 8, PAIRING_GENERATION);
      expect(transport.registry.acceptAcpMessage({
        relayId: RELAY_ID,
        userId: fixture.userId,
        message: { type: "relay:acp-terminal", registrationId: "hermes-acp", scope: started.scope, process: started.process, status: "failed", code: "upstream_failure", eventId: "late-disconnect-terminal", eventSequence: 2 },
  ...
  test("routes the real TaskRun through readiness, prepare, start, progress, raw report-back, and terminal acknowledgement", async () => {
    ...
    const observe = (event: ServerEvent) => {
      ...
      if (event.type === "message.new" && "content" in event && event.content === "Hermes completed the requested work.") eventTimeline.push("raw-report-back");
    };
    eventBus.on(observe);
    let terminalObserver: ReturnType<typeof observeTerminalOrder> | null = null;
    try {
      const { registry, sent } = await registerHermesTransport(fixture, "success");
      const timeline = eventTimeline;
      terminalObserver = observeTerminalOrder(registry, timeline);
      const readiness = registry.getAcpSession(RELAY_ID, fixture.userId);
      if (!readiness) throw new Error("D452 authenticated v14 ACP session missing");
      const task = await seedHermesTask(fixture, hermesMetadata(readiness));
      await dispatchTaskRun(task, {
        db,
      ...
      expect(run).toMatchObject({ taskId: task.id, jobId: job.id, status: "completed", modelId: null, resultText: "Hermes completed the requested work." });
      ...
      const raw = await db.select({ content: sessionMessages.content }).from(sessionMessages)
        .innerJoin(sessions, eq(sessions.id, sessionMessages.sessionId))
        .where(and(eq(sessions.roomId, fixture.roomId), eq(sessionMessages.content, "Hermes completed the requested work.")));
      expect(raw).toHaveLength(1);
      expect(sent.map((message) => message.type)).toEqual(["relay:acp-readiness", "relay:acp-prepare", "relay:acp-start"]);
      const progressIndex = observed.findIndex((event) => event.type === "task.progress");
      const rawMessageIndex = observed.findIndex((event) => event.type === "message.new" && "content" in event && event.content === "Hermes completed the requested work.");
      ...
      expect(registry.subscribeAcpExecution((sent[2] as Extract<RelayServerMessage, { type: "relay:acp-start" }>).scope, { connectionId: "d452-connection", processGeneration: 1, acpSessionId: "d452-acp", turnGeneration: 1, turnRef: "d452-turn" })).toBeNull();
  ...
  test("serializes a queued second Hermes Task until the first raw report-back retires its exact v14 turn", async () => {
    const fixture = await seedFixture("queued-follow-up");
    const timeline: string[] = [];
    const observed: ServerEvent[] = [];
    const firstResult = "Hermes queued turn 1 completed.";
    const secondResult = "Hermes queued turn 2 completed.";
    const observe = (event: ServerEvent) => {
      observed.push(event);
    ...
    let terminalObserver: ReturnType<typeof observeTerminalOrder> | null = null;
    try {
      const transport = await registerQueuedHermesTransport(fixture, timeline);
      terminalObserver = observeTerminalOrder(transport.registry, timeline);
      const readiness = transport.registry.getAcpSession(RELAY_ID, fixture.userId);
      if (!readiness) throw new Error("D452 authenticated v14 ACP session missing");
      const firstTask = await seedHermesTask(
        fixture,
        hermesMetadata(readiness),
        "Complete the first serialized Hermes task.",
      );
      const secondTask = await seedHermesTask(
        fixture,
        hermesMetadata(readiness),
        "Complete the queued Hermes follow-up.",
      );
      ...
      await eventually("first queued Hermes host turn", async () => transport.starts[0]);
      const secondDispatch = await dispatchTaskRun(secondTask, executionDeps);

      // Both TaskRuns are durably accepted, but their serialize route shares
      // this room's bot checkpoint.  Holding the first terminal must prevent
      // the second host turn from starting or emitting a report-back.
      ...
      await eventually("second queued Hermes host turn after first report-back", async () => transport.starts[1]);
      ...
      if (!firstStartAtHandoff) throw new Error("D452 first queued Hermes start missing at handoff");
      // Registry retirement is complete by the handoff, not deferred until
      // the later turn's terminal path.
      expect(transport.registry.subscribeAcpExecution(firstStartAtHandoff.scope, transport.processes[0]!)).toBeNull();
      ...
      if (!firstStart || !secondStart) throw new Error("D452 queued Hermes starts missing");
      ...
      expect(transport.registry.subscribeAcpExecution(firstStart.scope, transport.processes[0]!)).toBeNull();
      expect(transport.registry.subscribeAcpExecution(secondStart.scope, transport.processes[1]!)).toBeNull();
    } finally {
      terminalObserver?.restore();
      eventBus.off(observe);
      await cleanupFixture(fixture);
  ...
  test("reports an authenticated failed terminal before its one final containment attempt and retires it after acknowledgement", async () => {
    ...
    const observe = (event: ServerEvent) => {
      observed.push(event);
      if (event.type === "message.new" && "content" in event && event.content === SAFE_DELEGATED_TASK_FAILURE_RESULT) eventTimeline.push("safe-failure-report");
    ...
    try {
      const timeline = eventTimeline;
      const { registry, sent } = await registerHermesTransport(fixture, "failed", timeline);
      terminalObserver = observeTerminalOrder(registry, timeline);
      const readiness = registry.getAcpSession(RELAY_ID, fixture.userId);
      if (!readiness) throw new Error("D452 authenticated v14 ACP session missing");
      const task = await seedHermesTask(fixture, hermesMetadata(readiness));
      ...
      expect(run).toMatchObject({
        ...
        resultText: SAFE_DELEGATED_TASK_FAILURE_RESULT,
      });
      expect(job.status).toBe("failed");
  ...
  test("keeps metadata-less Native Tasks on the generic selector's undefined path", async () => {
    ...
    try {
      const task = await seedHermesTask(fixture, {});
      const registry = new InMemoryRelayRegistry();
      await registry.register(RELAY_ID, fixture.userId, capabilities, () => undefined, 14, DESKTOP_SESSION_ID, 7, PAIRING_GENERATION);
