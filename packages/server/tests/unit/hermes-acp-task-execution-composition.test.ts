...
import { createHermesAcpTaskExecutionComposition } from "../../src/acp/task-execution-composition";
...
describe("Hermes ACP task execution composition", () => {
  ...
  test("locks Task → TaskRun → Job, rejects every authority drift before mutation, and permits exact idempotency only after all checks", async () => {
    ...
    for (const [_name, task, run, job] of drifts) {
      ...
      const result = await createHermesAcpTaskExecutionComposition(asDb(db)).lifecycle.writer.linkJob(facts);
    ...
    expect(await createHermesAcpTaskExecutionComposition(asDb(idempotent)).lifecycle.writer.linkJob(facts)).toBe("already_linked");
  ...
  test("propagates the second completion error; Hermes failure preserves its stable code and opts into only the fixed safe Room text", async () => {
  ...
  test("retries one identical safe Hermes failure report-back after a transient post-transition delivery error", async () => {
    const failures: unknown[] = [];
    const db = new CompositionDb([...exactReaders(), ...exactReaders()]);
    const composition = createHermesAcpTaskExecutionComposition(asDb(db), {
      complete: async () => true,
