...
import { CompanionWindowManager } from "./companion-window";
import { isMainSpaceVisible } from "./window-presence";
...
// P0 — embedded-browser password layer. HUMAN-ONLY: these
...
// state in `~/Library/Application Support/Nautilo/` is
...
// `~/.nautilo${suffix}/desktop-auth[-<profile>].json` (via
...
const companionWindows = new CompanionWindowManager({
  mainWindow: () => mainWindow,
  isMainSpaceVisible,
  activeOwner: () => {
    const current = serverSessions.active;
    const contents = current?.view?.webContents;
    return current?.signedIn && contents && !contents.isDestroyed() && isVerifiedWorkbenchRenderer(contents)
      ? { contents, serverUrl: current.serverUrl, scope: current.scope } : null;
  },
  preload: path.join(__dirname, "preload-companion.js"),
  pickFiles: parent => pickFilesForComposer(parent),
});
...
function logColdBootDiagnostic(diagnostic: ColdBootDiagnostic): void {
  ...
  log.info(
    `[desktop][cold-boot] cold-boot generation=${diagnostic.generation} phase=${diagnostic.phase} category=${diagnostic.category} durationMs=${diagnostic.durationMs} acceptedGeneration=${diagnostic.acceptedGeneration} stateChanged=${diagnostic.stateChanged}`,
  );
}
 ...
 *
 * The security check uses `fs.realpathSync` on
...
function isPathWithinAllowedRoot(targetPath: string): boolean {
  ...
  // default. Boot ensures `genieWorkspaceRoot` is always
...
async function pickFilesForComposer(parent: BaseWindow | null = mainWindow): Promise<
  Array<{
    name: string;
    sizeBytes: number;
    base64: string;
...
> {
  ...
  // explicitly chose these files in the OS dialog this session. No HMAC or shared-secret model is used.
  const picked: Array<{
    name: string;
    sizeBytes: number;
    base64: string;
...
function commitCurrentFolderPath(
  ...
  if (mainWindow && !mainWindow.isDestroyed()) {
    ...
    // Deprecation alias for older windows. Emit both so any
  ...
  try {
    ...
    if (app.isPackaged && observedFingerprint && !sourceDevelopmentAuthority) {
      ...
      if (currentConfig && activeAuthority?.serverFingerprint === null &&
        ...
        log.info("[desktop][cold-boot] completed verified legacy active authority");
      }
    }
    ...
    if (
      ...
      log.info(
        `[desktop][auth] Logto auth resolved (endpoint=${nextLogtoConfig.endpoint})`,
      );
    ...
    log.warn(`[desktop][auth] ${detail}`);
    return false;
  } catch (err) {
    const detail = err instanceof Error ? err.message : String(err);
    log.warn(`[desktop][auth] verified health projection failed: ${detail}`);
    return false;
  }
}
...
async function ensureRelayToken(serverUrl: string): Promise<string | null> {
  ...
  // The deviceGroupId is derived only from the canonical
  ...
  if (!trustedServerFingerprint) {
    ...
    log.warn(
      "[desktop][pairing] No trusted server fingerprint; deferring first relay pair",
    );
    return null;
  }
  if (
  ...
  if (!accessToken) {
    log.warn(
      "[desktop][pairing] No Logto access token; deferring relay pair until sign-in",
    );
    return null;
  }
  try {
    ...
    log.info("[desktop][pairing] Paired new relay token");
    return minted;
  } catch (err) {
    log.error(
      `[desktop][pairing] Relay pairing failed; deferring relay startup: ${
...
async function startRelayForSession(session: ServerSession): Promise<void> {
  ...
  // never matches, so every relay tool call fails silently.
  const userId = await resolveRelayUserId(serverUrl);
  if (!userId) {
    log.warn(
      "[desktop][pairing] Could not resolve relay userId; deferring startRelay until sign-in",
    );
    return;
  }
  ...
  if (!relayToken) {
    log.warn("[desktop][pairing] No relay token available; deferring startRelay");
    return;
  }
  ...
  try {
    ...
    if (isRelayTokenRejection(err)) {
      log.warn(
        "[desktop][pairing] Cached relay token was rejected; clearing scoped token and re-pairing",
      );
...
async function refreshRelayForCurrentFolder(reason: string): Promise<void> {
  ...
  if (!serverUrl) {
    log.info(
      `[desktop][relay] Skipping relay root refresh after ${reason}; server URL is not resolved yet`,
    );
    return;
  }
  ...
  relayRootRefreshPromise = (async () => {
    ...
    if (status === "connected" || status === "connecting") {
      ...
      log.info(
        `[desktop][relay] Refreshing relay root after ${reason}: ${nextRoot}`,
      );
      try {
        await stopRelay();
      } catch (err) {
        log.warn(
          `[desktop][relay] Failed to stop relay for root refresh: ${
            err instanceof Error ? err.message : String(err)
          }`,
        );
        return;
      ...
      log.info(
        `[desktop][relay] Relay is ${status}; booting with current folder after ${reason}: ${nextRoot}`,
      );
    }
...
async function handleSignOut(): Promise<void> {
  ...
  try {
    ...
    log.warn(
      `[desktop][connections] failed to stop relay on sign-out: ${String(err)}`,
    );
  }
  try {
    ...
    log.error(`[desktop][computer-use] failed to revoke Computer use on sign-out: ${String(err)}`);
  }
  if (!config) return;
  try {
    ...
    log.warn(
      `[desktop][connections] failed to disable Codex on sign-out: ${String(err)}`,
    );
  }
  clearTokens();
  ...
  try {
    ...
    log.warn(
      `[desktop][workstation] failed to deactivate active profile on sign-out: ${String(err)}`,
    );
  }
  refreshAuthMenuState(false);
  try {
    ...
    log.warn(
      `[desktop][auth] failed to clear Logto Electron cookies: ${String(err)}`,
    );
  }
  broadcastAuthState("signed-out");
}
...
/**
 * P4.2 — sender-id gate for module-level handlers.
 *
 ...
 * without its complete readiness marker is intentionally ineligible.
 */
...
// The renderer supplies only the transient own-Human PIN. Electron main owns the
...
// task 2.3 — stable, initially-disabled relay port. No Codex runtime,
...
async function restoreCodexConnectionBeforeRelayStart(
  ...
  try {
    ...
    log.warn(
      `[desktop][connections] failed to restore enabled Codex connection: ${String(err)}`,
    );
  }
}
...
/**
 * Ready enrollment reuses the canonical PIN, ownership, attestation, and mint
...
/**
 * Electron's local pre-spawn fence. The relay client supplies its own
...
/**
 * P0 — human-only sender gate for the `passwords:*` channels. Allowed
...
// P0 — surface the built guest-preload path (a file:// URL) to the
...
// chunk 2 — pick + validate + commit in one round-trip. Used by
...
// chunk 2 — recent current folders surfaced to the renderer for
 ...
 * The current folder has no default. The equivalent for
...
function warnDeprecatedIpc(channel: string): void {
  ...
  log.warn(
    `[desktop][deprecated-ipc] ${channel} has been renamed; update callers to use the currentFolder:* equivalent.`,
  );
}
...
ipcMain.handle("coldBoot:retry", async (e) => {
  ...
  if (!connectBootstrapEntryHref || !mainWindow || mainWindow.isDestroyed()) {
    log.warn("[desktop][cold-boot] coldBoot:retry — no bootstrap entry or window");
    return;
  }
...
ipcMain.handle("coldBoot:useThisServerAnyway", async (e) => {
  ...
  try {
    ...
    log.warn("[desktop][cold-boot] coldBoot:useThisServerAnyway failed:", err);
  }
});
...
/**
 * `servers:add`, surfaced by Phase 4 as the persistent
...
ipcMain.handle("servers:forget", async (e, rawUrl: unknown) => {
  ...
  try {
    ...
    log.warn("[desktop][servers] failed to clear forgotten server state:", err);
  ...
  try {
    ...
    log.warn("[desktop][servers] failed to update forgotten server config:", err);
...
function updateTrayMenu(): void {
  ...
  tray.setContextMenu(
    Menu.buildFromTemplate([
      ...
      {
        label: "Open Folder...",
        // chunk 2 — direct pick + validate + commit. The modal
...
//
// P4.6 — production posture: when a local CA cert exists at
...
/**
 * P4.6 — chain-verify a presented Electron Certificate against the
...
function chainAnchorsTo(
  ...
  // A malformed certificate must fail closed.
...
// The placeholder screen and routing-hook stub are replaced by the
...
/**
 * The routing hook maps from cached `GET /api/setup/status`
...
/** Build the query string for `GET /api/voices/catalog`. */
 ...
 *
 * wires this for every API-proxy handler in
...
async function boot(): Promise<void> {
  ...
  // chunk 2 — prune recent-current-folders entries whose folders
  ...
  const initialColdBootObservation = launchConnectionCohort
    ? (() => {
        ...
        // exact facts; avoiding a second launch probe prevents a visible bounce.
        initiateLocalShell();
        coldBootLifecycle = "resuming";
  ...
  // P4c — exhaustive switch on all 4 ServerClaimState values.
  ...
  if (logtoConfig() && await shouldShowOnboarding(serverUrl)) {
    if (!loadTokens()) {
      console.log(
        "[desktop] Logto sign-in required before onboarding wizard",
      );
...
// P3.7 — About-this-app dialog metadata. macOS reads
...
app.on("before-quit", (event) => {
  ...
  void (async () => {
    ...
    try {
      ...
      log.warn(`[desktop][media] failed to dispose media proxies during quit: ${String(err)}`);
    }
    try {
      await stopRelay();
    } catch (err) {
      log.warn(
        `[desktop][connections] failed to stop relay during quit: ${String(err)}`,
      );
    }
    try {
      await browserResearchTargetManager?.disposeAll();
    ...
    try {
      ...
      log.warn(
        `[desktop][connections] failed to shut down Codex during quit: ${String(err)}`,
      );
    }
    try {
      await computerUseHostBroker.close();
      ...
      log.warn(
        `[desktop][computer-use] failed to close Computer Use Host during quit: ${String(err)}`,
      );
    }
    ...
    try {
      ...
      log.warn(
        `[desktop][workstation] failed to deactivate active profile during quit: ${String(err)}`,
      );
    } finally {
