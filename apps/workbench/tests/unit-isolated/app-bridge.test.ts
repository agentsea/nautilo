...
describe("installAppBridge", () => {
  ...
  test("unified picker admits canonical Video only, gates references, and rejects cross-purpose results", async () => {
    for (const scenario of ["valid", "ungranted", "invalid-document", "cross-purpose", "single-overflow"] as const) {
      ...
      const result = { kind: "ready", imports: scenario === "single-overflow" ? [image, image] : [], references: [], mediaIds: scenario === "cross-purpose" ? ["media_wrong"] : [], failures: [] };
      ...
      window.dispatchEvent(new MessageEvent("message", { data: { type: "nautilo.app.media.req", requestId: scenario, op: "pick", purpose: scenario === "ungranted" ? "references" : "media", multiple: scenario !== "single-overflow" }, source: contentWindow as unknown as MessageEventSource }));
      await waitUntil(() => posted.length === 1);
      expect(posted[0]).toMatchObject({ ok: true, value: scenario === "valid" ? result : { kind: "unavailable", code: scenario === "ungranted" ? "unsupported_environment" : scenario === "invalid-document" ? "invalid_document" : "invalid_response" } });
