...
export function CompanionConversationProvider({ snapshot, children }: { snapshot: CompanionSnapshot; children: ReactNode }) {
  ...
  const runtime = useExternalStoreRuntime({ messages: snapshot.messages, convertMessage: identityMessage, isRunning: snapshot.busy,
    onNew: async () => {} });
...
export function CompanionComposer({ draft, name, busy, onDraft, onSubmit, controls, attachments = [], onRemoveAttachment, error, sending = false }: {
