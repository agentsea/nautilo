...
export function EventFeedBell({
  ...
  return (
    <button
      ...
      type="button"
      aria-expanded={open}
      aria-label={quiet ? "Events, quiet" : `Events, ${unreadLabel}`}
      title={quiet ? "Events are quiet" : "Events"}
      onClick={onClick}
    ...
    >
      {quiet ? <BellOff className="h-4 w-4" aria-hidden="true" /> : <Bell className="h-4 w-4" aria-hidden="true" />}
      {!quiet && unreadCount !== null && unreadCount > 0 ? (
        <span
          aria-hidden="true"
